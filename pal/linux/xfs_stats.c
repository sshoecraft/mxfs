...
#ifdef CONFIG_XFS_QUOTA
	if (!proc_create_single("fs/mxfs/xqmstat", 0, NULL, xqmstat_proc_show))
		goto out;
	if (!proc_create_single("fs/mxfs/xqm", 0, NULL, xqm_proc_show))
		goto out;
#endif
	return 0;

out:
	remove_proc_subtree("fs/mxfs", NULL);
	return -ENOMEM;
}
