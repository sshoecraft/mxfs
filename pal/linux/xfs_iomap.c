	...
	u64			sequence_cookie)
{
	...
	if (imap->br_startblock == HOLESTARTBLOCK) {
		...
		iomap->addr = BBTOB(daddr + target->bt_sector_offset);
		if (mapping_flags & IOMAP_DAX)
			iomap->addr += target->bt_dax_part_off;
		pr_debug("mxfs: iomap addr=%lld (daddr=%lld + off=%lld)\n",
			 (long long)iomap->addr, (long long)BBTOB(daddr),
			 (long long)BBTOB(target->bt_sector_offset));
	...
	bool		update_isize)
{
	...
	do {
		...
		if (i_size) {
			 ...
			 * so the durable size will stay short forever. */
	...
	struct iomap		*srcmap)
{
	 ...
	 * which hung cross_visibility.  The di_size empty-content residual (a
	 ...
	 * stale state (cached-PR? stale=false?) at the failing read without
	 * instr=1 (which is 100x slower and hides the race). */
	{
		struct mxfs_v5_dlm;
	...
	bool			*did_zero)
{
	...
	return mxfs_iomap_zero_range(inode, pos, len, did_zero,
	...
	bool			*did_zero)
{
	...
	if (IS_DAX(inode))
		return dax_truncate_page(inode, pos, did_zero,
					&xfs_dax_write_iomap_ops);
	return mxfs_iomap_truncate_page(inode, pos, did_zero,
			&xfs_buffered_write_iomap_ops, &xfs_iomap_write_ops);
}
