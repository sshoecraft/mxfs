	...
	struct iov_iter		*to)
{
	 ...
	 * per-access reload/acquire works for the di_size/content residual
	 * (getattr/iomap/open/per-read/di0-CAW/di0-direct all net-negative).  The
	 * stale grant-less-inode refresh MUST be EVENT-DRIVEN at the DLM layer.
	 */
	...
	struct iov_iter		*to)
{
	 ...
	 * repeat reads hit the cached-PR fast path with no CAW.  No-op single-node /
...
write_retry:
	...
	ret = mxfs_iomap_file_buffered_write(iocb, from,
	...
	struct file	*file)
{
	 ...
	 * resolved a dentry whose inode is (or presents as) a DIR.  Log every
	 * directory open under a multi-node mount with the ino, mode and the
	 * name cat used; if node1_after_1 appears here, the bug is a stale
	 * dentry binding a DIR inode object distinct from the repaired REG one.
	 */
	...
	struct file		*file)
{
	...
	/*
	 * v0.10.36 read-once demote: a read-only close of a PR-held clean
	 * regular file queues the DLM release so the PR grant does not
	 * linger to be BAST-stripped one-by-one by a later unlink storm
	 * (32-node dir_reuse rm: 31 lingering PR holders x serialized slot
	...
	struct dir_context *ctx)
{
	...
	/*
	 * Take ILOCK_SHARED so the MXFS DLM hook (in xfs_ilock) fires.
	 * Without this, shortform-dir readdir returns from xfs_readdir
	 * without ever taking ILOCK (xfs_dir2_readdir.c:537), so a
	 * peer's directory updates are never observed — the in-memory
	 * if_data is stale until something else reloads the inode.
	 * For non-shortform formats, xfs_readdir takes ILOCK_SHARED
	 * again via xfs_ilock_data_map_shared; nested ILOCK_SHARED is
	 ...
	 * assumption that xfs_readdir only re-takes ILOCK_SHARED (nested-shared is
	 * fine).  That is FALSE for a non-shortform dir whose data-fork extents are
	 * NOT loaded (need_iread=1, the state a peer-driven mxfs_dlm_reload_inode
	 * leaves behind via xfs_idestroy_fork + xfs_inode_from_disk): then
	 * xfs_ilock_data_map_shared takes ILOCK_EXCL, and upgrading SHARED->EXCL on
	 * the same rwsem by the same thread DEADLOCKS forever — wedging the hot
	 * shared dir (the zero_silent_loss verify-find hang; the BMBT reload also
	 * BAILs "i_lock contended" so need_iread never clears = permanent).
	 *
	 * Only SHORTFORM (LOCAL-format) readdir needs an outer lock: xfs_dir2_sf_
	 * getdents reads the inline if_data WITHOUT taking ILOCK, so we hold
	 * ILOCK_SHARED to fire the MXFS DLM coherency hook and stabilize if_data
	 * across the read — and shortform never loads extents, so it never wants
	 * EXCL (no self-deadlock).  For every other format xfs_readdir takes its
	 * OWN ILOCK (SHARED normally, EXCL when it must xfs_iread_extents), which
	...
	struct xfs_zone_alloc_ctx *ac)
{
	...
	if (IS_DAX(inode)) {
		...
		/*
		 * MXFS: counted EX hold across page_mkwrite — the write-fault
		 * flavor of the xfs_filemap_fault folio-lock vs inode-DLM
		 * inversion (iomap_page_mkwrite locks the folio, then
	...
	struct vm_fault		*vmf)
{
	...
	trace_xfs_read_fault(ip, 0);

	/*
	 * MXFS: counted cluster-ilock hold (PR) across the whole fault.
	 *
	 * filemap_fault adds folios to the page cache LOCKED, then calls
	 * ->readahead / ->read_folio, whose iomap_begin takes
	 * xfs_ilock(ILOCK_SHARED) -> the per-inode cluster DLM.  Without an
	 * outer counted hold, that inner acquire can sleep waiting for this
	 * inode's own bast drain — while the drain sits in
	 * invalidate_inode_pages2 -> __folio_lock on a folio THIS task
	 * already locked.  Mutual wait, permanent (test20 2026-07-18,
	 * mmap_coherency 32/cawd: python3 in P73-WAITSTALL
	 * state=DEMOTING work_busy=RUNNING forever vs mxfs-ino-bast kworker
	 * in folio_wait_bit_common; relatime atime-EX BASTs supplied the
	 * demotes).  The buffered-read syscall path is immune only because
	 * xfs_file_buffered_read counts a DLM hold via IOLOCK_SHARED before
	 * any folio lock; do the same here.  DLM hold ONLY — taking i_rwsem
	 * inside a fault would invert against write()'s rwsem->mmap_lock
	 * ordering.  With the hold counted, the inner acquire nests via the
	 * FIX-1 nested-hold arms and the bast release re-arms at its
	 * holders!=0 gate until the fault finishes; ilock_end's 1->0 refire
	 * then drives the deferred release.  Readahead folios still in
	 * flight when we drop the hold unlock at bio completion — I/O-bound,
