...
#include "../pal.h"
#include "mxfs_libiscsi_fp.h"      /* generated by Kbuild; see Kbuild */
#include "mxfs_kcompat.h"          /* MXFS_HAVE_*: generated by Kbuild; see Kbuild */
...
/* 0.89.80 — the release this module was compiled for, and the structural
 * fingerprint of that kernel's libiscsi TMF declarations (generated by Kbuild
 * from the build's own headers).  See pal.h. */
const char *mxfs_pal_kernel_build_release(void)
{
	return UTS_RELEASE;
}
EXPORT_SYMBOL_GPL(mxfs_pal_kernel_build_release);

const char *mxfs_pal_libiscsi_fingerprint(void)
{
	return MXFS_LIBISCSI_FP;
}
EXPORT_SYMBOL_GPL(mxfs_pal_libiscsi_fingerprint);
                          ...
                          int *resid_out)
{
...
#ifdef MXFS_HAVE_SCSI_CMND_SENSE_LEN
...
#else
	/*
	 * RHEL 9 keeps struct scsi_request inside scsi_cmnd (upstream removed
	 * it in 5.18).  There a passthrough CDB, its retries and its outcome
	 * live in the scsi_request: the SCSI layer points scmd->cmnd at
	 * scsi_req(req)->cmd only at dispatch, so before that scmd->cmnd is
	 * not a buffer, and writing the CDB through it would fault.
	 */
	{
		struct scsi_request *sreq = scsi_req(req);

		sreq->cmd_len = COMMAND_SIZE(cdb[0]);
		memcpy(sreq->cmd, cdb, sreq->cmd_len);
		sreq->retries = 0;
	}
#endif
	req->timeout = 30 * HZ;
	...
	ret = scmd->result;
#else
	/* the outcome, as 5.14's __scsi_execute() read it */
	if (sshdr_out)
		scsi_normalize_sense(scsi_req(req)->sense,
				     scsi_req(req)->sense_len, sshdr_out);
	if (resid_out)
		*resid_out = scsi_req(req)->resid_len;

	ret = scsi_req(req)->result;
#endif
