 ...
 * 32-node CAW rig (`sg_persist --in --read-full-status`): 64
...
/* single-use ABSENT answers remembered per (key, bracket); one per
...
struct mxfs_scsipr_ctx {
	...
	void            *key_live_data;
	/* the type of the reservation this mount last OBSERVED in force
	 ...
	 * already-removed key). */
	bool            registered;
	bool            nexus_reused;   /* the plain REGISTER conflicted and
	...
	bool            gate_held;
	/*
	 * (0.59.2, design-consult STOP-SHIP #2 blockers 1 and 4): the key-state
	...
	/*
	 * (0.61.0, design-consult design ruling D6): THE SINGLE-USE ABSENCE
	...
	/*
	 * (0.61.0, D8): probe-thread lifecycle.  probe_stop/probe_kick/
...
/*
 * WHAT A FENCE ATTEMPT ACTUALLY PROVED.
 *
 * Until mxfs_scsipr_fence_node returned an int that callers
...
enum mxfs_fence_kind {
	...
	MXFS_FENCE_KIND_SINGLE_NODE_EXCLUSIVE = 17, /* REVOKED 0.89.18.  No PR
											   ...
											   * point is not reused */
	MXFS_FENCE_KIND_NO_VICTIM_KEY       = 18, /* the victim's 64-bit
	...
	MXFS_FENCE_KIND_SELF_SUCCESSION_DONE = 19, /* (docs/whole-cluster-
...
/*
 * self-succession, successor side: replace THIS host's previous
...
/* did the last mxfs_scsipr_register find our own derived key already
...
/* the mount is being refused but the registration on this nexus is
 ...
 */
/*
 * HOW FAR THE ATTEMPT GOT — the COMMAND-SUBMISSION BOUNDARY.
 *
 ...
 * unmountable.
 *
 * Deriving this from `kind` would be wrong and the design-consult ruling says
...
/* Lifecycle */
/* the key is the 64-bit per-boot key selected through the PR
...
/* is `key` registered on this LU right now (READ KEYS)? Used by the
...
/*
 * (D-377 / D-0356 design-consult STOP-SHIP blocker 3, merge criteria 2, 5,
 ...
 *
 * (0.59.2, design-consult STOP-SHIP #2): the proof is now COHERENT and
...
/* a FRESH bracket proving OUR registration is live inside the
...
/*
 * (0.61.0, design-consult design ruling D1 + D6): SETTLE-ABSENT — the only
...
/*
 * (0.61.0, D1(2)/(3)): THE HOST-WIDE DEPARTURE MUTEX lives here,
...
/*
 * (0.61.0, D8): probe-thread stop is BOUNDED.  0 = joined; -ETIMEDOUT
...
/* install the ledger-selected key before REGISTER (-EBUSY after). */
...
/*
 * plain REGISTER; -EEXIST (P305-PR-PREDECESSOR-KEY-PRESENT) when
 ...
 * gate cannot answer that, because it runs after our own RESERVE.
 ...
 *
 * (D-PR-RESERVATION-HEALTH-UNMONITORED-AFTER-MOUNT-381): the admission
 ...
 *
 * the outcome is reported through *out, NOT through the return
 ...
 * auto-re-register. */
/*
 * `arm_submit` is called on the LAST LINE before the PREEMPT AND
...
/*
 * — DOES THE EXCLUSION STILL HOLD? A DETECTOR, NOT A FENCE.
 *
 ...
 * fenced (design-consult ruling, follow-up).
 *
 ...
 * replayer.  It cannot PREVENT a write that races the check — design review ranked it
...
/* target-authoritative "am I the fenced node?" probe — PR IN /
...
/*
 * ADMISSION-TIME FENCING-CAPABILITY VALIDATION.
 *
 ...
 *
 * The abort leg is not hypothetical: proved MXFS had been running for
