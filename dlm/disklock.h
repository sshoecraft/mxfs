...
#include "recov_obl.h"          /* sess462: obligation record + list formats */
...
/*
 * sess450 (D-0356 / D-377 two-phase departure, sess449 design-consult ruling):
 ...
 *     asynchronous key lookup ctx->key_state_fn (tri-state, sess451; a
 *     table read fed by a probe thread's coherent READ KEYS / READ
 *     RESERVATION / READ KEYS bracket, sess452 — no PR I/O on the
 ...
 *     (P304-RETIRE-EXPIRED-WITHDRAWN); the sess9 withdraw pipeline fences
 ...
 *     proves our registration live (sess452, blocker 3; there is no OWN
 ...
 *     a clustered one never does (sess452, blocker 2), and a record
...
/*
 * sess465 (D-REJOIN-CLAIM-ENOSPC-DURING-TRANSIENT-SWEEP-GUARD-AT-CAPACITY-0523,
 ...
 * from MEASURED 32-node walls (tests/evidence, sess465 extraction of every
 ...
 *
 * sess438 (docs/whole-cluster-restart.md item 2): 23 → 19, freeing 64 B
...
/*
 * sess64 — the DURABLE RECOVERY DESCRIPTOR (design-consult ruling, sess63).
 *
 ...
 */
/*
 * sess405: v3 inserts the SNAPSHOTTING stage between FENCING and FENCED and
 ...
 */
/*
 * sess405 (docs/recovery-manifest.md): SNAPSHOTTING sits between FENCING and
...
/* sess434: the victim's own record carried MXFS_HB_FEAT_ADOPTED (pass-2 fresh
...
/*
 * sess440 (docs/whole-cluster-restart.md §6.2, design-consult ruling Q2): the
...
/*
 * sess405 (docs/recovery-manifest.md): the victim's FENCE-TIME AUTHORITY
...
/*
 * sess405: the victim's sealed fence-time manifest is STRUCTURALLY invalid
...
/* sess412: transaction assembly crossed an ophdr discontinuity in a
...
/* sess419: a TEST INJECTION published by the recovery owner through the
...
/*
 * ── THE RECOVERY-MANIFEST POINTER (sess405, docs/recovery-manifest.md) ─────
 *
...
struct mxfs_recov_body {
    ...
    struct mxfs_recov_manifest_ptr mptr;    /* sess405: manifest pointer */
...
/*
 * ── THE RECOVERY MANIFEST ON DISK (sess405, envelope rman region) ──────────
 *
...
/*
 * 0.41.1 (sess434, D-0354 lap 2 — design-consult ruling): WRITE-TIME provenance that
...
/*
 * sess441 (docs/whole-cluster-restart.md §6.5 shape B, design-consult ruling): this
...
/*
 * sess438 (docs/whole-cluster-restart.md item 2, design-consult ruling ccmemory
...
struct mxfs_disklock_heartbeat {
     ...
     */
    union {
        struct mxfs_evict_ring  evict;      /* sess55; sess438: 320B (19 entries) */
        struct mxfs_recov_body  recov;      /* sess64: 120B descriptor + outcome + pad */
    };
    struct mxfs_hb_identity ident;          /* sess438: 64B host/boot/PR-key identity */
...
struct mxfs_disklock_node_track {
    ...
    uint64_t        last_seq;
    bool            seq_seen;
    /* sess436: P-HB-INC-ZERO printed once per zero episode (the sector may
...
/*
 * sess450/sess451: "is this PR key registered on the LUN right now?" — the
 * READ KEYS the monitor issues to settle a RETIRE_PENDING record (see the
 * flag's comment).  TRI-STATE plus one special case (sess451, design-consult
 ...
 *
 * sess452 (0.59.2, design-consult STOP-SHIP #2 blockers 3 and 4): there is no OWN
...
/* sess454 (0.61.0, D1): the ABSENT settlement callback — contract at the
...
/*
 * sess419 (D-PURGE-NONATOMIC-PUBLICATION verification, design-consult ruling):
 ...
 *
 * sess383 (design-consult ruling): the callback now RETURNS a disposition instead of
...
struct mxfs_disklock_ctx {
    ...
    /* sess405: the envelope's recovery-manifest region (0 = none; then no
    ...
    /* sess440 (§6.2): this node is a whole-cluster bootstrap owner — no slot;
     * descriptors it owns carry MXFS_RECOV_F_OWNER_BOOTSTRAP. */
    bool                    owner_bootstrap;
    /* sess440: incarnation drawn at the bootstrap claim, consumed ONCE by the
     ...
     * forbids. */
    bool                    snlocal;
    /* sess434: this incarnation's claim was a pass-2 fresh claim — set by
    ...
    bool                    claim_fresh;
    /* sess441 (§6.5): own record carries MXFS_HB_FEAT_BOOTSTRAP_PENDING */
    ...
    /* sess426 (D-0347): when OUR heartbeat record last landed (CAS ok).
    ...
    uint64_t                hb_inc_zero_retained;   /* sess426: P-HB-INC-ZERO count */
    int                     hb_pid;
    ...
    /* sess450/451/452: RETIRE_PENDING settlement (see the flag's comment):
    ...
    void                             *key_state_data;
    /* sess454 (0.61.0, D1): the ABSENT settlement path (see typedef) */
    mxfs_disklock_settle_absent_fn   settle_absent_fn;
    void                             *settle_absent_data;
    /* sess451: per-slot rate limit for the UNKNOWN-key escalation */
    ...
    /* sess453 (D-0519): the victim incarnation whose CERTIFIED fence
    ...
    /* sess453 (0.60.0, review-#3 condition 8): a settlement write whose
    ...
    bool                             hb_img_valid;
    /* sess435 (D-0359 step 1): the slot claim landed through a real
    ...
    /*
     * sess438: this incarnation's host/boot/PR-key identity block, stamped
    ...
    struct mxfs_disklock_ident_obs {
        ...
        uint64_t        key;
        uint32_t        key_gen;
        uint8_t         host_uuid[16];  /* sess439: frozen with the key */
    ...
    mxfs_disklock_dbg_purge_hook     dbg_purge_hook;     /* sess419, test only */
    ...
    /*
     * sess405 (docs/recovery-manifest.md, writer guard S4): the PROTECTED
     ...
     * fence and the scan (GPT review sess405 item 1).  Recomputed
    ...
    /* sess407: bumped by every out-of-pass publication (synchronous refresh,
    ...
    /* sess407 (design-consult ruling Q1): gen validation, mask installation and the
...
/*
 * sess438: install this incarnation's identity block BEFORE the slot claim
...
/*
 * sess438: the PR key of the incarnation whose death was declared on `slot`
 ...
 */
/* sess439: the victim's host/boot uuids frozen with its key in the death
...
/*
 * sess435 (D-0359 step 1, design-consult ruling): OPERATIONAL CAW capability of the
...
/* sess449 (D-0356/D-377): after a clean release whose late PR unregister
...
/* sess450: a departure with NO PR key to retire (no PR on the target, or
...
/*
 * sess452 (0.59.2): the P305 same-boot settlement — the ONLY path that may
...
void mxfs_disklock_set_key_state_fn(struct mxfs_disklock_ctx *ctx,
                                    mxfs_disklock_key_state_fn fn,
                                    void *data);
/* sess451: the result vocabulary of one RETIRE_PENDING settlement attempt. */
...
/*
 * sess454 (0.61.0, design-consult design ruling D1): the ABSENT settlement no longer
...
int  mxfs_disklock_retire_cas_empty(struct mxfs_disklock_ctx *ctx, uint32_t slot,
                                    ...
                                    void *validate_data);
/*
 * sess451: settle one slot's RETIRE_PENDING record from the MOUNT thread —
...
/* sess441 (§6.4): mark pending AND freeze the victim's PR identity from the
 ...
 *      worker re-drives unconditionally (sess381, sess405).
...
/*
 * sess405: certify writes the certificate bytes and moves FENCING ->
...
/*
 * sess405 — the recovery manifest (docs/recovery-manifest.md).
 *
...
/* sess442: the same load for the bootstrap owner's adopted slot K, whose
...
int mxfs_disklock_recovery_manifest_read_escrow(struct mxfs_disklock_ctx *ctx,
                                                ...
                                                uint32_t *out_count,
                                                bool *out_no_caw);
/* sess442: the certified descriptor's validated manifest pointer, for the
 ...
 * prover's (sess74 ruled that inference unsound).  If the victim key is
 ...
 *
 * sess405: at SNAPSHOTTING the certificate is already proved and durable, so
...
/*
 * sess413 (D-532): durably give back every recovery lease this incarnation
...
/*
 * sess462 (D-FOREIGN-SLICE-INTENTS-ABANDONED item 5, increment 2; ruling
...
/*
 * sess374 (sess363 design-consult ruling, D-REFUSAL-GRANT-FREEZE-OUT-OF-CLOSURE-356):
...
int mxfs_disklock_terminal_gate_check(struct mxfs_disklock_ctx *ctx, int slot,
                                    ...
                                    uint64_t *ag_mask);
/*
 * sess494 (D-0493): CONTEXT-FREE terminal-guard classification of one
...
/*
 * sess330 (design-consult ruling): leaseless terminalization of the LEGACY
...
int mxfs_disklock_recovery_read_outcome(struct mxfs_disklock_ctx *ctx,
                                    int slot,
                                    struct mxfs_recov_outcome *oc_out);
/* sess405: install the protected-mask consumer (see ctx->protected_mask).
...
/* sess407: LOCAL PROOF — the caller has just made a descriptor at stage >=
...
bool mxfs_disklock_slot_holds_incarnation(struct mxfs_disklock_ctx *ctx,
                                          ...
                                          mxfs_epoch_t epoch,
                                          bool *out_read_ok);
/* sess424: enumerated terminal state of a victim's slot (ZERO record, or an
 ...
 * sess451 (design-consult STOP-SHIP blocker 2): a RETIRE_PENDING stamp — a clean
...
/*
 * sess440 (docs/whole-cluster-restart.md §6.2): whole-cluster bootstrap owner
...
/* sess441: fs_gen fold of a volume uuid (identical to set_fs_identity's). */
uint32_t mxfs_disklock_fs_gen_of(const uint8_t *fs_uuid);
/*
 * sess441 (§6.5 shape B): the bootstrap owner ADOPTS certified victim slot
...
/*
 * sess442 (§6.7 same-boot RESUME): adopt the incarnation the bootstrap
...
/*
 * sess442 (§6.7): re-take OUR OWN adopted K record after a same-boot unwind:
