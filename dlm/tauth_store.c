                          ...
                          uint32_t local_node, uint64_t local_inc)
{
    ...
    s->base = base;
    s->size = size;
    s->npages = 0;      /* sess427 (D-0348 step 2): read from the region header */
                                ...
                                uint64_t *seq_a, uint64_t *seq_b, int *copy_out)
{
    ...
    if (sa && sa == sb && memcmp(pg, scratch, sizeof(*pg)) != 0) {
        /* sess423 (step 4): two VALID images with the same seq and
...
/*
 * sess426 (D-0347): the conditional commit.
 *
                          ...
                          uint32_t torn_after_bytes)
{
    ...
    if (torn_after_bytes) {
         ...
         * sub-sector bio (the sess405 trap). */
