/*
 * MXFS — whole-cluster bootstrap record (sess439).  See bootstrap.h.
 *
 * SPDX-License-Identifier: GPL-2.0
 */
#include "bootstrap.h"
#include "scsipr.h"
#include "disklock.h"       /* sess440: the survivor scan reads the HB table */
                                           ...
                                           uint32_t fs_gen)
{
    ...
    if (!dev || !offset || !fs_uuid)
        return NULL;
    /* sess443 (§6.8, review SS-8): the geometry lives in the super; a
                         ...
                         const struct mxfs_bootstrap_identity *id)
{
    ...
    /* sess443 (§6.8): a fresh claim opens a new recovery EPISODE — every
...
int mxfs_bootstrap_complete(struct mxfs_bootstrap *b)
{
    ...
    /* sess441 (§6.5): the adopted slot K is completed by the owner's OWN
...
/* ── sess441: the adopted-slice escrow (§6.5 shape B) ── */
                                  ...
                                  const struct mxfs_bootstrap_escrow *e)
{
    ...
    if (!b || !e || e->slot >= 64)
        return -EINVAL;
    /* sess442 (§6.7 resume): a PREPARED escrow for the SAME K and victim may
                            ...
                            uint64_t epoch, uint64_t *seq)
{
    ...
    if (rc == 0) {
        if (cur->state != MXFS_BOOTSTRAP_IDLE &&
            ...
            if (seq)
                *seq = cur->seq;        /* sess440: change, never age */
            rc = 1;
        } else {
            rc = 0;
        }
...
/* ── sess440: refuse / resume / manifest (docs/whole-cluster-restart.md §6) ── */
                                       ...
                                       unsigned int n, uint64_t *hash_out)
{
    ...
    if (!b || !e || !hash_out || n > MXFS_BOOT_MF_MAX)
        return -EINVAL;
    bank = MXFS_BOOT_MF_BANK(term);            /* sess443: term parity */
...
/*
 * ── sess440: THE SURVIVOR SCAN (docs/whole-cluster-restart.md §6.1) ──
 *
                                 ...
                                 unsigned int *noident_out)
{
    ...
    *unread_out = unread;
    ...
    for (slot = 0; slot < MXFS_DISKLOCK_HB_SLOTS; slot++) {
        ...
        if (r->flags == MXFS_DISKLOCK_FLAG_RECOVERY_GUARD) {
            /*
             * sess494 (D-0493): a guard carrying a TERMINAL verdict is not a
        ...
        /* sess441 (§6.5): a predecessor term's adopted slice — the next
...
/*
 * ── sess443: COMPLETION TOMBSTONES, LINEAGE, THE TAKEOVER JOURNAL ──
