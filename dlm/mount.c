			       ...
			       int dead_slot, mxfs_epoch_t dead_epoch)
{
	...
	/*
	 * this TCP-transport path is entirely node-scoped — it purges DLM
	       ...
	       struct mxfs_mount **mnt_out)
{
	...
	if (mnt->dlm_transport == MXFS_DLM_TRANSPORT_CAW) {
		...
		if (mnt->scsipr) {
			...
			ret = mxfs_scsipr_register(mnt->scsipr, false); /* legacy path has no operator assertion; fail closed */
			if (ret) {
				mxfs_pal_log(MXFS_LOG_WARN,
				...
				/* a refused RESERVE means the LU carries a
	...
	if (!mnt->scsipr && mnt->dlm_transport == MXFS_DLM_TRANSPORT_CAW) {
		...
		if (mnt->scsipr) {
			...
			ret = mxfs_scsipr_register(mnt->scsipr, false); /* legacy path has no operator assertion; fail closed */
			if (!ret)
				ret = mxfs_scsipr_reserve(mnt->scsipr);   /* */
			if (ret) {
				mxfs_pal_log(MXFS_LOG_WARN,
...
err_dlm:
	...
	if (mnt->dlm_caw) {
		/* a FAILED mount is not a withdrawn one.  It has no frozen
...
void mxfs_unmount(struct mxfs_mount *mnt)
{
	...
	if (mnt->dlm_caw) {
		/* the release moved out of mxfs_dlm_caw_destroy and into
