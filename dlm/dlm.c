...
/* sess422: owner of a ledger-imported shared holder whose heartbeat slot
...
/* sess422: holder-side release ACK retry cadence / give-up. */
...
/*
 * sess422 (step 3d): who counts as a HOLDER in a compatibility decision.
                              ...
                              struct mxfs_bast_record *recs, int count)
{
    ...
    for (i = 0; i < count; i++) {
        /* sess422: a ledger-imported holder whose slot has no live tenant
        ...
        /* sess1(a9a03929) instrumented (run45 120s ino-131 wedge): make EVERY
...
/* sess35 (ccloop) ROOT FIX for the dir_reuse 2/tcp ~6s handoff (PROVEN BY INSTRUMENT:
    ...
    for (i = 0; i < nwaiters; i++) {
        ...
        for (other = chain; other; other = other->next) {
            if (!resource_equal(&other->resource, resource))
                continue;
            /* sess422: the PENDING_RELEASE holder being retired in THIS
             * transition is chosen past; a PENDING_DURABLE holder blocks
             * like a GRANTED one.
             *
             * sess425 (proven by instrument, tests/tauth/concurrent_release_test):
        ...
        if (ok) {
            /* sess422: the grant is DECIDED here and becomes GRANTED only
    ...
    uint8_t *blocked_mode_out)
{
    ...
    *blocked_mode_out = blocked->mode;
     ...
     * blocked waiter's requested mode (sess422: GRANTED only — a
...
/* sess422: the durable grant id + lineage + request id ride on the grant. */
                       ...
                       uint32_t dir_epoch, const struct dlm_grant_ids *ids)
{
    ...
    /* sess50(ccloop) instrumented: log the dir_epoch this master SENDS on an EX grant
...
/* ═══════════════════════════════════════════════════════════════════════
 * sess422 — TCP durable-authority ledger, master side
...
/* sess427 (D-0348 step 2): a resource's HOME PAGE under this ctx's routing
...
/* 3a: page-aligned mastership.  page = the resource's home page (sess427:
...
/* sess425 (D-0342): is this owner's ledger purge still incomplete?  Caller
 * holds table_rwlock. */
...
static bool dlm_owner_purged(struct mxfs_dlm_ctx *ctx, mxfs_node_id_t node, int slot)
{
    ...
    /*
     * sess425 (D-0344, proven by dlm_ledger_test 7b): the identity
     ...
     * an UNKNOWN blocker (the sess424 ruling) until recovery names it.
     */
    (void)slot;
                              ...
                              bool exclusive)
{
    ...
    for (lk = ctx->buckets[bucket]; lk; lk = lk->next) {
        ...
        /* sess424 (ruling item 4): a shared bit imported while its slot
 ...
 * recovery names it, the sess424 ruling). */
                                  ...
                                  uint64_t gen)
{
    ...
    for (i = 0; i < acc->n; i++) {
        ...
        for (s = 0; s < 64; s++) {
             ...
             * UNKNOWN owner by design, and the sess424 rule that resolves it
             ...
             * before it honours a slot.  sess424's formation case is
...
/* ── sess423 step 4: ordered page handoff ── */
...
static void dlm_page_now_mine(struct mxfs_dlm_ctx *ctx, uint32_t page, const char *how)
{
    ...
    if (!mxfs_tauth_pass_quiet)
        mxfs_pal_log(MXFS_LOG_WARN, "mxfs: P-TAUTH-PAGE-MINE page=%u via=%s owner_gen=%#llx",
...
static int dlm_page_acquire(struct mxfs_dlm_ctx *ctx, uint32_t page, uint64_t gen)
{
    ...
    if (a.state == MXFS_TAUTH_PG_UNOWNED) {
        ...
        /*
         * sess426 (D-0345, proven by instrument in tests/tauth/unowned_page_test):
    ...
    if (to == 0 || to == ctx->local_node || !dlm_node_in_view(ctx, to)) {
        /*
         * 0.75.6 (sess511, s510b arm 1): this branch parked every request
                                  ...
                                  const struct mxfs_dlm_page_handoff *msg)
{
    ...
    case MXFS_HANDOFF_FREEZE_REQ: {
        ...
        if (msg->config_id != ctx->my_view_hash ||
            ...
            ctx->handoff_defers++;
            /* sess425 (D-0345 instrumented): a DEFER that repeats forever is a
        ...
        if (a.state == MXFS_TAUTH_PG_UNOWNED) {
            ...
            /*
             * sess428 (D-0349, design-consult ruling option (c)): one durable
                             ...
                             mxfs_node_id_t hint_node, uint64_t hint_inc)
{
    ...
    if (a.auth_node != node || a.auth_inc != inc)
        return 0;
    /*
     * 0.89.3 (D-0981, design consult Astra 2026-09-19): the one guard every
                              ...
                              uint64_t *gen_out)
{
    ...
    *gen_out = gen;
    if (!dlm_ledger_active(ctx))
        return 0;
    /* sess423 (step 4): the page's durable authority must be THIS node
...
struct dlm_txn_item {
    ...
    bool            ack_deferred;   /* sess425: a release-only re-commit
...
static int dlm_txn_commit(struct mxfs_dlm_ctx *ctx, struct dlm_txn *txn)
{
    ...
    for (attempt = 0; attempt < 3; attempt++) {
        ...
        rc = mxfs_tauth_ledger_commit(ctx->ledger, ops, txn->n, txn->gen,
                                      (uint64_t)ctx->current_epoch);
        if (rc == -EPERM)
            rc = -ESTALE;   /* sess423: the page is no longer ours: remaster */
...
/*
 * sess425 (proven by instrument, tests/tauth/concurrent_release_test s5): a LOCAL
 ...
 * (blocked waiters behind the new holders) and the sess35 grantee BASTs
                            ...
                            int commit_rc)
{
    ...
    mxfs_pal_rwlock_wrlock(ctx->table_rwlock);
    /*
     * sess424 (design-consult ruling ccmemory ccloop-c7ee71c6-sess424-GPT-ruling-
 ...
 */
/* sess425: a refused bundled transition must never strand its release.
...
/* sess425: a release whose retirement could not be committed (neither
...
static int dlm_promote_txn(struct mxfs_dlm_ctx *ctx, struct dlm_txn *txn)
{
    ...
    for (round = 0; round < 6; round++) {
        ...
        if (txn->n == 0) {
            /* nothing to commit: still re-evaluate blocked waiters (sess36) */
            struct mxfs_bast_record post_basts[MXFS_MAX_BAST_RECORDS];
            int npost = 0;
            struct mxfs_lock *pb;
            uint8_t blocked_mode = MXFS_LOCK_NL;
        ...
        if (rc != 0 && rc != -ESTALE && nrel && ngrant) {
            /* sess425: the bundle was refused for a grant reason (-EBUSY
        ...
        if (rc != 0)
            break;
        /* sess425: a retirement re-scans even when this round granted
...
/* sess425: master side — re-drive every stuck retirement (see
...
/* sess425 (D-0342): re-run every incomplete owner purge older than the
...
/* sess426 (D-0347): may the store take over a commit ticket left by
                                ...
                                int slot)
{
    ...
    /*
     * sess425 (D-0342, ruling ccmemory ccloop-c7ee71c6-sess425-GPT-ruling-
...
struct mxfs_dlm_ctx *mxfs_dlm_create(mxfs_node_id_t local_node)
{
    ...
    /* sess422: holder-side un-ACKed release list */
...
void mxfs_dlm_destroy(struct mxfs_dlm_ctx *ctx)
{
    ...
    /* sess422: ledger bookkeeping (the ledger object itself belongs to the
 ...
 */
/*
 * sess425 (instrumentation, rig test3 'lock request failed after 10
                          ...
                          uint8_t *granted_mode, uint64_t *progress)
{
    ...
    /* sess422: per-request id (idempotent retries return the durable grant) */
    ...
    if (master != ctx->local_node) {
        ...
        req.mode = mode;
        req.flags = flags;
        req.owner_inc = ctx->local_inc;      /* sess422: ledger identity */
...
lockreq_sent:
        ...
        if (pend->status != 0) {
            ...
            if (ret == MXFS_ERR_UPGRADE_CONFLICT)
                return -EDEADLK;
            /* sess422: the master is no longer the page owner under the
            ...
            if (ret == MXFS_ERR_REMASTER) {
                ...
                /* sess425 (D-0345 instrumented): which master we asked, under
            ...
            /* sess425: a durable holder's retirement was in flight when
             * the master decided — it retires it and we retry */
            if (ret == MXFS_ERR_LEDGER_BUSY)
                return dlm_retry(ctx, 3);
            /* sess426 (D-0348): the home page is full — wait, never EIO */
    ...
    /* sess422: the ledger page must be current + imported before any
    ...
    for (lk = chain; lk; lk = lk->next) {
        if (resource_equal(&lk->resource, resource) &&
            ...
            /* sess422: our own grant/release is mid-transition — the
             * committing thread delivers; do not decide twice. */
            if (lk->state == MXFS_LSTATE_PENDING_DURABLE) {
                /*
                 * sess425 (instrumented: formation_test lap 'fails=1', rig test3
            ...
            if (lk->state == MXFS_LSTATE_GRANTED) {
                lk->unclaimed = false;      /* sess425: adopted (shortcut or upgrade) */
                /*
                ...
                if (lk->mode >= mode) {
                     ...
                     * WAITING/BLOCKED entries from other nodes. */
                    {
                        ...
                        for (other = chain; other; other = other->next) {
                            ...
                            /* sess7(a9a03929) FIX-17 (PROVEN BY INSTRUMENT, run90
                ...
                /* Upgrade: check compat with OTHER holders */
                {
                    ...
                    if (conv_compat) {
                        /*
                         * sess5 (ccloop a16ec5f2) EPOCH-HOLE FIX (instrumented —
...
check_compat:
    ...
    if (compat) {
        /* Grant immediately (sess422: decided here, durable + delivered by
        ...
        /* sess1(ccloop a9a03929) PROVEN BY INSTRUMENT (run44 wedge): this was the
        ...
        /* sess35 ROOT FIX (the grantee BAST for a jumped older waiter) now
    ...
    /* Incompatible — queue and wait */
    {
        ...
        newlk->owner_inc = ctx->local_inc;      /* sess422: ledger identity */
        newlk->owner_slot = ctx->local_slot;
        newlk->req_id = req_id;
        pend->req_id = req_id;
        /*
        ...
        if (ret == -ETIMEDOUT) {
            ...
            mxfs_pal_rwlock_wrlock(ctx->table_rwlock);
            {
                ...
                while (*pp) {
                    if (*pp == newlk) {
                        /* sess4(a16ec5f2) INVARIANT GUARD (instrumented, proven in
                                 ...
                                 const struct mxfs_dlm_lock_resp *resp)
{
...

    {
        ...
        if (n <= 16 || (n % 256) == 0)
            pr_warn("mxfs: P912-QACK-RX n=%d rx=%llu type=%u ino=%llu ag=%u master=%u holder_mode=%s req_id=%u — the master has this request queued\n",
                  ...
                  uint8_t *granted_mode, int max_retries)
{
    ...
    if (!ctx || !resource || mode >= MXFS_LOCK_MODE_COUNT)
        return -EINVAL;
    memcpy(why0, ctx->retry_why, sizeof(why0));   /* sess425: per-request tally */
    ...
    do {
        ...
        if (ret != MXFS_DLM_RETRY) {
            ...
            mxfs_pal_log(MXFS_LOG_DEBUG,
                         ...
                         "(mode=%s, retries_left=%d)",
                         mode_name(mode), retries - 1);
            /* sess423: a parked decision (page handoff in flight) must not
                         ...
                         uint32_t expected_gen, int open_op)
{
    ...
    uint64_t rel_auth = 0, rel_seq = 0, rel_lineage = 0;   /* sess422 */
    uint8_t rel_mode = 0;
    ...
    /* Find (prefer GRANTED/CONVERTING).  sess422: the entry is unlinked
    ...
    while (*pp) {
        ...
        /* sess422: our own release of this tenure is already in flight */
        if (resource_equal(&lk->resource, resource) &&
            lk->owner == ctx->local_node &&
            lk->state == MXFS_LSTATE_PENDING_RELEASE &&
     ...
     *
     * sess4(a16ec5f2) ROOT FIX (instrumented, proven runs 19+20 by P4L pointer
    ...
    if (master == ctx->local_node && found_holder) {
        /*
         * sess422 (step 3e): we master this resource — retire our record
        ...
        /* sess35 P37: local-release for the contended dir. */
        if (resource->ino == 131 && resource->type == MXFS_LTYPE_INODE)
            pr_warn_ratelimited("mxfs: P37-LREL ino=131 releaser=%u master=%u\n",
    ...
    *pp = found->next;
     ...
     * release stays pending until the master's RELEASE_ACK (sess422 step
...
/*
 * sess7(a9a03929) FIX-20b (PROVEN BY INSTRUMENT, run91/94): send an UNCONDITIONAL
                                        ...
                                        const struct mxfs_resource_id *resource)
{
    ...
    rel.grant_gen = 0;
    rel.owner_inc = ctx->local_inc;     /* sess422: the master validates the
                          ...
                          mxfs_node_id_t owner, uint8_t new_mode)
{
    ...
    if (new_mode <= old_mode) {
         ...
         * 8/tcp dir_reuse intra-block double-alloc).  sess422: the
                               ...
                               bool selective)
{
    ...
    for (i = 0; i < ctx->bucket_count; i++) {
        ...
        while (*pp) {
            ...
            /* sess425 (D-0342): an imported blocker of an owner whose
    ...
    /*
     * sess422: successors are granted through the ledger — collect every
                                       ...
                                       const struct mxfs_resource_id *resource)
{
     ...
     * (sess422: through the ledger, outside the table lock) */
    {
        struct dlm_txn *txn = mxfs_pal_alloc(sizeof(*txn));
 ...
 *
 * sess10 (ccloop 72513a13, PROVEN BY INSTRUMENT 32/tcp fio cascade): a node whose
...
void mxfs_dlm_release_all(struct mxfs_dlm_ctx *ctx)
{
    ...
    /* sess422 (step 3e): unmount may not complete with a release un-ACKed
    ...
    /* sess425: the master-side ledger counters (the ledger's own
                                        ...
                                        const struct mxfs_resource_id *resource)
{
    ...
    /* sess422 (step 3a): PAGE-ALIGNED mastership — every resource on one
     * ledger page has the same master, so a page has exactly one writer.
     * sess427 (D-0348 step 2): home_page = seeded_hash % npages with the
                                 ...
                                 const mxfs_node_id_t *nodes, int count)
{
    ...
    if (changed) {
        ...
        pr_warn("mxfs: MXFS-MEMBERSHIP local=%u active_count=%d view=%#llx\n",
                ctx->local_node, count, (unsigned long long)ctx->my_view_hash);

        mxfs_pal_rwlock_wrlock(ctx->table_rwlock);
        /*
         * sess422 (step 3): the page-ownership generation moves with the
        ...
        /*
         * sess423 (step 4): pages the new view moves away from us stop
        ...
        for (i = 0; i < (int)ctx->bucket_count; i++) {
            ...
            while (*pp) {
                ...
                /*
                 * sess422 (D-0287, step 3): what THIS node holds survives
...
/* sess422: the DURABLE grant id {authority_epoch, grant_seq64} and lineage
                        ...
                        uint32_t *epoch_out)
{
     ...
     * per-node P-DOUBLEGRANT cannot see.  Decisive for the flap hypothesis. */
    {
        ...
        if (cnt > 0) {
            uint32_t h = dlm_res_page(ctx, res);   /* sess422: page-aligned */
        ...
        if (dg_shadow[i].used && resource_equal(&dg_shadow[i].res, res)) {
            ...
            if (dg_shadow[i].active && dg_shadow[i].owner != owner) {
                pr_warn_ratelimited(
                    ...
                    owner, gen);
                /* sess48 (instrumented): dump the master's live lock chain for this
    ...
    if (mine >= 0) {
        ...
        /* sess53(ccloop) instrumented under-fire probe: the grantee logs
                                    ...
                                    const struct mxfs_dlm_lock_req *req)
{
    ...
    /* sess422: not the page master under the current membership -> the
    ...
    if (mxfs_dlm_resource_master(ctx, resource) != ctx->local_node) {
        ...
        ctx->ledger_remaster++;
        /* sess425 (D-0345 instrumented): name the disagreement — the sender's
    ...
    /* sess422: page current + blockers imported before any decision */
    {
        int prc = dlm_ledger_prepare(ctx, resource, &ledger_gen);
        ...
        if (prc) {
            if (prc == -EAGAIN) {
                ...
                /* sess425 (D-0345 instrumented): we ARE the master under our view
    ...
    for (lk = chain; lk; lk = lk->next) {
        if (resource_equal(&lk->resource, resource) &&
            ...
            /* sess422: the sender's grant / release is mid-transition —
            ...
            if (lk->state == MXFS_LSTATE_GRANTED) {
                ...
                if (lk->mode >= mode) {
                    ...
                    /* sess422: the record is already durable (a live entry
                ...
                /* Upgrade: check compat with others */
                {
                    ...
                    if (conv_compat) {
                        /* sess422: upgrade = ledger transition; the old
                 ...
                 *
                 * sess8/sess12 (2/tcp) DOUBLE-GRANT ROOT (PROVEN BY INSTRUMENT via
...
remote_check_compat:
    ...
    /* Allocate and insert (sess422: an immediate grant is PENDING_DURABLE
    ...
    newlk->owner_inc = req->owner_inc;      /* sess422: ledger identity */
    ...
    if (compat) {
        ...
        dlm_txn_add_grant(txn, newlk, 0, 0, false);
        mxfs_pal_rwlock_unlock(ctx->table_rwlock);
        /* sess35 ROOT FIX (the grantee BAST for a jumped older waiter)
         * fires from dlm_txn_finalize once the grant is delivered. */
        grc = dlm_grant_txn(ctx, txn);
        mxfs_pal_free(txn);
        return grc;
     ...
     * corrupt the work_next chain at 3+ nodes. */
    {
        ...
        for (lk = ctx->buckets[bucket]; lk; lk = lk->next) {
            if (!resource_equal(&lk->resource, resource))
                continue;
            if (lk->state != MXFS_LSTATE_GRANTED)
                continue;   /* sess422: pending holders get theirs at finalize */
            if (lk->owner == MXFS_DLM_NODE_UNKNOWN)
                continue;   /* sess422: nobody to BAST; the purge retires it */
            if (!lock_compat[lk->mode][mode]) {
                                  ...
                                  const struct mxfs_dlm_lock_resp *resp)
{
     ...
     *
     * sess422: the mirror also carries the DURABLE grant id + lineage the
    ...
    if (status == 0) {
        ...
        if (!have_mirror) {
            ...
            if (newlk) {
                ...
                newlk->auth_epoch = resp->authority_epoch;   /* sess422 */
    ...
    if (status == 0) {
        ...
        if (!have_mirror && !matched) {
             ...
             * there too).
             *
             * sess422: the reject carries the durable grant id the master
                                    ...
                                    const struct mxfs_dlm_lock_release *rel)
{
    ...
    /* sess422: not the page master any more -> the releaser re-routes
    ...
    for (lk = ctx->buckets[bucket]; lk; lk = lk->next) {
        ...
        if (lk->state == MXFS_LSTATE_PENDING_RELEASE) {
             ...
             * sess425: NOT ACKed OK — an ACK is valid only once the
         ...
         *
         * sess422: the durable grant id is the stronger key — a release
        ...
        /* sess422: the releaser's incarnation must be the record's */
    ...
    if (!found) {
         ...
         * it means the entry was already cleaned up.  sess422: with the
    ...
    /*
     * sess422 (step 3e): retire the record and grant the successors in
    ...
    /* sess35 P37 / sess5 P5R: release visibility at the master. */
    if (resource->ino == 131 && resource->type == MXFS_LTYPE_INODE)
        pr_warn_ratelimited("mxfs: P37-RREL ino=131 sender=%u rc=%d\n", sender, rrc);
    if (resource->type == MXFS_LTYPE_AG)
        pr_warn_ratelimited("mxfs: P5R-AGREL ag=%u sender=%u rc=%d\n",
