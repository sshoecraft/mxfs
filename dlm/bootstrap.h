/*
 * MXFS — WHOLE-CLUSTER BOOTSTRAP RECORD (docs/whole-cluster-restart.md
...
/* v2 = refused_slot/refused_reason fields + the manifest sectors
 * (MXFS_BOOTSTRAP_BYTES 8192, MXFS_PROTO_GEN 14).
 * v3 = the adopted-slice ESCROW (§6.5 shape B, MXFS_PROTO_GEN 15). */
/* v4 = the escrow carries the manifest POINTER (264 bytes), PROTO_GEN 16. */
/* v5 = 32 KiB region: manifest banks, completion tombstones,
...
/*
 * (§6.6, design-consult ruling Q5): a sealed slice ended TERMINAL (quarantined
...
/* rec_flags bit (§6.8.4): this entry is the FENCED OLD OWNER of the
...
struct mxfs_bootstrap_rec {
	...
	struct mxfs_bootstrap_escrow escrow;    /* 224..488: v3, mptr */
	uint64_t    episode_term;       /* 488: v5 — the term of the CLAIM
	...
	uint16_t    lineage_count;      /* 496: v5 — terms ended by a
									 *      takeover in this episode (0..8) */
	uint16_t    takeover_gen;       /* 498: v5 — +1 per takeover CAS */
...
/*
 * COMPLETION TOMBSTONES (§6.8.4; review SS-6/SS-7).  A slot's
...
/*
 * LINEAGE (§6.8.5).  One entry per term a takeover ended in this
...
/*
 * THE TAKEOVER JOURNAL (§6.8.2; review SS-3/SS-4).  One CAW-written
...
/*
 * (§6.8.3 step 14): RESEAL an abandoned term as T+1 under a NEW
...
int mxfs_bootstrap_seal(struct mxfs_bootstrap *b, uint64_t victim_bitmap,
			...
			uint64_t manifest_hash);
/*
 * CLAIMED -> IDLE under our exact image.  Legal ONLY before the
 ...
 *
 * (D-BOOTSTRAP-OWNER-LIVENESS-CROSS-NODE-CLOCK-0450): the owner's
...
/*
 * (§6.7): exact-owner SAME-BOOT RESUME.  A mount that claimed the
...
/*
 * (§6.5 shape B): the adopted-slice escrow.  prepare: RECOVERING,
...
/* the same, into bank(term) before the record names us (a takeover
...
/*
 * (§6.8.4): completion tombstones.  write: exact-image CAW of the
...
/* the same under an explicit term, before the record names us (a
...
/*
 * (§6.8.5): lineage.  write: FUA entry `idx` (crc sealed; magic,
...
/*
 * (§6.8.2): the takeover journal.  read: the raw sector (0; an
