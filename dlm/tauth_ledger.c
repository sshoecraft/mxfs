...
/* sess427 (D-0348 step 2): the seeded fnv1a-32 in mxfs_tauth.h over the
...
void mxfs_tauth_ledger_stats(struct mxfs_tauth_ledger *l, const char *why)
{
    ...
    /* sess426 (D-0349): where a commit's milliseconds go, by phase */
    {
...
/*
 * sess426 (D-0348, design-consult ruling ccloop-c7ee71c6-sess426-GPT-ruling-tauth-
                                    ...
                                    struct mxfs_tauth_page *img, uint64_t gen)
{
    ...
    if (rc == -ESTALE || rc == -EBUSY) {
        /* sess426 (D-0347): nothing was written — the platter moved past
...
/*
 * sess428 (D-0349, design-consult ruling option (c)): the BOOTSTRAP node prepares a
                    ...
                    uint64_t stamp_ms)
{
    ...
    if (!e) {
        if (grant) {
            /* sess426 (D-0348): every entry of the home page is occupied
                               ...
                               struct mxfs_tauth_page *img, uint64_t config_epoch)
{
    ...
    if (rc == -ESTALE || rc == -EBUSY) {
        /* sess426 (D-0347): refused before any byte landed.  -ESTALE on a
