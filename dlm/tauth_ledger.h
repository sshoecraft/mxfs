 ...
 * (docs/tcp-authority-ledger.md step 3c,; design-consult rulings in ccmemory
...
struct mxfs_tauth_ledger {
	...
	/* (step 4): the configuration id this node writes into every
...
/* (step 4): a page's durable authority as the platter (fresh) or
...
/* (D-0348 step 2): a resource's HOME PAGE (routing) and preferred
...
/* (D-0349): bootstrap node only — UNOWNED -> PREPARED(target) in one
