...
struct mxfs_lock {
	...
	bool                 handoff;    /* this grant is a cross-node EX
	...
	uint32_t             dir_epoch;  /* (design review): per-resource MONOTONIC
	...
	/*
	 * (tcp-authority-ledger step 3): the holder's ledger identity
	...
	uint64_t             lineage;
	uint64_t             decide_gen;
	/* a PENDING_RELEASE whose retirement commit failed (neither
	...
	/* a LOCAL grant just made GRANTED by finalize and not yet
	...
	void                 *pend_waiter; /* the mxfs_dlm_pending a
...
/*
 * (step 3e): a LOCK_RELEASE sent to a remote master that has not
...
struct mxfs_dlm_ctx {
	...
	/* wall-clock (ms) of the most recent membership CHANGE (set in
	...
	/*
	 * (tcp-authority-ledger step 3): the durable authority ledger.
	...
	/* (D-0348 step 2): the ROUTING GEOMETRY — the region's page
	 ...
	 * page load instead of imported (dead ids never return). */
	...
	/* (D-0342): owners whose ledger purge did NOT complete (a page
...
#define MXFS_DLM_CONSUMED_TOMBS 256
	...
	uint64_t                ledger_grants, ledger_denies, ledger_imports,
				...
				/* 0.75.5: a predecessor's shared bit on our own
							...
							open_mark_only,
							/* refused bundles whose release was
...
/* retry-budget variant — see dlm.c.  Lets the xfs inode-acquire slow
...
uint8_t mxfs_dlm_held_mode(struct mxfs_dlm_ctx *ctx,
			   const struct mxfs_resource_id *resource);
/* diagnostic: table entries on a ledger page still between
...
/*  atomic-context-safe form of the above.  The
...
int mxfs_dlm_settled_grant_nb(struct mxfs_dlm_ctx *ctx,
			      const struct mxfs_resource_id *resource);
/* per-grant generation token THIS node holds for `resource` (highest-mode
...
uint32_t mxfs_dlm_grant_gen(struct mxfs_dlm_ctx *ctx,
			    const struct mxfs_resource_id *resource);
/* durable grant id + lineage of our held grant (false = none). */
bool mxfs_dlm_grant_id(struct mxfs_dlm_ctx *ctx,
		       const struct mxfs_resource_id *resource,
		       uint64_t *auth_epoch, uint64_t *grant_seq,
		       uint64_t *lineage);

/* did THIS node's currently-held EX grant for `resource` arrive as a
...
/* (design review): return the MONOTONIC cross-node handoff epoch the master
...
/* the requester's ledger identity + request id ride with the
...
/* ─── durable authority ledger (tcp-authority-ledger step 3) ─── */
...
/* routing geometry without (or before) an attached ledger. */
...
/* ─── ordered page handoff (tcp-authority-ledger step 4) ─── */
