...
/*
 * NUDGE v2 companion — HOPELESS-DEFER interval.  A waiter whose last
...
/*
 * v0.5.3 ( scaling_curve): idle BAST poll interval when
...
#define MXFS_CAW_BAST_POLL_RELAX_MS 4000    /* idle poll when UDP BAST path is up.
		1000 -> 4000.  Waiters re-send UDP hints every
...
#define MXFS_CAW_BAST_RESEND_FAST_MS 25     /* first
...
/*  hard ceiling for the liveness-extended wait.  The
...
struct mxfs_caw_lock_slot {
	...
	/*
	 * D-AGLOCK-...-LIVELOCK-488 (design-consult ruling, Option B'): sticky
	...
	/*
	 * (run14d): bitmap of nodes waiting for an EXCLUSIVE-class mode
	 ...
	 * analog of the TCP master's per-resource dir_epoch (design).
	...
	uint8_t                 pad3[3];
	/*
	 * (3e02e7dd) canonical dir logical-block0 record, WRITE-ONCE
	...
	/*
	 * (D-CROSSNODE-OPEN-UNLINK / AGI orphan-coordinator): bitmap of
	...
	/*
	 * (D-FOREIGN-REPLAY full fix, step 1 — review-ruled token design):
	...
	/*
	 * (D-FOREIGN-REPLAY lineage discriminator, design-consult
 ...
 *
 * the kernel arm used to be a MACRO that NOTHING EVER INVOKED
...
/*  grant/release nudge — same wire struct as the
...
/* NUDGE v2: receive-side record of the last nudges, scanned by
...
/*
 * ─── BAST DISPATCH QUEUE (instrumented rooted, design-consult reviewed) ───
 *
 ...
 *
 * saw the shape and tuned around it: "every hint fires bast_cb on all
...
/* local request registry entry — definition is private to
 * dlm_caw.c; nothing outside the CAW transport may reach into it. */
struct mxfs_caw_lreq;

/*
 * ─── (design-consult ruling, defects D2 + D4): THE LIFECYCLE STATE ───
 *
 ...
 * allocated, so there is no lock-less ctx to degrade for.
 *
...
struct mxfs_dlm_caw_ctx {
	 ...
	 *
	 * v5 dynamic-allocated array sized by max_held (set at
	...
	/*
	 * step-6 F1 (ruling item 5): WEDGED-release pins.  A
...
#define MXFS_CAW_MAX_PINNED 4
	...
	/*
	 * (docs/recovery-manifest.md, writer guard S4): victim slots
	 * whose EX/PW authority is WRITE-PROTECTED — pushed by the disklock
	 * monitor (mxfs_disklock_set_protect_cb -> mxfs_dlm_caw_set_protected_mask)
	 * whenever a slot's recovery descriptor is observed at stage >=
	 * SNAPSHOTTING.  caw_slot_ex() refuses (-EPERM, P-RMAN-GUARD-REFUSED)
	 * any CAS that would clear a protected node's EX/PW bit, change the
	 * ex_grant_epoch / resource_lineage / binding of a slot while a
	 * protected bit is set, or grant EX/PW to another node there — unless
	 * the caller is the recovery owner's purge (MXFS_CAW_CAS_F_PURGE).  The
	 * audit says no such path exists; the guard makes that an invariant.
	 * Volatile: written by the monitor thread, read on every CAS.
	 */
	...
	/* peers=: nudges go by unicast to each listed address instead of the
	 * group, and only listed senders are heard.  count 0 = multicast. */
	struct mxfs_static_peers bast_peers;

	/*
	 * BAST dispatch queue — see the block comment above
	...
	/*  GRANT NUDGE — the reverse of the BAST hint.
	...
	uint64_t                nudge_seq;
	/* NUDGE v2: ring of the last MXFS_CAW_NUDGE_RING received
	...
	/*  liveness oracle for the wait-timeout
	...
	/* (ruling item B) — OUT-OF-CLOSURE SCRUB ORACLE.
	...
	/* (ruling part 1, second half) — WAIT CANCELLATION.
	 ...
	 * out the full DLM timeout (measured quarantine at t=171s, the
	 * same waiter reached the refusal gate at t=467s — the five-
	...
	/*
	 * (D-FOREIGN-REPLAY step 4a) — MOUNT ADOPT WINDOW.
	 *
	 ...
	 * returns -EOVERFLOW, leaving the on-disk manifest intact (design review
	 * ruling item 6: on any failure, never "clean up" retained
	 ...
	 *
	 * v5 dynamic-allocated, sized by ctx->max_held.
	 */
	 ...
	 * Measured (2-node rsync): ~2.9K of 6.1K slot reads
	...
	struct mxfs_caw_grant_meta {
		...
		bool                    handoff;
		bool                    valid;
		/* (3e02e7dd) canonical dir block0, cached from the
	 ...
	 * sharing grant_meta's mutex would be scheduling-while-atomic.  Instrumented
	...
	/*
	 * — LOCAL REQUEST REGISTRY (one entry per live (node, resource)).
	 *
	...
	/*
	 * Paired with lreq_lock.  this is the OWED-CLEANUP WAKE CHANNEL.
	 ...
	 * on it.  It carries no other traffic: the "last leaver waits out an
	 ...
	 * by the collector.  It REPLACES the rotating bucket cursor, which
	...
	uint32_t                  owed_q_n;
	/*
	 * ─── (design-consult ruling, blocker 2): THE ADMISSION GATE ───
	 *
	 ...
	 * ), but the threads that run them are XFS's, not ours: there is
	 ...
	 * must run EXCLUSIVELY.  design review found the race that makes that
	...
	/*
	 * teardown sequencing, all guarded by lreq_lock.
	 *
	 ...
	 * computing anything of its own.  (ruling A1.  The `stop_ran`
	...
	bool                      departed_clean;
	/*
	 * ─── (design-consult ruling A2/A3 + B): TEARDOWN STATE ───
	 *
	...
	int                       held_at_stop;
	/*
	 * ─── (design-consult ruling, blocker 3): TERMINAL ESCALATION ───
	 *
	...
	/*
	 * ─── (design-consult ruling item 7): MEMBERSHIP CONTEXT ───
	 *
	...
	/*
	 * (D-RELEASEALL-LREQ-RETIRE-MISSING, design-consult ruling).
	 *
	...
	/*
	 * (design-consult ruling on blocker 2 — guaranteed-progress owed
	...
	/*
	 * (design-consult ruling, blocker 6 — the condvar lost-wakeup window).
	 *
	...
	/*
	 * (design-consult ruling item 5 — "an ENOMEM fallback that proceeds
...
/* 4096 direct-mapped buckets gave birthday-certain
...
/* chain heads for the local request registry.  Sized for the
...
/* pre-allocated registry entries reserved for destructive clear
...
/*
 * ─── the owed-cleanup worker (design-consult ruling, blocker 2) ───
 *
...
/* Teardown drain budget.  Unmount is not a budget rule workload path, but this
...
/*
 * ─── (design-consult ruling, blocker 1): TIME BUDGETS, NOT RETRY COUNTS ───
 *
 ...
 * safety deadline, not a budget rule performance budget.
 */
...
/*
 * ─── (design-consult ruling B): THE MANDATORY FAIL-STOP DEADLINE ───
 *
...
/*
 * (design-consult ruling A1): stop is IDEMPOTENT AND SERIALISING, not
...
/*
 * teardown contract for the layer above (mxfs_v5_dlm_shutdown).
 *
 ...
 * instead, loudly.
 *
 * Both answers are the OWNER'S STORED VERDICT, read under lreq_lock (
...
/* peers=: set before mxfs_dlm_caw_start; NULL or empty keeps multicast */
...
/*
 * ─── (design-consult ruling, step 5): THE ESCALATION CHANNEL ───
 *
...
/* (474 leg A): mxfs_dlm_caw_lock with an ABSOLUTE mxfs_pal_time_ms
...
/* (D-488 leg 7): acquire carrying the caller's ATTESTED in-core
...
/* (design review audit C1): open_op piggybacks this node's open-holder bit on
...
int mxfs_dlm_caw_unlock(struct mxfs_dlm_caw_ctx *ctx,
			  const struct mxfs_resource_id *resource);
/* D-488 (ruling): unlock reporting the tri-state outcome
...
int mxfs_dlm_caw_open_set(struct mxfs_dlm_caw_ctx *ctx,
			  const struct mxfs_resource_id *resource);
/* B6 read for a caller holding no per-inode claim (routed freer
...
/*
 * — TEST-ONLY observation helpers for the same-node reconcile
 * exerciser (D-SAMENODE-WAITER-CANCEL-COLLISION closure vehicle,
 * mxfs_v5_dlm_caw_samenode_selftest).  Read-only: a slot image by find_slot
 * (no CAS) and the local request registry's state for one resource under
 * lreq_lock.  Never call from a lock/unlock path.
 */
struct mxfs_caw_test_bits {
	bool		found;		/* live slot located */
	uint32_t	slot_idx;
	uint32_t	generation;
	uint64_t	holders_ex;
	uint64_t	holders_pw;
...
/* attempts/pending for the resource's registry entry; -ENOENT = no entry */
int mxfs_dlm_caw_test_lreq_state(struct mxfs_dlm_caw_ctx *ctx,
				 const struct mxfs_resource_id *resource,
				 uint32_t *attempts, uint32_t *tenure_ex,
				 bool *owed_pending);
/* arm / read back the consumable injection knobs the exerciser drives */
enum mxfs_caw_test_knob {
	MXFS_CAW_TK_WAIT_EXPIRE,	/* K6: next registered wait gives up */
...
/* read the shared on-disk slot generation (cross-node AG epoch). */
int mxfs_dlm_caw_read_generation(struct mxfs_dlm_caw_ctx *ctx,
				 const struct mxfs_resource_id *resource,
				 uint64_t *out_gen);

/* mxfs_dlm_caw_read_ex_grant_epoch was DELETED in — an
 ...
 *
 * *out_lineage = the slot's resource_lineage, taken from the SAME
 * slot image as the other outputs (ruling Q-C: no second read — a
...
/* read-only concurrent-EX detector — popcount(OR of holders_ex over
...
/* 0.75.28: are all on-disk holders of `resource` (any class, our bit excluded)
 * heartbeating per holders_alive_fn?  1 = yes; 0 = no holder, a dead holder,
 * or no oracle.  Read-only. */
...
/*  a864 unconditional scan-based self-release — CAS-clear our bit
...
/* (3e02e7dd) canonical dir block0 query/publish — see the
...
/* record a WEDGED-release pin — release_all will refuse to clear
...
/*
 * (D-FOREIGN-REPLAY step 4a) — flagged form of the above.
 *
...
/*
 * — read-only census of what node_mask still owns in the table.
 ...
 */
/*
 * (docs/recovery-manifest.md) — the PROVER-side fence-time manifest
 * collection.  Read-only census of every slot whose EX or PW bitmap carries
 * victim_slot's bit, emitted in slot-index order as mxfs_rman_entry records
 * into `ents` (capacity `max`, MXFS_RMAN_MAX_ENTRIES for the full table).
 * Like footprint_scan, an incomplete read is an ERROR (-EIO), never a short
 * manifest: an unread slot may hold authority and a manifest that omits it
 * would refuse valid images (or, worse, the writer guard would not protect
 * it).  Returns 0 with *out_count, or a negative errno.  *out_scanned is the
 * number of slots read (== MXFS_CAW_MAX_SLOTS on success).
 */
void mxfs_dlm_caw_set_protected_mask(struct mxfs_dlm_caw_ctx *ctx,
				     uint64_t mask);
/* TEST ONLY (knob mxfs.rman_test_mutate): find the first CAW slot
 * holding EX/PW authority of a node in `mask` and CAS an image that clears
 * that bit.  mode 1 = through the guarded path (the guard MUST refuse:
 * P-RMAN-GUARD-REFUSED); mode 2 = with the purge exemption (bypasses the
 * guard; the replayer/pre-purge revalidation MUST then detect the mutation).
 * Returns the caw_slot rc, -ENOENT if no such slot, -EINVAL on bad args. */
int mxfs_dlm_caw_test_mutate_protected(struct mxfs_dlm_caw_ctx *ctx,
				       uint64_t mask, int mode);
/* the current-safety reader — same as mxfs_dlm_caw_victim_manifest_read
 * plus the victim's exact mode bits (MXFS_RMAN_MODE_*) and the slot index, so
 * a manifest entry can be compared field for field against the live slot. */
...
/* the same collection on a bare device (the whole-cluster bootstrap
 * owner fences before it has an engine); lock_region_offset =
 * disklock_offset + MXFS_DISKLOCK_HB_SIZE. */
...
/* open/close the mount adopt window (see ctx->mount_adopt_window).
...
/*  wire the liveness oracle for the wait-timeout
