...
/* sess454 (0.61.0, D1): the departure mutex and the PROUT wrappers — see
...
/* sess454 (D8): contexts whose probe thread outlived its bounded join. */
...
/* sess452: the probe thread must be joined before any of this is freed.
 * sess454 (D8): a join that times out quarantines the context instead —
                                            ...
                                            uint64_t local_key)
{
    ...
    ctx->dev = dev;
    ctx->local_key = local_key;     /* sess438: ledger-selected per-boot key */
    ctx->reserved = false;
    /* sess451/452: tri-state key-state snapshot + bracket scratch (see
...
void mxfs_scsipr_abandon(struct mxfs_scsipr_ctx *ctx)
{
    ...
    /* sess452: 0.59.1 freed only the ctx here and leaked the snapshot lock
                                  ...
                                  bool replace_predecessor)
{
    ...
    /*
     * sess439 (measured, lone_mount_create remount_refused on 0.45.0):
     ...
     */
    {
        ...
        mxfs_scsipr_snap_invalidate(ctx, "register-probe");   /* sess452 */
        if (sr == 0) {
            mxfs_pal_log(MXFS_LOG_WARN,
    ...
    mxfs_scsipr_snap_invalidate(ctx, "register");            /* sess452 */

    /*
     * sess433 (D-379(B) / D-0355, sess432 design-consult ruling): -EEXIST means
    ...
    if (ret == -EEXIST) {
        ...
        mxfs_scsipr_snap_invalidate(ctx, "register-replace"); /* sess452 */
    }
...
/*
 * sess438 (ruling item 4): VERIFY the registration against the table.
 ...
 * this nexus (a UA-consumed PROUT, sess381's silent case): fail the
                                          ...
                                          uint64_t old_key)
{
    ...
    mxfs_scsipr_snap_invalidate(ctx, "register-succeed");    /* sess452 */
    if (ret) {
        mxfs_pal_log(MXFS_LOG_ERR,
...
/*
 * ── sess452 (0.59.2): the bracketed key-state proof ──────────────────────
 *
...
static void scsipr_probe_fn(void *arg)
{
    ...
    mxfs_pal_log(MXFS_LOG_INFO, "scsipr: P-PR-PROBE-THREAD '%s' started",
                 ctx->dev_name);
    ...
    /* sess454 (D8): the last store this thread makes to the context; a
...
int mxfs_scsipr_probe_start(struct mxfs_scsipr_ctx *ctx)
{
    ...
    if (ctx->probe_thread)
        return 0;
    mxfs_scsipr_quarantine_reap();          /* sess454 (D8): free what exited */
...
/*
 * sess454 (0.61.0, D8): bounded stop.  A probe thread that does not return
...
/*
 * ── sess454 (0.61.0, D1 + D6): settle-absent with the single-use token ──
 */
...
static int scsipr_reserve_locked(struct mxfs_scsipr_ctx *ctx)
{
    ...
    ret = mxfs_pal_scsi_pr_reserve(ctx->dev, ctx->local_key,
                                   MXFS_SCSIPR_RESV_TYPE);
    mxfs_scsipr_snap_invalidate(ctx, "reserve");             /* sess452 */
                                         ...
                                         uint32_t *out_gen, int *out_count)
{
    ...
    if (repair) {
        ...
        mxfs_scsipr_snap_invalidate(ctx, "reserve-repair");  /* sess452 */
        if (rr == 0 || rr == -EBUSY) {
                                 ...
                                 uint64_t victim_key, bool abort)
{
    ...
    /*
     * sess381 design-consult ruling: NEVER issue a PREEMPT whose SARK is our own key.
    ...
    ret = mxfs_pal_scsi_pr_preempt(ctx->dev, ctx->local_key, victim_key,
                                   abort, type);
    mxfs_scsipr_snap_invalidate(ctx, "preempt");             /* sess452 */
                           ...
                           struct mxfs_fence_result *out)
{
    ...
    if (!ctx || !ctx->dev)
        return -EINVAL;
    if (!victim_key) {
        /* sess438: no frozen key for the exact dead incarnation.  The old
 ...
 * that the recovery stops rather than publishing over it.  GPT (design-consult ruling
...
int mxfs_scsipr_validate_admission(struct mxfs_scsipr_ctx *ctx)
{
    ...
    /* sess453 (0.60.0, review-#3 condition 6): a nonzero key is not enough —
                                   ...
                                   struct mxfs_fence_result *out)
{
    ...
    /* ── THE COMMAND-SUBMISSION BOUNDARY (sess381) ── */
    if (arm_submit) {
        ret = arm_submit(arm_data);
        if (ret) {
            mxfs_pal_log(MXFS_LOG_ERR,
...
static int scsipr_unregister_locked(struct mxfs_scsipr_ctx *ctx)
{
    ...
    /*
     * sess440 (chain 23 s440b, 0.45.1): the refused same-boot dirty remount
    ...
    mxfs_scsipr_snap_invalidate(ctx, "unregister");          /* sess452 */
