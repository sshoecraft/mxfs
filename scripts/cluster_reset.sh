...
run "$T1" "sudo insmod /mnt/mxfs-src/mxfs.ko dyndbg=+p && echo T1_INSMOD_OK; sudo /mnt/mxfs-src/tools/mkfs_mxfs -f $MXFS_DEV > /tmp/mkfs.log 2>&1 && echo T1_MKFS_OK; sudo blockdev --flushbufs $MXFS_DEV; sync; sleep 1; sudo mount -t mxfs $MXFS_DEV /mnt/shared 2>&1 && echo T1_MOUNT_OK || (echo MOUNT_FAIL; cat /tmp/mkfs.log)"

echo "=== T2 insmod + mount ==="
run "$T2" "sudo insmod /mnt/mxfs-src/mxfs.ko dyndbg=+p && echo T2_INSMOD_OK; sleep 1; sudo mount -t mxfs $MXFS_DEV /mnt/shared && echo T2_MOUNT_OK"
