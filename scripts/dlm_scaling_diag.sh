#!/bin/bash
# dlm_scaling_diag.sh — measure WHERE dlm_scaling@N spends its time.
#
...
for n in "${NODES[@]}"; do
    ( sq "$n" 90 "nohup iostat -x -d 5 24 /dev/mapper/mpatha >/tmp/dsdiag_iostat.txt 2>&1 & echo started" ) >/dev/null &  # device-adjudicated: the iostat of the CAW rig's multipath map is this diagnostic's subject, not a filesystem measurement
done
wait
