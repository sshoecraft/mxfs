#!/bin/bash
# sess435 chain 16: lap 2 of the LUN vergate arms with the B4 DURABILITY leg re-asserted (file=1 after the post-restore replay). Gated on chain 15.
# sess435 chain 16: the vergate "loop arms" on the SHARED LUN (VG_DEV port,
...
cd /src/mxfs || exit 1
LABEL=${1:-s435i}
LOG=tests/evidence/sess435_chain16_vergate_lun2_$LABEL.log
GATE=tests/evidence/sess435_chain15_0415_agwait_s435h.log
{
  echo "=== sess435 chain14 start $(date -u +%FT%TZ) ==="
  for t in $(seq 1 600); do grep -q '^DONE' "$GATE" 2>/dev/null && break; sleep 10; done
  grep -q '^DONE' "$GATE" || { echo "ABORT: chain15 not DONE after 6000 s"; echo "DONE $(date -u +%FT%TZ)"; exit 1; }
