#!/bin/bash
# sess425_chain.sh — build + rig-verify 0.35.1 (D-0341: concurrent-release
# DOUBLE-GRANT page wedge fix) on the CURRENT fleet.  Same shape and the budget rule
# budget as sess424_chain.sh; the sweep pattern adds the sess425 P-lines
# (RELEASE-STUCK, DLM-STATS) so a re-drive or a stuck retirement is
# captured raw, and the tag summary shows FREEZE-DRAIN-TIMEOUT /
...
sweep() {
  ...
  echo "STAGE sweep $st mounts: refused=$(cat "$D"/${st}_test*.txt | grep -ac "can.t read superblock") remaster=$(cat "$D"/${st}_test*.txt | grep -ac 'status=12') retry_exhausted=$(cat "$D"/${st}_test*.txt | grep -ac 'lock request failed after')"
}
{
  echo "=== sess425 chain $LABEL start $(date -u +%FT%TZ) build=$(cat VERSION) ==="
  B="$E/sess425_${LABEL}_build.log"
  ...
  timeout 180 make -C tests/tauth clean test > "$E/sess425_${LABEL}_tauth.log" 2>&1; urc=$?
  echo "STAGE tauth usermode rc=$urc $(grep -E 'RESULT|fails=' "$E/sess425_${LABEL}_tauth.log" | tr '\n' ' ')"
