#!/bin/bash
# sess435 chain 17: build 0.41.6 (P138-ACQ acquire-level accounting)
...
cd /src/mxfs || exit 1
LABEL=${1:-s435j}
LOG=tests/evidence/sess435_chain15_0416_acq_$LABEL.log
EV=tests/evidence/sess435_chain15_0416_acq_$LABEL
GATE=tests/evidence/sess435_chain16_vergate_lun2_s435i.log
mkdir -p "$EV"
{
  echo "=== sess435 chain15 start $(date -u +%FT%TZ) ==="
  for t in $(seq 1 600); do grep -q '^DONE' "$GATE" 2>/dev/null && break; sleep 10; done
  grep -q '^DONE' "$GATE" || { echo "ABORT: chain16 not DONE after 6000 s"; echo "DONE $(date -u +%FT%TZ)"; exit 1; }
  echo "gate passed at $(date -u +%FT%TZ) (iter $t)"
  OLD=$(modinfo mxfs.ko | awk '/srcversion/{print $2}')
  timeout 500 make modules > tests/evidence/sess435_build17_$LABEL.txt 2>&1; brc=$?
  timeout 120 make tools >> tests/evidence/sess435_build17_$LABEL.txt 2>&1; trc=$?
