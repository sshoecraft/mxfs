#!/bin/bash
# sess428_chain.sh — build + rig-verify 0.38.3 (D-0351 FREE-PUBLISH fix, second
# lap) on the CURRENT fleet, 32/caw only:
...
cd /src/mxfs || exit 2
LABEL=${1:-s432}
WAITLOG=${2:-tests/evidence/sess427_s431.log}
E=tests/evidence
D="$E/sess428_${LABEL}_dmesg"
mkdir -p "$E" "$D"
PAT='P55C-|P-FREEOB|P-CR62|P-CR3-CANCEL|P-CR63-DEFER-DISKLIVE|P-SESSION-POISON|Internal error|P119-NONEX-FLUSH-SKIP|P128-INACT-DEFER|P32D-DEADINCARN|P-RECYCLE-DEADSTAMP|P-RECYCLE-GATE|P87-|P88-|P34H-POISON|force-shutdown|xfs_trans_cancel|status=12|lock request failed after'
TAGS='P55C-[A-Z-]+|P-FREEOB-[A-Z-]+|P-CR62|P-CR3-CANCEL|P-CR63-DEFER-DISKLIVE|P-SESSION-POISON|P119-NONEX-FLUSH-SKIP|P128-INACT-DEFER|P32D-DEADINCARN-SKIP|P-RECYCLE-DEADSTAMP-CLEAR|P34H-POISON-[A-Z]+|P8[78]-[A-Z-]+'
prep_caw() { timeout 300 ./run.sh 32 caw prep_cluster > "$E/sess428_${LABEL}_prep_$1.log" 2>&1; echo "STAGE prep caw $1 rc=$?"; }
sweep() {
  local st=$1 i
  ...
  echo "STAGE sweep $st verdicts: shutdowns=$(cat "$D"/${st}_test*.txt | grep -ac 'P-SESSION-POISON\|P-CR3-CANCEL\|Internal error') freeob_bad=$(cat "$D"/${st}_test*.txt | grep -ac 'P-FREEOB-REFUSED\|P-FREEOB-FOREIGN\|P-FREEOB-NOSHELL\|P-FREEOB-NOEPOCH\|P55C-FREE-FOREIGN\|P-FREEOB-GATE-STUCK') deadskip=$(cat "$D"/${st}_test*.txt | grep -ac 'P32D-DEADINCARN-SKIP') foreign_mode0=$(cat "$D"/${st}_test*.txt | grep -a 'P55C-FREE-FOREIGN' | grep -ac 'disk_mode=00') retry_exhausted=$(cat "$D"/${st}_test*.txt | grep -ac 'lock request failed after')"
}
{
  echo "=== sess428 chain $LABEL start $(date -u +%FT%TZ) build=$(cat VERSION) wait_on=$WAITLOG ==="
  w=0
  ...
  timeout 240 make -C tests/tauth clean test > "$E/sess428_${LABEL}_tauth.log" 2>&1; urc=$?
  echo "STAGE tauth usermode rc=$urc $(grep -aoE '=== [a-z_]+ RESULT [A-Z]+ fails=[0-9]+|=== tauth_test: fails=[0-9]+' "$E/sess428_${LABEL}_tauth.log" | tr '\n' ' ')"
