...
ssh_n '/src/mxfs/tools/prep_tcm_node_scst.sh >/tmp/p.log 2>&1; modprobe libcrc32c; lsmod|grep -q "^mxfs " || insmod /src/mxfs/mxfs.ko dyndbg=+p; sg_persist --out --register-ignore --param-sark=0x5eed '"$MXFS_DEV"' >/dev/null 2>&1; sg_persist --out --clear --param-rk=0x5eed '"$MXFS_DEV"' >/dev/null 2>&1; echo y|/src/mxfs/tools/mkfs_mxfs '"$MXFS_DEV"' >/tmp/m.log 2>&1 && echo MKFS_OK; grep -i "log:" /tmp/m.log; mount -t mxfs '"$MXFS_DEV"' '"$MNT"' && echo MOUNT_OK' 70
