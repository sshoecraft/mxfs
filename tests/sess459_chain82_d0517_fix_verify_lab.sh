...
LABEL=${1:-s459b}
LAPS=${2:-4}
GATE=${GATE:-tests/evidence/sess459_chain81_d0517_buflsn_s459a.log}
LOG=tests/evidence/sess459_chain82_d0517_fixverify_$LABEL.log
EV=tests/evidence/sess459_d0517fix_$LABEL
...
{
  echo "=== sess459 chain82 START $(date -u +%FT%TZ) tree VERSION=$(cat VERSION) ==="
  T0=$(date +%s)
  timeout 600 make modules KCFLAGS=-DMXFS_ICLUS_RELMARK_READY=1 > tests/evidence/sess459_chain82_build_lab_$LABEL.log 2>&1; brc=$?
  ...
  echo "STAGE build_lab rc=$brc wall=$(( $(date +%s) - T0 ))s modinfo_lab=$LAB sv=$(modinfo mxfs.ko | awk '/srcversion/{print $2}') errors=$(grep -c 'error:' tests/evidence/sess459_chain82_build_lab_$LABEL.log) probe_string=$PROBE fix_string=$FIX"
  ...
  T0=$(date +%s)
  timeout 600 make modules > tests/evidence/sess459_chain82_build_prod_$LABEL.log 2>&1; brc=$?
  echo "STAGE build_prod rc=$brc wall=$(( $(date +%s) - T0 ))s modinfo_lab=$(modinfo mxfs.ko | grep -c 'mxfs_iclus_relmark_lab') sv=$(modinfo mxfs.ko | awk '/srcversion/{print $2}') errors=$(grep -c 'error:' tests/evidence/sess459_chain82_build_prod_$LABEL.log)"
