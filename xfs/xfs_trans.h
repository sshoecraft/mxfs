...
typedef struct xfs_trans {
	...
	struct list_head	t_mxfs_ag_unlocks; /* MXFS deferred AG DLM unlocks
						    * queued by alloc/ialloc and
						    * fired on trans commit/cancel
						    * to keep DLM held until the
						    * AG-meta modifications hit
						    * the log (priority-2 fix). */
	struct list_head	t_mxfs_inode_unlocks; /* MXFS deferred inode-DLM
						       * BAST triggers, fired on
						       * xfs_trans_free.  Holds
						       * BAST processing until the
						       * trans is fully freed and
						       * inode log items are in
						       * AIL — ensures peer reads
						       * post-modification disk
						       * state (priority-3 dir-stale
						       * + Free-inode-has-blocks). */
	unsigned long		t_pflags;	/* saved process flags state */
	/*
	 * step 5.3 (P1 of the design-consult ruling): globally unique
...
/* (MXFS): non-static so xfs_icache.c can ask "does tp already hold
