	...
	uint			last)
{
	 ...
	 * release checks (run64 never-landed leaf family). */
	{
	...
	struct xfs_buf		*bp)
{
	...
	/*
	 * mxfs: the block is being freed — its content is dead by decision,
	 * so retire the node-local undestaged accounting.  A binval'd block
	 * needs NO destage (the free is log-only; the CANCEL record
	 * suppresses replay of prior logs), but b_mxfs_logged_seq >
	 * b_mxfs_written_seq survives the free on the cached struct.  If the
	 * daddr is later reallocated (dir_reuse), the recycled/lingering
	 * buffer reads as "committed content never landed" while its bytes
	 * are the dead pre-free image: the dir release fence can neither
	 * prove it durable (data_durable=0, P42-RELDUR bad=1) nor write it
	 * (P3F-UNLANDED-LOST refuses non-dir magic), so it spins until the
	 * P97 backstop SHUTS DOWN the node while peers starve behind the
	 * multi-minute EX hold (proven: 32-node dir_reuse r3 — rank1's r2
	 * bmbt leaf daddr=18839216 freed by the rm-tail btree collapse,
	 * daddr reallocated as an r3 dir data block, 15006 P3F laps, P97
	 * shutdown, 14 peers died rc=-110).  Equalizing here is safe: an
	 * aborted-dirty transaction forces shutdown, so the pre-free debt
	 * this clears can never be owed again by a running FS.
	 */
