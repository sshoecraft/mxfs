	...
	xfs_lsn_t			current_lsn)
{
	...
	if (dip->di_version >= 3) {
		...
		/*
		 * MXFS foreign-slice replay: current_lsn comes from the DEAD
		 * node's per-node log slice; the on-disk di_lsn was stamped by
		 * whichever live node last flushed this inode from ITS OWN
		 * slice.  Per-node slices have independent cycle/block
		 * numbering, so XFS_LSN_CMP between them is meaningless and the
		 * normal newer-on-disk skip below cannot fire.  Use the
		 * node-independent di_changecount (monotonic per-inode across
		 * all nodes, since every modifier reads-current-then-increments
		 * under DLM) to decide: if the on-disk inode's changecount is
