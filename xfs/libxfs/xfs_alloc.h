...
typedef struct xfs_alloc_arg {
	...
	/*
	 * MXFS: non-blocking per-AG DLM acquire for single-AG allocation
	 * attempts (the exact_bno/near_bno locality optimizations).  When set,
	 * xfs_alloc_vextent_prepare_ag uses mxfs_ag_dlm_trylock even without
	 * XFS_ALLOC_FLAG_TRYLOCK: a peer-held AG fails this attempt (the
	 * caller falls back to another AG / the start_ag sweep) instead of
	 * blocking up to 120s.  Blocking on ONE specific AG while holding a
	 * shared-dir EX is the proven create-growth<->dialloc AG<->dir
	 * deadlock (8/tcp dir_reuse: P36-RETRY type=3 60x -> rc=-110 -> dirty
	 * xfs_trans_cancel -> shutdown).  Nested/cached self-held AGs still
	 * succeed instantly via the holder-count fast path, so callers that
	 * already hold the AG (dialloc chunk allocs) are unaffected.
	 */
	bool		mxfs_ag_trylock;
	/*
	 * MXFS diagnostic-only: owner inode of a bmap-path allocation.
	 * xfs_bmap_btalloc passes XFS_RMAP_OINFO_SKIP_UPDATE (oi_owner=0),
	 * which made the P-DBLALLOC in-the-act detector dead code for file
	 * DATA allocations — the exact class that lands urandom over a live
	 * dir block (daddr-120 EFSBADCRC shutdowns).  Carries ap->ip->i_ino
	 * without touching rmap semantics.
	 */
	uint64_t	mxfs_diag_owner;
...
struct xfs_extent_free_item {
	...
	/* MXFS: -EAGAIN requeue cycles consumed waiting out a peer-held AG
	 * DLM in xfs_extent_free_finish_item (both alloc sites zalloc). */
	unsigned int		xefi_agwait;
};
