	...
	struct xfs_buf	*bp)
{
	 ...
	 */
	if (!bp->b_error) {
		extern void mxfs_dir_bmbt_track(struct xfs_buf *);
	...
	struct xfs_buf	*bp)
{
	 ...
	 * Log owner ino + numrecs + comm so we can catch a STALE leaf write
	 * (low numrecs landing AFTER the dir advanced = the di>leaf torn pair a
	 * peer reads as P59-IREAD-MISMATCH).  comm=xfsaild => background-
	 * writeback clobber (no DLM gate); comm=mkdir => in-tenure.  Almost
	 * exclusively the shared storm dir (only BTREE-format inode), low volume.
	 */
	{
		...
		if (blk && be16_to_cpu(blk->bb_level) == 0) {
			unsigned int nr = be16_to_cpu(blk->bb_numrecs);
			struct xfs_bmbt_irec r0 = {0}, rl = {0};

			if (nr >= 1 && nr <= 200) {
				xfs_bmbt_disk_get_all(
					xfs_bmbt_rec_addr(bp->b_mount, blk, 1), &r0);
				xfs_bmbt_disk_get_all(
			...
			pr_warn_ratelimited("mxfs: P66-LEAFWRITE owner=%llu numrecs=%u daddr=%lld tenure=%llu lsn=%llu off0=%llu offN=%llu comm=%s\n",
				...
				(unsigned long long)be64_to_cpu(blk->bb_u.l.bb_lsn),
				(unsigned long long)r0.br_startoff,
				(unsigned long long)rl.br_startoff,
				current->comm);
		}
	}
	...
	unsigned int		new_numrecs)
{
	...
	/*
	 * mxfs v0.10.31 guard: if_broot == NULL with a nonzero byte count is
	 * a torn fork (see xfs_idestroy_fork).  Heal it loudly rather than
	 * returning NULL out of the nop path below (NULL+4 create oops).
	 */
	if (WARN_ONCE(!ifp->if_broot && old_size != 0,
		      "mxfs: P80-BROOT-TORN ino=%llu broot=NULL bytes=%u",
		      (unsigned long long)ip->i_ino, old_size)) {
