	...
	struct xfs_buf		*bp)
{
	...
	if (error) {
		pr_warn("mxfs: P62-SBV which=COMMON err=%d daddr=0x%llx dlm=%d single=%d fdblocks=%llu/%llu icount=%llu ifree=%llu\n",
			...
			(unsigned long long)sb.sb_ifree);
		goto out_error;
	}
	 ...
	 * buffer, because in a cluster the per-node-private lazy counters can
	 * legitimately drift out of single-node range (a node freeing space a
	 * peer allocated drives its fdblocks past sb_dblocks).  But the SB
	 * buffer reaches verify_write via paths that can carry an un-clamped
	 * image to disk (observed: xfs_sb_write_verify CORRUPT_INCORE shutdown
	 * mid cache_coherency even with the xfs_log_sb clamp in place).  The
	 * AUTHORITATIVE free record is the per-AG AGF/AGI (DLM+FUA coordinated)
	 * and the SB summary is rebuilt from AG counts on log recovery, so an
	 * out-of-range lazy counter here is benign — clamp the on-disk buffer
	 * in place (CRC is computed AFTER this, below) rather than force-
	 * shutting-down the whole cluster.  Primary SB + multi-node only;
	 ...
	 * benign transient icount=0 tripped xfs_verify_icount -> CORRUPT_INCORE
	 * shutdown mid cache_coherency (test_rename_visibility).  Gate instead on
	 * a PERSISTENT indicator: live DLM => strict multi-node check; DLM gone
	 * (unmount) but an MXFS envelope present => still clamp (the on-disk SB
	 * summary is rebuilt from per-AG AGI/AGF on the next mount, so an
	 * out-of-range lazy counter at unmount is benign; a healthy single-node
	 * MXFS keeps in-range counters so the clamp body below is a no-op there).
	 */
	if (xfs_buf_daddr(bp) == XFS_SB_DADDR && !sb.sb_inprogress &&
	    (mp->m_mxfs_dlm ? !mxfs_v5_dlm_is_single_node(mp->m_mxfs_dlm)
			    : mp->m_mxfs_has_envelope)) {
		unsigned long long imin, imax;
		bool clamped = false;

		if (sb.sb_fdblocks > sb.sb_dblocks) {
			sb.sb_fdblocks = sb.sb_dblocks;
			dsb->sb_fdblocks = cpu_to_be64(sb.sb_fdblocks);
			clamped = true;
		}
		xfs_icount_range(mp, &imin, &imax);
		if (sb.sb_icount < imin || sb.sb_icount > imax) {
			sb.sb_icount = sb.sb_icount < imin ? imin : imax;
			dsb->sb_icount = cpu_to_be64(sb.sb_icount);
			clamped = true;
		}
		if (sb.sb_ifree > sb.sb_icount) {
			sb.sb_ifree = sb.sb_icount;
			dsb->sb_ifree = cpu_to_be64(sb.sb_ifree);
			clamped = true;
		}
		if (clamped)
	...
	struct xfs_trans	*tp)
{
	 ...
	 * mount and advanced only by THIS node's own allocs/frees.  In a
	 * cluster, peers allocate and free the same shared space without
	 * touching this node's counters, so a node that frees blocks/inodes
	 * a peer allocated can drive its fdblocks past sb_dblocks (or icount
	 * out of its valid range) — a benign divergence, since the
	 * AUTHORITATIVE free-space record is the per-AG AGF/AGI (DLM- +
	 * FUA-coordinated), and the SB summary is explicitly rebuilt from the
	 * AGF block counts on log recovery (see the lazysbcount comment
	 * above).  Upstream's single-node write-verify
	 * (xfs_validate_sb_write: fdblocks<=dblocks, valid icount,
	 * ifree<=icount) treats such a value as in-core corruption and
	 * force-shuts-down the FS (observed: cross_write_read 4-node ->
	 * xfs_sb_write_verify CORRUPT_INCORE).  XFS already clamps ifree to
	 * icount and uses the _positive variant for fdblocks/frextents for
	 * exactly this class of "lazy counter out of range" problem; extend
	 * the same defensive clamp to the cluster fdblocks/icount case so a
	 * benign per-node divergence does not masquerade as corruption.
	 * Single-node keeps upstream behaviour untouched.
	 */
	{
		...
		if (mp->m_mxfs_dlm &&
			...
			xfs_icount_range(mp, &imin, &imax);
			if (mp->m_sb.sb_icount < imin)
				mp->m_sb.sb_icount = imin;
			else if (mp->m_sb.sb_icount > imax)
				mp->m_sb.sb_icount = imax;
