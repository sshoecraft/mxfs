...
out_shutdown:
	/*
	 * MXFS: -ETIMEDOUT here is a cluster AG-DLM acquire that outlived
	 * every retry (see mxfs_efi_agwait_max), not a metadata corruption —
	 * name it explicitly so the shutdown is attributable at a glance.
	 * Any intent type OTHER than extent-free reaching this with -110
	 * lacks a requeue guard and needs the same treatment.
	 */
	if (error == -ETIMEDOUT)
