...
extern int mxfs_pal_scsi_read_fua_bdev(struct block_device *bdev,
				       ...
				       uint32_t len);
extern bool mxfs_buf_has_uncheckpointed_mods(struct xfs_buf *bp);
	...
	void			*priv)
{
	 ...
	 * ever invalidates cached xfs_bmbt_buf_ops buffers; the read-time
	 ...
	 * uncheckpointed local mods).  It survived the reload-time eviction
	 * (mxfs_dir_evict_bmbt_blocks) — whether by an rhashtable-walk miss or a
	 * transient pin is the open (a)/(b) question, but THIS fix is independent
	 * of that: it repairs the harm at the point of consumption.  Consuming the
	 * stale leaf makes ir.loaded < if_nextents → EFSCORRUPTED → FS shutdown →
	 * the 1600-dirent zsl cascade.  Overwrite the clean stale cached leaf with
	 * the fresh FUA on-disk image so the records read out below are the peer's
	 * committed map.  ONLY when !ahead: an AHEAD buffer carries our own
	 * un-destaged newer work and must NOT be clobbered (it is the release-side
	 * durability path's responsibility, not a stale read).  Cost is one FUA
	 * read per leaf per (re)load (xfs_need_iread_extents gates the caller), so
	 * it is bounded and concentrated on large/contended dirs where the bug
	 * lives; single-node and non-dir paths are excluded.
	 */
	if (bp && whichfork == XFS_DATA_FORK && S_ISDIR(VFS_I(ip)->i_mode) &&
	    mp->m_mxfs_dlm && !mxfs_v5_dlm_is_single_node(mp->m_mxfs_dlm)) {
		uint32_t	p34_len = BBTOB(bp->b_length);
		void		*p34_buf = kmalloc(p34_len, GFP_NOFS);

		if (p34_buf) {
			uint64_t p34_lba = (uint64_t)bp->b_maps[0].bm_bn +
				mp->m_ddev_targp->bt_sector_offset;
			int p34_rc = mxfs_pal_scsi_read_fua_bdev(
				mp->m_ddev_targp->bt_bdev, p34_lba,
				p34_buf, p34_len);

			if (p34_rc == 0) {
				struct xfs_btree_block *p34_blk = p34_buf;
				uint16_t p34_drecs =
					be16_to_cpu(p34_blk->bb_numrecs);
				 ...
				 * flush since — BLI retired, nothing pinned/
				 * dirty/in-AIL, but the FUA platter read still
				 * returns the PRE-write image; the old
				 * predicate judged it "clean stale" and
				 * REGRESSED the leaf to the pre-grow platter
				 * image at decode (iext one left-merge behind
				 * the leaf -> bunmapi i!=1).  The v0.10.28
				 * b_tenure_id==ex_grant_seq form was REFUTED
				 * (run 122308Z): the grant seq does not bump
				 * on fast-path re-admits, so a stale leaf
				 * resurrected across a REAL handoff matched
				 * "same tenure" and blocked the NEEDED refresh
				 * (ir.loaded=23 vs nx=32 EFSCORRUPTED).  Use
				 * the mount flush epoch instead: written this
				 * epoch => no flush since => platter may be
				 * behind THIS content => never regress; any
				 * release flush advances the epoch and (a
				 * peer handoff always implies our release
				 * flush) re-enables the refresh exactly when
				 * peer-newer platter content is possible.
				 */
				bool	ahead = mxfs_buf_has_uncheckpointed_mods(bp) ||
					(bp->b_mxfs_wr_flush_epoch != 0 &&
					 bp->b_mxfs_wr_flush_epoch ==
					 atomic64_read(&mp->m_mxfs_flush_epoch));

				if (stale) {
					static atomic_t p34n = ATOMIC_INIT(0);
					if (atomic_inc_return(&p34n) <= 2000 ||
						...
						pr_warn("mxfs: P34B-BMBT-%s ino=%llu daddr=%lld level=%d cached_recs=%u disk_recs=%u bflags=0x%x wr_fepoch=%llu cur_fepoch=%llu realns=%llu\n",
							...
							(unsigned)p34_drecs,
							bp->b_flags,
							(unsigned long long)bp->b_mxfs_wr_flush_epoch,
							(unsigned long long)atomic64_read(&mp->m_mxfs_flush_epoch),
							(unsigned long long)ktime_get_real_ns());
				}

				/*
				 * THE FIX: refresh the clean stale leaf in place
				 * from the proven-correct on-disk image, then
				 * re-read num_recs from the refreshed block so the
				 * record-copy loop below consumes the peer's map.
				 */
				if (stale && !ahead) {
					static atomic_t p68n = ATOMIC_INIT(0);
					memcpy(bp->b_addr, p34_buf, p34_len);
					num_recs = xfs_btree_get_numrecs(block);
	 ...
	 * (ifp->if_nextents), adopted at the last inode reload — can be STALE-LOW
	 * relative to the just-refreshed fresh leaf when a peer grew the dir
	 * between this node's dinode reload and this lazy extent read (the
	 * i_dlm_dir_gen-notify race that lets a fastex re-acquire self-skip the
	 ...
	 * leafsum == if_nextents; every P68 refresh matched), so the on-disk
	 * dinode is the authority.  FUA cache-bypassing re-read THIS inode's
	...
	if (bp && whichfork == XFS_DATA_FORK && S_ISDIR(VFS_I(ip)->i_mode) &&
	    mp->m_mxfs_dlm && !mxfs_v5_dlm_is_single_node(mp->m_mxfs_dlm) &&
	    ir->loaded + num_recs > ifp->if_nextents) {
		uint32_t	d_len = BBTOB(ip->i_imap.im_len);
		void		*d_buf = d_len ? kmalloc(d_len, GFP_NOFS) : NULL;

		if (d_buf) {
			uint64_t d_lba = (uint64_t)ip->i_imap.im_blkno +
				mp->m_ddev_targp->bt_sector_offset;
			int d_rc = mxfs_pal_scsi_read_fua_bdev(
				mp->m_ddev_targp->bt_bdev, d_lba, d_buf, d_len);

			if (d_rc == 0) {
				struct xfs_dinode *d_dip =
					d_buf + ip->i_imap.im_boffset;
				xfs_extnum_t disk_nx =
	...
	int			whichfork)
{
	...
	if (ir.loaded != ifp->if_nextents) {
		...
		pr_warn(
			...
			(unsigned long long)ir.loaded,
			(unsigned long long)ifp->if_nextents,
			ifp->if_broot ? be16_to_cpu(ifp->if_broot->bb_level) : 0xffff);
	}
	...
	uint32_t		flags)
{
			...
			if (i != 1 && whichfork == XFS_DATA_FORK &&
				...
				pr_warn("mxfs: P64-LEFTCONTIG-DESYNC ino=%llu iext=%llu leaf_numrecs=%u broot_nrecs=%u old_off=%llu old_sb=%llu old_cnt=%llu daddr=%lld comm=%s\n",
					...
					be16_to_cpu(XFS_BUF_TO_BLOCK(
						cur->bc_levels[0].bp)->bb_numrecs),
					be16_to_cpu(ip->i_df.if_broot->bb_numrecs),
					(unsigned long long)old.br_startoff,
					(unsigned long long)old.br_startblock,
					(unsigned long long)old.br_blockcount,
					(long long)xfs_buf_daddr(
						cur->bc_levels[0].bp),
					current->comm);
			}
			if (XFS_IS_CORRUPT(mp, i != 1)) {
	...
	case 0:
		...
		if (cur == NULL) {
			...
			uint16_t pre_lnr = 0xffff;
			long long pre_daddr = -1;
			rval = XFS_ILOG_CORE;
			error = xfs_bmbt_lookup_eq(cur, new, &i);
			if (error)
				goto done;
			...
			if (whichfork == XFS_DATA_FORK && cur->bc_levels[0].bp &&
				...
				pre_lnr = be16_to_cpu(XFS_BUF_TO_BLOCK(
					cur->bc_levels[0].bp)->bb_numrecs);
				pre_daddr = (long long)xfs_buf_daddr(
					cur->bc_levels[0].bp);
			}
			 ...
			 * inserted into does NOT hold if_nextents records) so volume
			 * is self-limiting.  If this fires the desync is BORN in the
			...
			if (whichfork == XFS_DATA_FORK && cur->bc_levels[0].bp &&
			    ...
			    be16_to_cpu(XFS_BUF_TO_BLOCK(
				cur->bc_levels[0].bp)->bb_level) == 0) {
				struct xfs_buf *lbp = cur->bc_levels[0].bp;
				...
				if ((unsigned long long)lnr != ifp->if_nextents)
					pr_warn("mxfs: P63-INSERT-DESYNC ino=%llu if_nextents=%llu post_lnr=%u pre_lnr=%u pre_daddr=%lld daddr=%lld bf=0x%x dg=%u sb=%llu comm=%s\n",
						...
						(long long)xfs_buf_daddr(lbp),
						lbp->b_flags,
						lbp->b_mxfs_dir_gen,
						(unsigned long long)new->br_startblock,
						current->comm);
			}
		}
		break;
	...
	bool			ag_only)
{
	...
	if (ap->eof) {
		...
		/*
		 * MXFS: this exact-bno attempt targets ONE specific AG (the
		 * EOF-adjacent block's AG) purely for locality.  Never block
		 * on a peer-held AG DLM here — under a held shared-dir EX
		 * that is the proven create-growth AG<->dir deadlock.  Fail
		 * the attempt instead; the aligned/start_ag fallbacks below
		 * sweep other AGs (blocking second pass remains the ENOSPC
		 * backstop there).
		 */
		args->mxfs_ag_trylock = true;
		error = xfs_alloc_vextent_exact_bno(args, ap->blkno);
		args->mxfs_ag_trylock = false;
		if (!caller_pag) {
	...
	if (ag_only) {
		/* MXFS: single-AG locality attempt — never block on a
		 * peer-held AG DLM (see the exact_bno comment above). */
		args->mxfs_ag_trylock = true;
	...
	int			stripe_align)
{
	...
	if (!error && args->fsbno == NULLFSBLOCK) {
		/* MXFS: single-AG locality attempt — never block on a
		 * peer-held AG DLM; low_space below sweeps other AGs. */
		args->mxfs_ag_trylock = true;
		error = xfs_alloc_vextent_near_bno(args, ap->blkno);
		args->mxfs_ag_trylock = false;
	}
	...
	int			stripe_align)
{
	 ...
	 * node's dir-block grow targeted the ONE home AG; for 7 of 8 nodes
	 * that is a peer-held AG and xfs_alloc_vextent_prepare_ag's bounded
	 * NOQUEUE poll burns up to 4s per peer-held AG in msleep — stack-
	 * proven 7-25s single-create stalls (runs 40/41), each of which
	 * starves the whole cluster behind the held dir EX.  Shared-dir
	 * block contiguity is worthless (8 writers already interleave), so
	 * also skip the EOF-contiguity attempt, which targets the home AG.
	 */
	if (args->mp->m_mxfs_dlm &&
	    !mxfs_v5_dlm_is_single_node(args->mp->m_mxfs_dlm) &&
	    S_ISDIR(VFS_I(ap->ip)->i_mode) &&
	    xfs_bmapi_whichfork(ap->flags) == XFS_DATA_FORK) {
		xfs_agnumber_t	affine_agno = args->mp->m_mxfs_node_slot %
					      args->mp->m_maxagi;

		error = xfs_alloc_vextent_start_ag(args,
				XFS_AGB_TO_FSB(args->mp, affine_agno, 0));
		if (error || args->fsbno != NULLFSBLOCK)
			return error;
	...
	struct xfs_bmalloca	*ap)
{
	...
	struct xfs_alloc_arg	args = {
		...
		.alignment	= 1,
		.minalignslop	= 0,
		/* MXFS: P-DBLALLOC detector owner (diagnostic; rmap unaffected) */
		.mxfs_diag_owner = ap->ip->i_ino,
	};
	xfs_fileoff_t		orig_offset;
	xfs_extlen_t		orig_length;
	int			error;
	...
	*logflagsp = XFS_ILOG_CORE;
	...
	if (cur) {
		...
		if (XFS_IS_CORRUPT(mp, i != 1)) {
			 ...
			 * a raw coherent read of the same daddr:
			 *   same  → the LUN agrees with the cached leaf ⇒ the
			 *           IEXT is the stale side (kept across a
			 *           tenure the leaf moved past);
			 *   differ→ the cursor used a STALE CACHED leaf (the
			 *           evict missed it).
			 */
			struct xfs_buf	*lbp = cur->bc_levels[0].bp;

			if (lbp && lbp->b_addr && lbp->b_target &&
			    lbp->b_target->bt_bdev &&
			    S_ISDIR(VFS_I(ip)->i_mode)) {
				extern int mxfs_pal_bdev_read_plain_bdev(
					struct block_device *, uint64_t,
					void *, uint32_t);
				uint32_t blen = BBTOB(lbp->b_length);
				void *raw = ((blen & 511) == 0 && blen) ?
					kmalloc(blen, GFP_NOFS) : NULL;
				int rrc = -1, samelun = -1;
				struct xfs_btree_block *cb = lbp->b_addr;

				if (raw)
					rrc = mxfs_pal_bdev_read_plain_bdev(
						lbp->b_target->bt_bdev,
						(uint64_t)lbp->b_maps[0].bm_bn +
						lbp->b_target->bt_sector_offset,
						raw, blen);
				if (rrc == 0)
				...
				pr_warn("mxfs: P75-BMBT-DEL-MISMATCH ino=%llu got_off=%llu got_blk=%lld got_cnt=%llu leaf_daddr=%lld leaf_recs=%u leaf_lsn=0x%llx bflags=0x%x fua_fresh=%d samelun=%d rrc=%d dlm_mode=%u comm=%s — %s\n",
					...
					(unsigned long long)got.br_blockcount,
					(long long)lbp->b_maps[0].bm_bn,
					be16_to_cpu(cb->bb_numrecs),
					(unsigned long long)be64_to_cpu(
						cb->bb_u.l.bb_lsn),
					lbp->b_flags,
					!!(lbp->b_flags & _XBF_FUA_FRESH),
					samelun, rrc,
				 ...
				 * distinguishable from a truly absent
				 * mapping. */
				{
					...
					for (ri = 0; ri < nr; ri++) {
						struct xfs_bmbt_irec lr;

						xfs_bmbt_disk_get_all(
							xfs_bmbt_rec_addr(mp,
								cb, 1 + ri),
							&lr);
						if (lr.br_startoff + lr.br_blockcount <
						    got.br_startoff - 2 ||
						    lr.br_startoff >
						    got.br_startoff +
						    got.br_blockcount + 2)
							continue;
						pr_warn("mxfs: P75b-LEAFREC ino=%llu rec[%d] off=%llu blk=%lld cnt=%llu\n",
							(unsigned long long)ip->i_ino,
							ri,
							(unsigned long long)lr.br_startoff,
							(long long)lr.br_startblock,
							(unsigned long long)lr.br_blockcount);
					}
				}
				{
					struct xfs_iext_cursor	pcur;
					struct xfs_bmbt_irec	prec;
					int			pk = 0;

					for_each_xfs_iext(ifp, &pcur, &prec) {
						if (prec.br_startoff + prec.br_blockcount <
						    got.br_startoff - 2 ||
						    prec.br_startoff >
						    got.br_startoff +
						    got.br_blockcount + 2) {
						...
						pr_warn("mxfs: P75b-IEXTREC ino=%llu rec[%d] off=%llu blk=%lld cnt=%llu\n",
							(unsigned long long)ip->i_ino,
							pk,
							(unsigned long long)prec.br_startoff,
							(long long)prec.br_startblock,
							(unsigned long long)prec.br_blockcount);
						pk++;
					}
				}
				kfree(raw);
	...
	if (!(bflags & XFS_BMAPI_REMAP)) {
		...
		if (xfs_is_reflink_inode(ip) && whichfork == XFS_DATA_FORK) {
			 ...
			 * queuer.  A reused/stale incarnation freeing a peer-owned
			 * extent = the double-free root.  Multi-node, AG0, capped. */
			if (mp->m_mxfs_dlm &&
				...
				if (atomic_inc_return(&p3ef) <= 20000)
					pr_warn("mxfs: P3-EFREE-Q ino=%llu gen=%u dlm_mode=%u stale=%d agbno=%u len=%u fork=%d comm=%s\n",
						...
						(unsigned)XFS_FSB_TO_AGBNO(mp, del->br_startblock),
						(unsigned)del->br_blockcount,
						whichfork, current->comm);
			}
	...
	xfs_extnum_t		nexts)		/* number of extents max */
{
	 ...
	 * extents T1 never allocated.  This diagnostic prints the inode
	 * and the in-memory bmap so we can see what extents the unmap is
	 * about to traverse.
	 */
	if (unlikely(mxfs_instr_enabled || mxfs_dirwr_enabled) &&
	    ...
	    (ip->i_df.if_format == XFS_DINODE_FMT_EXTENTS ||
	     ip->i_df.if_format == XFS_DINODE_FMT_BTREE)) {
		struct xfs_iext_cursor p29_icur;
		struct xfs_bmbt_irec p29_irec;
		int p29_count = 0;
		...
		pr_warn("mxfs: P29-INSTR bunmapi-entry ino=0x%llx start=%llu len=%llu fmt=%d nextents=%lld\n",
			...
			ip->i_df.if_format,
			(long long)(p29_ifp ? p29_ifp->if_nextents : 0));
		if (p29_ifp && xfs_iext_lookup_extent(ip, p29_ifp, 0,
						       &p29_icur, &p29_irec)) {
			do {
				if (p29_count++ < 6)
					pr_warn("mxfs: P29-INSTR  ext[%d] off=%llu sb=0x%llx len=%llu\n",
						...
						(unsigned long long)p29_irec.br_blockcount);
			} while (xfs_iext_next_extent(p29_ifp,
						       &p29_icur, &p29_irec));
		}
	}
