...
bool xfs_dir_isempty(struct xfs_inode *dp);	/* sess466: dirshard rmdir */
...
extern int xfs_dir_lookup(struct xfs_trans *tp, struct xfs_inode *dp,
				const struct xfs_name *name, xfs_ino_t *inum,
				struct xfs_name *ci_name, uint8_t *ftypep);
/* mxfs: lock-free lookup for callers already holding dp ILOCK (see xfs_dir2.c) */
extern int xfs_dir_lookup_locked(struct xfs_trans *tp, struct xfs_inode *dp,
				const struct xfs_name *name, xfs_ino_t *inum);
