	...
	int		flags)		/* flags, XFSA_FIXUP_... */
{
	...
	if (flags & XFSA_FIXUP_CNT_OK) {
		...
		if (XFS_IS_CORRUPT(mp, i != 1)) {
			 ...
			 * pag_dlm_meta_gen → read-time invalidation missed
			 * them) or gen-fresh (gen matches but content wrong →
			 * on-disk inconsistency / false-fresh stamp).
			 */
			struct xfs_perag *p70_pag = to_perag(cnt_cur->bc_group);
			...
			pr_warn("mxfs: P70-INSTR cnt-lookup-fail agno=%u fbno=%u flen=%u rbno=%u rlen=%u flags=0x%x pag_gen=%llu cached=%d holders=%d "
				...
				(unsigned long long)p70_pag->pag_dlm_meta_gen,
				p70_pag->pag_dlm_cached, p70_pag->pag_dlm_holders,
				p70_cb ? (long long)p70_cb->b_maps[0].bm_bn : -1LL,
				(unsigned long long)(p70_cb ? p70_cb->b_mxfs_ag_gen : 0),
				p70_cb ? p70_cb->b_flags : 0, p70_cd,
				p70_bb ? (long long)p70_bb->b_maps[0].bm_bn : -1LL,
				(unsigned long long)(p70_bb ? p70_bb->b_mxfs_ag_gen : 0),
				p70_bb ? p70_bb->b_flags : 0, p70_bd);
			xfs_btree_mark_sick(cnt_cur);
			return -EFSCORRUPTED;
		}
	}
	...
	if (flags & XFSA_FIXUP_BNO_OK) {
		...
		if (XFS_IS_CORRUPT(mp, i != 1)) {
			...
			int p70_cnr = p70_cb ? be16_to_cpu(XFS_BUF_TO_BLOCK(p70_cb)->bb_numrecs) : -1;
			...
			pr_warn("mxfs: P70-INSTR bno-lookup-fail agno=%u fbno=%u flen=%u rbno=%u rlen=%u flags=0x%x pag_gen=%llu cached=%d holders=%d "
				...
				p70_cb ? (long long)p70_cb->b_maps[0].bm_bn : -1LL,
				(unsigned long long)(p70_cb ? p70_cb->b_mxfs_ag_gen : 0),
				p70_cb ? p70_cb->b_flags : 0, p70_cd, p70_cnr,
				p70_bb ? (long long)p70_bb->b_maps[0].bm_bn : -1LL,
				(unsigned long long)(p70_bb ? p70_bb->b_mxfs_ag_gen : 0),
				p70_bb ? p70_bb->b_flags : 0, p70_bd, p70_bdirty, p70_bpin, p70_bnr,
				p70_gbno, p70_glen, p70_gs);
			xfs_btree_mark_sick(bno_cur);
			return -EFSCORRUPTED;
		}
	}
	 ...
	 * cnt buffer, its delete/insert is applied to a stale base → the
	 * peer's recent free-space changes are clobbered = the LOST cnt-tree
	 ...
	 * READ(16) round trip (mxfs_ag_buf_disk_differs).  Measured: 9248
	 * READ_16s per canonical 8137-file rsync = the single largest
	 * single_node_paired overhead (ratio 151% -> see bench.json).
	 */
	if (unlikely(mxfs_instr_enabled)) {
		struct xfs_perag *p74_pag = to_perag(cnt_cur->bc_group);
		struct xfs_buf *p74_cb = cnt_cur->bc_levels[0].bp;
		if (p74_cb && p74_pag->pag_dlm_meta_gen &&
		    p74_cb->b_mxfs_ag_gen < p74_pag->pag_dlm_meta_gen) {
			int p74_d = mxfs_ag_buf_disk_differs(p74_cb);
			...
			pr_warn("mxfs: P74-INSTR cnt-modify-on-STALE-buf agno=%u fbno=%u flen=%u rbno=%u rlen=%u "
				...
				(unsigned long long)p74_cb->b_mxfs_ag_gen,
				(unsigned long long)p74_pag->pag_dlm_meta_gen,
				p74_d, p74_pin, p74_dirty, p74_pag->pag_dlm_holders);
		}
	}
	 ...
	 * undisturbed.  Fires only on the rare inconsistency.  This pinpoints the
	 * exact alloc/free op that produces the divergence (P70 root).
	 */
	if (pag_agno(to_perag(cnt_cur->bc_group)) == 1 &&
	    cnt_cur->bc_nlevels == 1 && bno_cur->bc_nlevels == 1) {
		xfs_agblock_t p76_res[2] = { nfbno1, nfbno2 };
		xfs_extlen_t  p76_rl[2]  = { nflen1, nflen2 };
		int k;
		for (k = 0; k < 2; k++) {
			struct xfs_btree_cur *cdup = NULL, *bdup = NULL;
			int ci = -1, bi = -1, de;
			if (p76_res[k] == NULLAGBLOCK)
				continue;
			de = xfs_btree_dup_cursor(cnt_cur, &cdup);
			if (!de)
				de = xfs_btree_dup_cursor(bno_cur, &bdup);
			if (!de) {
				if (xfs_alloc_lookup_eq(cdup, p76_res[k], p76_rl[k], &ci))
					ci = -2;
				...
				if (ci != bi)
					pr_warn("mxfs: P76-INSTR fixup-LEFT-INCONSISTENT agno=1 residual=(%u,%u) in_cnt=%d in_bno=%d allocd=(%u,%u) from=(%u,%u) cnt_bp=%p bno_bp=%p\n",
						...
						bno_cur->bc_levels[0].bp);
			}
			if (bdup)
				xfs_btree_del_cursor(bdup, XFS_BTREE_NOERROR);
			if (cdup)
				xfs_btree_del_cursor(cdup, XFS_BTREE_NOERROR);
		}
	}
	...
	enum xfs_ag_resv_type		type)
{
	 ...
	 * set, not that we are the SOLE EX holder — and a popcount>1 "concurrent
	 * EX" slot is auto-repaired by slot_appears_corrupt before ag_held sees
	 * it.  So the last unchecked lead is TRANSIENT CONCURRENT EX (both nodes
	 * believe they hold via last-write-wins CAS).  NEXT: a raw slot read at
	 * modify time logging popcount(holders_ex) WITHOUT the repair (add a
	 * dlm_caw helper that reads the slot raw); popcount>1 = concurrent EX =
	 * the root.  See state.md.
	 */
	...
	if (haveleft) {
		...
		if (ltbno + ltlen < bno)
			 ...
			 * containment only; partial overlap keeps the hard corruption. */
			{
				extern int mxfs_ag_skip_dblfree;
				if (mxfs_ag_skip_dblfree && ltbno + ltlen > bno &&
				    mp->m_mxfs_dlm &&
			...
			if (XFS_IS_CORRUPT(mp, ltbno + ltlen > bno)) {
				struct xfs_agf *p15_agf = agbp->b_addr;
				struct xfs_buf *p28_lbp = NULL;
				...
				pr_warn("P15-INSTR realns=%llu FREE-AG-EXTENT-FAIL-LEFT caller=%pS caller2=%pS comm=%s agno=%u bno=%u len=%u ltbno=%u ltlen=%u haveright=NA agf_freeblks=%u agf_longest=%u pagf_freeblks=%u pagf_longest=%u\n",
					...
					(unsigned)bno, (unsigned)len,
					(unsigned)ltbno, (unsigned)ltlen,
					(unsigned)be32_to_cpu(p15_agf->agf_freeblks),
					(unsigned)be32_to_cpu(p15_agf->agf_longest),
					...
					(unsigned)pag->pagf_longest);
					{
						...
						pr_warn("mxfs: P47-INACT agno=%u bno=%u inact_ino=%llu inact_agno=%llu incore_gen=%u disk_di_mode=0%o disk_di_gen=%u verdict=%s\n",
							...
							mxfs_dbg_inactive_gen,
							(unsigned)p47_dm, p47_dgen,
							p47_dm == 0 ? "DISK-FREE=>B-stale/double-free" :
						 ...
						 * range covers it => bnobt lost-update (tree
						 * wrong on disk); if it differs from the in-core
						 * extent the inactivation is freeing => the
						 * IN-CORE inode extent map is STALE vs disk
						 * (inode-coherency, not bnobt). */
						{
							extern int mxfs_dbg_disk_di_first_dext(
								struct xfs_mount *, xfs_ino_t,
								uint64_t *, uint64_t *, uint64_t *,
								uint64_t *, uint8_t *);
							uint64_t p81_sb = 0, p81_len = 0, p81_off = 0,
								 p81_nd = 0;
							uint8_t  p81_fmt = 0xFF;
							uint64_t p81_freed_fsb =
								XFS_AGB_TO_FSB(mp, pag_agno(pag), bno);
							int p81_rc = mxfs_dbg_disk_di_first_dext(
								mp, p47_ino, &p81_sb, &p81_len,
								&p81_off, &p81_nd, &p81_fmt);
							bool p81_disk_claims = p81_rc == 0 &&
								p81_len > 0 &&
							...
							pr_warn("mxfs: P81-DEXT ino=%llu freed_fsb=0x%llx disk_rc=%d disk_fmt=%u disk_ndext=%llu disk_ext0=[off=%llu,sb=0x%llx,len=%llu] disk_claims_freed=%d verdict=%s\n",
								...
								(unsigned long long)p81_nd,
								(unsigned long long)p81_off,
								(unsigned long long)p81_sb,
								(unsigned long long)p81_len,
								p81_disk_claims,
								p81_disk_claims ?
								  "DISK-INODE-OWNS-FREED=>bnobt-lost-update" :
								  "DISK-INODE-DIFFERS=>incore-extent-stale");
						}
					}
				/*
				 ...
				 * kill_block_super → evict_inodes → xfs_inactive →
				 * xfs_free_eofblocks → xfs_itruncate_extents →
				 * xfs_bunmapi → __xfs_free_extent → here).  The inode
				 * is the VICTIM (its EOF blocks are legitimately owned
				 * but the bnobt — clobbered to ~pristine during the
				 * concurrent test phase, P88 ts < unmount ts — shows
				 * them free) → the bnobt lost-update IS the root.
				 */
				if (p28_lbp) {
					 ...
					 * landed = WRITE/durability bug.  differs=1 =>
					...
					pr_warn("P28-INSTR LEFT-FAIL leaf_buf daddr=0x%llx flags=0x%x b_log_item=%p hold=%u disk_differs=%d in_ail=%d\n",
						...
						p28_lbp->b_flags,
						p28_lbp->b_log_item,
						p28_lbp->b_hold, p85_dd,
						(p28_lbp->b_log_item &&
						 test_bit(XFS_LI_IN_AIL,
						   &p28_lbp->b_log_item->bli_item.li_flags)) ? 1 : 0);
					p33_blk = xfs_btree_get_block(bno_cur,
							0, &p33_dump_bp);
					if (p33_blk && p33_dump_bp) {
						__u16 nr = be16_to_cpu(
							p33_blk->bb_numrecs);
						union xfs_btree_rec *r0 =
						...
						pr_warn("mxfs: P33-INSTR fail-bnobt-snap "
							...
							"bli=%p hold=%u nr=%u "
							"rec0=(%u,%u) recN=(%u,%u) tp=%p\n",
							(unsigned)pag_agno(pag),
							(unsigned long long)
							p33_dump_bp->b_maps[0].bm_bn,
							p33_dump_bp->b_flags,
							p33_dump_bp->b_log_item,
							p33_dump_bp->b_hold,
							(unsigned)nr,
							(unsigned)be32_to_cpu(
							   r0->alloc.ar_startblock),
							(unsigned)be32_to_cpu(
							   r0->alloc.ar_blockcount),
							(unsigned)be32_to_cpu(
							   rN->alloc.ar_startblock),
							(unsigned)be32_to_cpu(
							   rN->alloc.ar_blockcount),
							tp);
					}
				}
				xfs_btree_mark_sick(bno_cur);
		...
		if (bno + len < gtbno)
			...
			if (XFS_IS_CORRUPT(mp, bno + len > gtbno)) {
				...
				pr_warn("P14-INSTR realns=%llu FREE-AG-EXTENT-FAIL agno=%u bno=%u len=%u gtbno=%u gtlen=%u haveleft=%d ltbno=%u ltlen=%u agf_freeblks=%u agf_longest=%u pagf_freeblks=%u pagf_longest=%u\n",
					...
					(unsigned)pag_agno(pag),
					(unsigned)bno, (unsigned)len,
					(unsigned)gtbno, (unsigned)gtlen,
					haveleft,
					(unsigned)ltbno, (unsigned)ltlen,
					(unsigned)be32_to_cpu(p14_agf->agf_freeblks),
					(unsigned)be32_to_cpu(p14_agf->agf_longest),
					(unsigned)pag->pagf_freeblks,
					(unsigned)pag->pagf_longest);
				{
					struct xfs_buf *p28_lbp = bno_cur->bc_levels[0].bp;
					if (p28_lbp) {
						struct xfs_btree_block *p33_blk;
						struct xfs_buf *p33_dump_bp = NULL;
	...
	struct xfs_buf		**agfbpp)
{
	 ...
	 * under concurrent rename).  BEFORE the read, if the cached AGF buffer's
	 * stamp lags pag_dlm_meta_gen (bumped on every fresh AG acquire),
	 * invalidate it (clear XBF_DONE|_XBF_FUA_FRESH) so the sanctioned read
	 * below FUA-re-reads the peer's committed AGF.  Deadlock-safe pattern
	 * copied from xfs_da_read_buf's i_dlm_dir_gen block (XBF_TRYLOCK; skip
	 * if dirty/pinned/delwri — our copy is then authoritative).
	 * NOTE: covers the AGF only; the bnobt/cntbt/inobt BTREE blocks need the
	 * same b_mxfs_ag_gen check in their read path (next step).
	 */
	...
	uint32_t		alloc_flags)
{
	...
	/*
	 * MXFS: acquire per-AG DLM lock (holder counting handles nesting).
	 * On the XFS_ALLOC_FLAG_TRYLOCK first pass use the non-blocking
	 * acquire: if a peer holds this AG, treat it like a busy AGF (skip to
	 * the next AG via agbp==NULL) instead of blocking up to 120s with
	 * inode ILOCKs held — this breaks the block-alloc hold-and-wait
	 * deadlock.  The blocking second pass (no TRYLOCK) waits as before.
	 */
	if ((alloc_flags & XFS_ALLOC_FLAG_TRYLOCK) || args->mxfs_ag_trylock)
		 ...
		 * this AG while waiting for our dir → both -110 → dirty-
		 * cancel shutdowns.  Bounded acquire; -EAGAIN falls into the
		 * existing skip-to-next-AG path below, and the AG iteration
		 * terminates at an AG we already hold.
		 */
		error = mxfs_ag_dlm_lock_bounded(args->mp, args->pag);
	else
		error = mxfs_ag_dlm_lock(args->mp, args->pag);
	if (error) {
		if (error == -EAGAIN) {
	...
	if (error) {
		...
		mxfs_ag_dlm_unlock(args->mp, args->pag);
		if (need_pag)
			xfs_perag_put(args->pag);
		args->agbno = NULLAGBLOCK;
		return error;
	...
	bool			drop_perag)
{
	 ...
	 * next read (imap_to_bp rc=-5 / iformat_extents EFSCORRUPTED) -> forced
	 * shutdown.  To pick the fix we must know whether the allocator handed a
	 * DATA-fork request a block that is STILL an inode chunk (M1: stale
	 * free-space view) and whether the conflicting inode chunk was allocated
	 * by THIS node (intra-node, in-core free-space incoherency across one
	 * node's own ialloc->bmap sequence) or a peer (cross-node).  We keep a
	 * lean per-AG ring of inode-chunk extents THIS node allocates
	 * (oi_owner == XFS_RMAP_OWN_INODES) and, on every DATA-fork allocation
	 * (oi_owner is a real inode number, >= 0), scan the ring for overlap.
	 * Multi-node only; the ring scan is O(256) and cheap.  Catches the
	 * intra-node case decisively; the cross-node case shows up as a DATA
	 * allocation that overlaps NO ring entry yet still clobbers a cluster.
	 */
	if (mp->m_mxfs_dlm && !mxfs_v5_dlm_is_single_node(mp->m_mxfs_dlm) &&
	    args->len > 0 && args->pag) {
		enum { P55_RING = 256, P55_MAXAG = 64 };
		static struct { u32 agbno; u32 len; u32 slot; } p55_iring[P55_MAXAG][P55_RING];
		static u32 p55_ihead[P55_MAXAG];
		static DEFINE_SPINLOCK(p55_lock);
		xfs_agnumber_t agno = pag_agno(args->pag);
		u32 a0 = (u32)args->agbno, a1 = (u32)args->agbno + (u32)args->len;
		int myslot = mxfs_v5_dlm_get_node_slot(mp->m_mxfs_dlm);

		if (agno < P55_MAXAG) {
			if (args->oinfo.oi_owner == XFS_RMAP_OWN_INODES) {
				unsigned long fl;
				spin_lock_irqsave(&p55_lock, fl);
				{
					u32 h = p55_ihead[agno] % P55_RING;
					p55_iring[agno][h].agbno = a0;
					p55_iring[agno][h].len = (u32)args->len;
					p55_iring[agno][h].slot = (u32)myslot;
					p55_ihead[agno]++;
				}
				spin_unlock_irqrestore(&p55_lock, fl);
			} else if ((int64_t)args->oinfo.oi_owner >= 0) {
				unsigned long fl;
				int hit = -1;
				u32 hb = 0, hl = 0, hs = 0;
				spin_lock_irqsave(&p55_lock, fl);
				{
					int k;
					for (k = 0; k < P55_RING; k++) {
						u32 b0 = p55_iring[agno][k].agbno;
						u32 bl = p55_iring[agno][k].len;
						if (!bl)
							continue;
						if (a0 < b0 + bl && b0 < a1) {
							hit = k;
							hb = b0; hl = bl;
							hs = p55_iring[agno][k].slot;
							break;
						}
					}
				}
				...
				if (hit >= 0)
					pr_warn("mxfs: P55-ALLOC-OVER-INODE agno=%u DATA alloc=(%u,%u) owner_ino=%llu OVERLAPS inode-chunk=(%u,%u) chunk_slot=%u my_slot=%d %s wasfromfl=%d realns=%llu\n",
						...
						hb, hl, hs, myslot,
						(hs == (u32)myslot) ? "INTRA-NODE" : "CROSS-NODE",
						(int)args->wasfromfl,
						(unsigned long long)ktime_get_real_ns());
				 ...
				 * buffer (xfs_inode_buf_ops) at the daddr we just
				 * handed to a DATA-fork request.  If one is cached,
				 * we are about to write dir/file data over a live
				 * inode cluster regardless of who allocated it = the
				 * proven imap_to_bp clobber, caught at the SOURCE.
				 * Non-blocking (XBF_TRYLOCK), no btree / no AGI lock.
				 */
				{
					extern const struct xfs_buf_ops xfs_inode_buf_ops;
					extern const struct xfs_buf_ops xfs_inode_buf_ra_ops;
					int bpc = M_IGEO(mp)->blocks_per_cluster;
					xfs_agblock_t cb;

					if (bpc < 1)
						bpc = 1;
					for (cb = (a0 / bpc) * bpc; cb < a1; cb += bpc) {
						struct xfs_buf *ibp = NULL;
						xfs_daddr_t cd = xfs_agbno_to_daddr(args->pag, cb);
						if (xfs_buf_incore(mp->m_ddev_targp, cd,
						    XFS_FSB_TO_BB(mp, bpc),
				 ...
				 * inode-chunk allocation.  This plain LIO-coherent read of
				 * the just-allocated DATA daddr decisively tests whether the
				 * allocator handed a DATA request a block that ALREADY holds
				 * a LIVE inode cluster on the shared platter = cross-node AG
				 * free-space double-allocation (the proven 0xc40 inode-
				 * cluster-clobbered-by-urandom root).  Ratelimited so the
				 * dmesg ring does not wrap out early fires.  Fires =>
				 * cross-node double-alloc confirmed; 0 => M2 stale extent-map
				 * mis-write.
				 */
				/*
	...
	if (({ extern int mxfs_dblalloc_probe; mxfs_dblalloc_probe; }) &&
	    ...
	    ((s64)args->oinfo.oi_owner > 0 || args->mxfs_diag_owner > 0) &&
	    mp->m_ddev_targp &&
	    ({ static atomic_t p_dblalloc_n = ATOMIC_INIT(0);
	       atomic_inc_return(&p_dblalloc_n) <= 40000; })) {
		extern int mxfs_pal_bdev_read_plain_bdev(struct block_device *,
			uint64_t, void *, uint32_t);
		xfs_agnumber_t da_agno = pag_agno(args->pag);
		xfs_daddr_t da_daddr = XFS_AGB_TO_DADDR(mp, da_agno,
							args->agbno);
		uint32_t da_len = mp->m_sb.sb_blocksize;
		void *da_tmp = (da_len >= 512 && (da_len & 511) == 0) ?
			kmalloc(da_len, GFP_NOFS) : NULL;

		if (da_tmp &&
		    mxfs_pal_bdev_read_plain_bdev(mp->m_ddev_targp->bt_bdev,
			(uint64_t)da_daddr + mp->m_ddev_targp->bt_sector_offset,
			da_tmp, da_len) == 0) {
			...
			if (dirblk || leaf || inode) {
				...
				pr_warn("mxfs: P-DBLALLOC agno=%u agbno=%u len=%u new_owner_ino=%llu daddr=%lld holds=%s disk_owner=%llu magic0=%02x%02x%02x%02x off8=0x%04x tenure=%llu node=%d wasfromfl=%d comm=%s realns=%llu\n",
					...
					(unsigned)args->len,
					(unsigned long long)(args->oinfo.oi_owner ?:
						args->mxfs_diag_owner),
					(long long)da_daddr,
					dirblk ? "dir-block" :
						leaf ? "dir-leaf" : "inode",
					(unsigned long long)da_owner,
					m[0], m[1], m[2], m[3], off8,
					(unsigned long long)args->pag->ag_dlm_tenure_id,
				 ...
				 * on-disk AGF (FUA).  DIFFER => allocator ran
				 * on a STALE in-core view (READ coherence).
				 * SAME => the on-disk free-space itself lost
				 * the prior alloc (WRITE/durability or silent
				 * discard).  Anomaly-only path, cost ok.
				 */
				{
					void *ab2 = kmalloc(512, GFP_NOFS);

					if (ab2) {
						uint64_t alba2 = (uint64_t)XFS_AG_DADDR(mp,
							da_agno, XFS_AGF_DADDR(mp)) +
						...
						if (mxfs_pal_scsi_read_fua_bdev(
							...
							pr_warn("mxfs: P-DBLALLOC-AGF agno=%u incore_freeblks=%u disk_freeblks=%u differ=%d — %s\n",
								...
								dfree2,
								(args->pag->pagf_freeblks != dfree2) ? 1 : 0,
								(args->pag->pagf_freeblks != dfree2) ?
								"in-core AGF STALE = READ coherence" :
								"in-core==disk = WRITE/durability loss");
						}
						kfree(ab2);
					}
				}
	...
	/*
	 * P33-INSTR: dump bnobt leaf state to track when content goes from
	 * post-alloc to pre-alloc state.  Scoped to AG=0 (the failing AG in
	 * captured corruption events).  Uses a fresh bnobt cursor on the
	 * same trans; the leaf buf is already in cache (we just allocated
	 * out of it), so this should not trigger any reads.
	 */
	if (({ extern int mxfs_instr_enabled; mxfs_instr_enabled; }) &&
	    args->len > 0 && args->pag && args->agbp &&
	    pag_agno(args->pag) == 0) {
		struct xfs_btree_cur *p33_cur;
		int p33_stat = 0;
		int p33_err;

		p33_cur = xfs_bnobt_init_cursor(mp, args->tp, args->agbp,
						args->pag);
		if (p33_cur) {
			p33_err = xfs_alloc_lookup_ge(p33_cur, 0, 0, &p33_stat);
			if (!p33_err && p33_stat) {
				struct xfs_buf *p33_bp = NULL;
				struct xfs_btree_block *p33_blk =
					xfs_btree_get_block(p33_cur, 0,
							    &p33_bp);
				if (p33_blk && p33_bp) {
					__u16 nr = be16_to_cpu(
						p33_blk->bb_numrecs);
			...
			xfs_btree_del_cursor(p33_cur,
				p33_err ? XFS_BTREE_ERROR :
					  XFS_BTREE_NOERROR);
		}
	}
...
out_drop_perag:
	/*
	 * MXFS: defer the per-AG DLM unlock to xfs_trans_free.  Releasing
	 * here (before the trans commits) lets a peer ACQ-FRESH and read
	 * pre-allocation disk content, double-allocating the same extent.
	 * The XFS comment near the head of this function explicitly notes
	 * the AGF cannot be released until trans commit; the DLM AG lock —
	 * which serializes peer access to AG metadata — must follow the
	 * same lifetime.  Error / no-agbp paths above release immediately
	 * via mxfs_ag_dlm_unlock since no AG-meta modification happened.
	 */
	if (args->agbp && args->pag)
	...
	bool				skip_discard)
{
	...
	/*
	 * MXFS: defer the per-AG DLM unlock to xfs_trans_free.  Mirrors the
	 * fix at xfs_alloc_vextent_finish — releasing the DLM here (before
	 * trans commit) lets a peer ACQ-FRESH and read pre-free disk content,
	 * so the peer's bnobt view is inconsistent with ours and a later free
	 * trips the "ltbno + ltlen > bno" verifier (priority-2 corruption
	 * recurrence captured in run-1 of v0.3.19 stress at iter 5).
	 */
