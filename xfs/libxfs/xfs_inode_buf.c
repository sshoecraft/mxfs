	...
	bool		readahead)
{
	...
	for (i = 0; i < ni; i++) {
		...
		if (unlikely(!di_ok ||
			 ...
			 * caller path) + the daddr/slot, multi-node only, once
			 * per few, BEFORE the shutdown.  Whether it is xfsaild
			 * (stale-inode flush), inodegc (inactivation), or iget
			 * decides where the real fix lands.
			 */
			if (mp->m_mxfs_dlm &&
 ...
 * a peer modified the dir inode (per-inode DLM) without invalidating this node's
 * per-CLUSTER buffer; when this node later flushes a co-resident child inode it
 * writes the WHOLE cluster, reverting the dir on disk.
 *
 * This logs, on EVERY inode-cluster buffer WRITE, the {ino, sf entry count,
 * first name} of every S_ISDIR + FMT_LOCAL dinode in the buffer.  Catch the dir
 * inode being written with count=1 AFTER the timeline shows it reached count=4
 * → proves the stale-cluster revert + names the writing node (its dmesg).
 */
static void
mxfs_inode_buf_write_dirlog(
	struct xfs_buf	*bp)
{
	struct xfs_mount	*mp = bp->b_mount;
	 ...
	 * in-core reload reversion); a dir written sub-max NOT in this mask is
	 * a FOREIGN slot the merge already overlaid from disk -> if it is still
	 * sub-max the DISK itself was already stale (H-B: chain corruption).
	 */
	if (ni > 0 && ni <= 64) {
	 ...
	 * cannot do this.  Catch any cluster WRITE whose in-core image already
	 * carries the mixed signature (>=1 allocated slot AND >=1 zero-magic
	 * slot) OR a SHORT buffer (ni < inodes_per_cluster) — naming WHO submits
	 * the half-zero cluster and via which path (stack), BEFORE verify errors.
	 */
	if (ni > 0 && ni <= 64) {
		uint64_t	zero_mask = 0, alloc_mask = 0;
		int		full_ni = M_IGEO(mp)->inodes_per_cluster;

		for (i = 0; i < ni; i++) {
			struct xfs_dinode *d =
				xfs_buf_offset(bp, i << inodelog);
			uint16_t m = be16_to_cpu(d->di_magic);
		...
		if ((zero_mask && alloc_mask) || ni < full_ni) {
	...
	for (i = 0; i < ni; i++) {
		...
		if (!S_ISDIR(mode))
			continue;
		if (XFS_DFORK_FORMAT(dip, XFS_DATA_FORK) != XFS_DINODE_FMT_LOCAL)
			continue;

		ino = XFS_AGINO_TO_INO(mp, agno, first_agino + i);
		sfp = (struct xfs_dir2_sf_hdr *)XFS_DFORK_DPTR(dip);
		sfep = xfs_dir2_sf_firstentry(sfp);
		nl = sfp->count ? sfep->namelen : 0;
		if (nl > 24)
			nl = 24;
		/*
		 ...
		 * clusters have no SF dir, or dirs like "test_cv_disc" /
		 * ".mxfs_barriers").  The revert write keeps node2 as its first
		 * (surviving) entry, so the filter still catches count=1 reverts.
		 */
...
xfs_inode_buf_write_verify(
	struct xfs_buf	*bp)
{
	mxfs_inode_buf_write_dirlog(bp);
	xfs_inode_buf_verify(bp, false);
}
	...
	struct xfs_dinode	*from)
{
	...
	if (unlikely(from->di_version == 1)) {
		 ...
		 * counter, so the first dec-below-real event is attributable.
		 * Capped, edge-only.
		 */
		{
			unsigned int p9_old = inode->i_nlink;
			unsigned int p9_new = be32_to_cpu(from->di_nlink);
	 ...
	 * rolls it back — the leaf block keeps its higher-block refs while the
	 * data block + extent/size are dropped => P26-DSCAN-MISS leaf-vs-data
	 * tear.  At this point ip->i_df still holds the live (pre-adopt) fork
	 * and ip->i_disk_size the live size.  iget cache-miss has a fresh
	 * struct (old size 0) so never fires.  Capped, always-on.
	 */
	{
		long long p33_old = (long long)ip->i_disk_size;
		long long p33_new = (long long)be64_to_cpu(from->di_size);
		uint64_t p33_oldnx = (uint64_t)ip->i_df.if_nextents;
		uint64_t p33_fl2 = be64_to_cpu(from->di_flags2);
		uint64_t p33_newnx = (p33_fl2 & XFS_DIFLAG2_NREXT64) ?
			be64_to_cpu(from->di_big_nextents) :
			be32_to_cpu(from->di_nextents);
		bool p33_size_shrink = (p33_old > 4096 && p33_new < p33_old);
		bool p33_nx_shrink = (p33_oldnx >= 2 && p33_newnx < p33_oldnx);

		if (S_ISDIR(inode->i_mode) &&
		    (p33_size_shrink || p33_nx_shrink)) {
			...
			if (atomic_inc_return(&p33fd) <= 80)
				pr_warn("mxfs: P33-FROMDISK-DIRSHRINK ino=%llu old_size=%lld new_size=%lld old_nx=%llu new_nx=%llu old_fmt=%u new_fmt=%u disk_gen=%u sz_shrink=%d nx_shrink=%d comm=%s — dir data-fork REVERTED smaller (leaf-vs-data tear source)\n",
					...
					(unsigned long long)p33_newnx,
					ip->i_df.if_format,
					from->di_format,
					be32_to_cpu(from->di_gen),
					p33_size_shrink, p33_nx_shrink,
					current->comm);
		}
	}
	...
	xfs_lsn_t		lsn)
{
	 ...
	 * SMALLER than what is already on disk (and on-disk was >1 block),
	 * this write reverts the dir's data fork — leaf-vs-data tear written
	 * durably.  Capped, always-on (S_ISDIR on the in-core mode).
	 */
	if (S_ISDIR(inode->i_mode)) {
		long long p33_old = (long long)be64_to_cpu(to->di_size);
		long long p33_new = (long long)ip->i_disk_size;

		if (p33_old > 4096 && p33_new < p33_old &&
		    be16_to_cpu(to->di_mode) == inode->i_mode) {
	 ...
	 * stale-base RMW clobber moment, caught at destage time. */
	if (S_ISDIR(inode->i_mode) && to->di_version >= 3 &&
	    be32_to_cpu(to->di_gen) == inode->i_generation &&
	    be16_to_cpu(to->di_mode) == inode->i_mode) {
		u64 ccr_old = be64_to_cpu(to->di_changecount);
		u64 ccr_new = inode_peek_iversion(inode);
