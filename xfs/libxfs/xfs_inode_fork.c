	...
	struct xfs_ifork	*ifp)
{
	...
	/*
	 * mxfs v0.10.31: zero if_broot_bytes with if_broot.  Upstream only
	 * destroys forks on teardown, so a stale byte count is never re-read;
	 * mxfs's reload/adopt destroys and rebuilds LIVE dir forks
	 * (mxfs_dlm_reload_inode -> xfs_idestroy_fork + xfs_inode_from_disk).
	 * A dir that had converted extents->btree (if_broot_bytes = 1-rec
	 * root size) and was then reloaded from an extents-format disk image
	 * kept the stale nonzero if_broot_bytes with if_broot == NULL; the
	 * NEXT extents->btree conversion's xfs_bmap_broot_realloc(1) matched
	 * new_size == old_size (nop path), returned the NULL if_broot, and
	 * xfs_bmbt_init_block wrote the level/numrecs header into NULL+4 —
	 * the dir_reuse@16 create-path oops that killed dd IN PLACE holding
	 * dir i_rwsem + i_lock + i_dlm_ex_holders, wedging the dir on the
	 * node and starving all 15 peers (test9 2026-07-10, Oops 0002 at
	 * xfs_btree_init_block+0x36, RBX=0).
	 */
	ifp->if_broot_bytes = 0;
	...
	struct xfs_inode	*ip)
{
	...
	case S_IFDIR: {
		...
		if (unlikely(!sfp))
			mxfs_probe("mxfs: P171-SFNULL ino=%llu if_bytes=%d comm=%s wr_last=%pS/%d/%s rd_last=%pS/%d/%s rd_held=%d un_last=%pS\n",
				(unsigned long long)ip->i_ino,
