	...
	struct xfs_buf			**bpp)
{
	 ...
	 * agbno, not fresh block CONTENT).  Without this, the allocator walks a
	 * stale free-space btree and double-allocates -> EFSBADCRC/Bmap BTree
	 * corruption + FS shutdown under concurrent rename.  For AG btrees,
	 * invalidate a cached buffer whose b_mxfs_ag_gen lags pag_dlm_meta_gen
	 * (bumped on every fresh AG acquire) so the read below FUA-re-reads the
	 * peer's committed block.  Helper self-gates (single-node / gen==0
	 * no-op) and is deadlock-safe (XBF_TRYLOCK; skip dirty/pinned/delwri).
	 * AG btree pag is via to_perag(cur->bc_group) (the old cur->bc_ag.pag
	 * field was refactored away in this kernel).  AGF/AGI header hooks are
	 * already wired (xfs_read_agf/xfs_read_agi).
	 */
	 ...
	 * loads prior-tenure records and trips ir.loaded != if_nextents →
	 * EFSCORRUPTED shutdown (16-node dpn=100 storm).  Helper self-gates
	 * (multi-node dirs with i_dlm_dir_gen only) and is deadlock-safe.
	 */
	else if (cur->bc_ops->type == XFS_BTREE_TYPE_INODE)
		mxfs_dir_bmbt_invalidate_stale(cur->bc_ino.ip, d,
				xfs_btree_bbsize(cur));
	 ...
	 * xfsaild write a stale free-space image over a peer's durable alloc:
	 * P124-ALLOC-REVERT). An in-tenure RMW logs the block, which stamps it
	 * current via mxfs_ag_meta_track and keeps it tenure-guarded. */
