	...
	xfs_rfsblock_t		dblocks)
{
	...
	INIT_WORK(&pag->pag_dlm_bast_work, mxfs_dlm_ag_bast_work_fn);
	INIT_WORK(&pag->pag_dlm_release_work, mxfs_dlm_ag_release_work_fn);
	pag->pag_dlm_demoting = false;
	init_waitqueue_head(&pag->pag_dlm_demote_wq);
#endif /* __KERNEL__ */
