	...
	struct xfs_dir2_sf_hdr	*sfhp)		/* shortform directory hdr */
{
	 ...
	 * dropping shared dirs back into the shortform coherency machinery
	 * (in-inode dirents; base/ours/theirs merges; epoch-scoped removed
	 * sets) whose residual races mint ghost dirents under 4-node
	 * create+rename+rm churn (dlm_fairness "drained exp=0 got=N": a
	 * tenure-refused destage leaves the sf dinode lagging on disk and
	 * peers import + re-assert the ghost).  Block-format dirs keep
	 * dirents in DATA BLOCKS covered by the proven-at-4/caw buffer
	 * coherence pipeline (FUA cold reads, epoch evicts, release drains,
	 * P5R self-trans refresh).  Returning 0 without converting is the
	 * same contract as the caller's own size>fork_size early return:
	 * the dir simply stays block; the trans still owns bp.
	 */
	...
	xfs_da_args_t		*args)		/* operation arguments */
{
	 ...
	 * inode-EX popcount.  Cross-node correlation by realns discriminates:
	 *   (A) concurrent-EX  : ex_pop>1 here == two nodes hold the dir inode
	 *       EX simultaneously -> broken cluster exclusion.
	 *   (B) clobber-after-commit : ex_pop==1 but the base list is MISSING a
	 *       peer's already-committed entry -> this node built on a stale
	...
	if (({ extern int mxfs_instr_enabled; mxfs_instr_enabled; }) &&
	    ...
	    dp->i_df.if_format == XFS_DINODE_FMT_LOCAL && sfp) {
		struct xfs_dir2_sf_entry *e = xfs_dir2_sf_firstentry(sfp);
		char names[200];
		int  pos = 0, k, exn = 0, exp;

		names[0] = '\0';
		for (k = 0; k < sfp->count && pos < (int)sizeof(names) - 12; k++) {
			int nl = min_t(int, e->namelen, 10);
			pos += scnprintf(names + pos, sizeof(names) - pos,
					 "%.*s ", nl, e->name);
			e = (void *)e +
			    xfs_dir2_sf_entsize(dp->i_mount, sfp, e->namelen);
		}
		...
		mxfs_pal_log(MXFS_LOG_WARN,
			...
			exp, exn, (unsigned long long)ktime_get_real_ns());
	}
	...
	xfs_da_args_t		*args)		/* operation arguments */
{
	 ...
	 * reader holds ILOCK_SHARED here, so the writer that nulled if_data did
	 * so WITHOUT holding i_lock exclusively, or left it persistently NULL.
	 * Capture the inode's exact identity + DLM state and bail -EIO instead
	 * of dereferencing NULL, so the test continues and the timeline can be
	...
	if (unlikely(!sfp)) {
		mxfs_pal_log(MXFS_LOG_WARN,
			...
			(unsigned long long)dp->i_ino,
			dp->i_df.if_format,
			(long long)dp->i_df.if_bytes,
			(long long)dp->i_disk_size,
			dp->i_dlm_stale, dp->i_dlm_mode,
			(unsigned)VFS_I(dp)->i_generation,
			(unsigned long long)dp->i_df.if_nextents,
	...
	xfs_da_args_t		*args)
{
	 ...
	 * log state + wall clock so the cross-node timeline shows whether the
	 * remover's release published it and who re-adopted the stale image. */
	{
		extern unsigned long long mxfs_watch_ino;

		if (mxfs_watch_ino && dp->i_ino == mxfs_watch_ino &&
		    mp->m_mxfs_dlm &&
	 ...
	 * dlm_fairness 12-leftover ratchet). */
