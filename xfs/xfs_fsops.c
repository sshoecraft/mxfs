	...
	int		lnnum)
{
	 ...
	 * dir slot (CAS gen 512->130k) and one of them WON the dir EX 6
	 * minutes post-shutdown, starving the surviving nodes into their own
	 * timeouts.  The withdraw fences acquires and stops our disklock
	 * heartbeat so peers' dead-node purge reclaims whatever we hold.
	 */
	if (mp->m_mxfs_dlm)
		mxfs_dlm_shutdown_withdraw(mp);
