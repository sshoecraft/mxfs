...
/*
 * __assign_str changed from 2-arg to 1-arg in kernel 6.10.
 * Provide a compat wrapper that accepts 2 args on all kernels.
 */
#if LINUX_VERSION_CODE < KERNEL_VERSION(6, 10, 0)
#define mxfs_assign_str(dst, src) __assign_str(dst, src)
#else
#define mxfs_assign_str(dst, src) __assign_str(dst)
#endif
...
TRACE_EVENT(xfs_alloc_cur_check,
	...
	TP_fast_assign(
		...
		mxfs_assign_str(name, cur->bc_ops->name);
		__entry->bno = bno;
		__entry->len = len;
		__entry->diff = diff;
		__entry->new = new;
...
DECLARE_EVENT_CLASS(xfs_defer_pending_class,
	...
	TP_fast_assign(
		...
		mxfs_assign_str(name, dfp->dfp_ops->name);
...
DECLARE_EVENT_CLASS(xfbtree_freesp_class,
	...
	TP_fast_assign(
		__entry->xfino = file_inode(xfbt->target->bt_file)->i_ino;
		mxfs_assign_str(btname, cur->bc_ops->name);
		__entry->nlevels = cur->bc_nlevels;
		__entry->fileoff = fileoff;
	),
...
DECLARE_EVENT_CLASS(xfs_getparents_rec_class,
	...
	TP_fast_assign(
		...
		mxfs_assign_str(name, pptr->gpr_name);
	),
...
DECLARE_EVENT_CLASS(xfs_metadir_update_class,
	...
	TP_fast_assign(
		...
		mxfs_assign_str(fname, upd->path);
	),
