	...
	struct xfs_btree_cur		**state)
{
	...
	/*
	 * MXFS: a peer-held AG times out the blocking AG-DLM acquire at the
	 * head of __xfs_free_extent (-ETIMEDOUT after the 120s CAW poll).
	 * That is cross-node contention, not corruption — under cluster-wide
	 * I/O saturation an EX holder's release drain legitimately takes
	 * minutes (32-node cawd fio_perf collapse, 2026-07-18: eight nodes
	 * shut down because xfs_defer_finish_noroll treats any non-EAGAIN
	 * error as fatal).  The extent has not been touched, so convert to
	 * the existing -EAGAIN requeue (re-log intent, roll, retry) — each
	 * retry re-registers this node as a CAW waiter, which is what BASTs
	 * the holder into releasing.  Bounded by mxfs_efi_agwait_max so a
	 * genuine cross-node deadlock still surfaces as today's shutdown
	 * instead of livelocking forever.
	 */
