	...
	struct xfs_trans	*tp)
{
	...
	/*
	 * MXFS Approach A: clear the per-task active-trans pointer if it's
	 * us, BEFORE the inode-DLM drain.  Drain calls mxfs_dlm_bast_process
	 * which may indirectly re-enter ilock paths; those must see no trans
	 * context (this trans is now being torn down) and fire inline rather
	 * than re-queue onto a tp that's mid-free.  The published-trans
	 * invariant: current->journal_info either points to a still-allocated
	 * outer/dup'd trans, or is NULL.  See __xfs_trans_alloc / xfs_trans_dup
	 * for the publish/handoff side.
	 */
	if (current->journal_info == tp)
		current->journal_info = NULL;

	/*
	 * MXFS: drain any deferred AG DLM unlocks queued by the alloc / ialloc
	 * paths.  Releasing the DLM here — after xlog_cil_commit on the
	 * success path or with no commit on the cancel path — guarantees the
	 * AG-meta modifications are at least in CIL (success) or never
	 * happened (cancel) before any peer can ACQ-FRESH and read disk
	 * contents.  The earlier release-at-vextent-finish let peers read
	 * pre-allocation disk state and double-allocate the same extent.
	 */
	...
	struct xfs_trans	*tp)
{
	 ...
	 * tp is about to be freed by the caller (xfs_trans_roll); ntp
	 * continues the chain.  Without the handoff, tp's free would clear
	 * current->journal_info even though ntp is still active, causing
	 * subsequent ilock_end calls in the rolled trans to fire bast
	 * inline instead of deferring.
	 */
	if (current->journal_info == tp)
	 ...
	 * bnobt/agf state, and release before our new tp re-acquires.  When
	 * the new tp's defer iteration processes a pending EFI from a prior
	 * sub-trans, the EFI's (bno,len) may now overlap blocks the peer
	 * already freed → "ltbno+ltlen > bno" corruption (line 2106).
	 *
	 * The defer chain semantically owns the AG-DLM grant for its entire
	 * lifetime, mirroring how xfs holds the AGF buf across rolls.
	 * Splicing here keeps the grant alive until the FINAL tp's trans_free.
	 */
	/*
	 * Migrate ONLY the AG-DLM unlock pendings.  The inode-DLM unlock
	 * pendings deliberately stay on the OLD tp so they drain at the
	 * old tp's trans_free (i.e., on every roll boundary).  Migrating
	 * inode unlocks across rolls delays peer-side ilock BAST processing
	 ...
	 * release at this tp's trans_free, so a dirty defer chain no
	 * longer holds every AG it ever touched while blocking on the
	 * next one (run96/97 cross-node AG convoy: P1-AGWAIT ag=0
	 * trans_held_ags=[4] → rc=-110 → error-path shutdown).
	 */
	mxfs_trans_migrate_ag_unlocks(tp, ntp);
	...
	uint			flags)
{
	 ...
	 * mxfs_dlm_ilock_end can defer bast_process until xfs_trans_free.
	 * XFS does not use current->journal_info; we repurpose it.
	 *
	 * Inner trans (current->journal_info already non-NULL) skip
	 * publication — the outer trans's drain runs LATER and will
	 * naturally cover any defers made between alloc/free of the
	 * inner trans.  This is degraded behavior but correct: the
	 * inner trans's free will not fire its own drain (its list is
	 * empty since defers all went to the outer's list).
	 *
	 * Conflict with another fs setting journal_info: degrades to
	 * inline bast (existing pre-Approach-A behavior).  XFS-only
	 * builds (the typical MXFS case) won't see this.
	 */
