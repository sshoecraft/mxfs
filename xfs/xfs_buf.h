 ...
 */
struct xfs_buf;
struct mxfs_icwr_entry;		/* step-5 F3: private to xfs_mxfs_dlm.c */
...
/*
 * MXFS v6a phase 1 (v0.3.129): freshness gate for the
...
/*
 * MXFS (v0.3.148): set on cluster bufs that have been added to
 ...
 * stale (iter 3 corruption signature
 ...
 * pinned; NOT the /76 readahead leak — XBF_READ_AHEAD is clear).  Exactly
...
struct xfs_buf {
	...
	uint32_t		b_mxfs_dir_gen;	/* dp->i_dlm_dir_gen stamp at last fresh dir-block read; read-time block-dir coherency */
	uint32_t		b_mxfs_coherent_gen;	/* dp->i_dlm_dir_gen at last PLATTER-VERIFIED addname coherence check (mxfs_dir_addname_coherent_refresh). Distinct from b_mxfs_dir_gen (stamped on EVERY read, incl stale reads) — only the addname guard sets this, so it dedups the per-addname FUA platter check to ONCE per block per dir_gen (cheap, the budget rule). 0 = never verified -> check. */
	uint32_t		b_mxfs_dir_incarn;	/* VFS i_generation stamp at last fresh dir-block read/init for the owning dir inode; a cached buffer whose stamp != the reading inode's current i_generation is a PREVIOUS-incarnation ABA alias of a reused inode# at a reused daddr (its in-AIL "undestaged" content belongs to the freed prior incarnation) -> invalidate+re-read even past the dirty/in-AIL guard. 0 = never stamped (treat as needs-fresh-read). */
	uint32_t		b_mxfs_logged_seq;	/* run14d: bumped at every xfs_trans_log_buf; node-LOCAL modification counter */
	uint32_t		b_mxfs_written_seq;	/* run14d: snapshot of b_mxfs_logged_seq at write submit; logged != written => committed-unwritten LOCAL mods (cross-node-safe destage test — payload LSNs are stamped by whichever node last wrote the block, so LSN compares across journals are meaningless) */
	u64			b_mxfs_wr_flush_epoch;	/* mount m_mxfs_flush_epoch snapshot at this buffer's last successful WRITE completion.  == current epoch => no device flush since this content was written => the platter may be BEHIND this buffer (target write cache) => never regress it from a platter/FUA read (xfs_iread_bmbt_block P34B guard).  0 = never written by this node (mount epoch starts at 1). */
	u64			b_mxfs_ag_gen;	/* pag->pag_dlm_meta_gen stamp at last fresh AG-meta read; read-time AG free-space coherency (anti double-alloc) */
	u64			b_tenure_id;	/* pag->ag_dlm_tenure_id stamp at last AG-meta read under the current AG-DLM hold; == current => this-node-authoritative, NEVER discard mid-tenure (replaces failed gen/LSN preserve-vs-discard heuristics for the AGI unlinked-list lost-update) */
	uint32_t		b_mxfs_grant_gen;	/* owning dir inode's i_dlm_cached_grant_gen (acked-TCP per-grant token) at last COHERENT read of this dir block. design review: freshness must mean "read under the CURRENT grant", NOT the lossy i_dlm_dir_gen (stamped fresh over stale content -> the 4/tcp dir_reuse stale_base=0 durable clobber). An EX RMW whose target block's b_mxfs_grant_gen != the inode's current grant gen was last read under an OLDER grant (we lost+reacquired EX since = a peer modified) -> re-read before the RMW. 0 = never stamped. */
	uint32_t		b_mxfs_dir_epoch;	/* design review tenure model: owning dir inode's i_dlm_dir_valid_epoch (reliable level-triggered cross-node handoff epoch) at last COHERENT read of this dir block. If b_mxfs_dir_epoch < dp->i_dlm_dir_valid_epoch, this block was read under an EARLIER tenure than the inode now knows coherent (a peer was granted + modified the LUN since) -> the cached payload is a STALE RMW base -> re-read before use, OVERRIDING the payload-LSN undestaged keep-guard (epoch only advances after WE released EX, whose work Invariant-1 drained durable -> nothing un-drained to resurrect). 0 = never handed off (single-node / never-BAST'd) -> never stale. */
	u64			b_mxfs_idirty_mask;	/* PERSISTENT per-sector mask of inode-cluster sectors THIS node has logged (modified) in this buffer incarnation. mxfs_submit_partial_inode_write accumulates currently-logged sectors here and writes the UNION, so a freshly-allocated inode whose log item already detached (but whose sector was never whole-written) is still written -> fixes the dir_reuse durable inode-alloc REVERT. A peer inode this node only READ is never logged -> never in the mask -> never written -> false-sharing still protected. Cleared on xfs_buf_stale (buffer invalidation/reuse) so an ABA-reused buffer does not carry a stale mask. */
	bool			b_mxfs_dir_wr_counted;	/* (design review writeback-completion-barrier): this dir DATA/leaf write bio was counted into mp->m_mxfs_dir_wr_inflight at submit; the matching __xfs_buf_ioend decrements + wakes exactly once.  The dir EX release fence waits for the counter to reach 0 so NO prior-tenure stale dir-block write can land after the next holder begins (root of dir_reuse readdir=799 single-dirent durable loss). */
	u8			b_mxfs_io_tokens;	/* /455 (0.61.0, D4): departure tokens this buffer holds — one per submission ADMITTED by xfs_buf_submit_bio, retired one per TERMINAL completion (the end of __xfs_buf_ioend, or the transient-error release exit of xfs_buf_ioend_handle_error), never at the retry decision, all under b_mxfs_acct->lock (struct mxfs_depart_acct, xfs_mount.h). */
	u8			b_mxfs_io_rejected;	/* submissions of this buffer REJECTED after the departure freeze (or on token overflow) whose -EIO completion has not yet run — they took no token, and their completion is discounted here (and in acct->rejected_pending) before any token is retired, so a rejected generation can never retire an admitted generation's token. */
	bool			b_mxfs_io_carry;	/* set at xfs_buf_ioend_handle_error's resubmit: the retry KEEPS the generation's token (no retire, no re-take); consumed by the next mxfs_depart_token_take. */
	bool			b_mxfs_io_soft;		/* set by xfs_buf_ioend_fail_unsubmitted — this completion is an AUDITED no-I/O software completion (never entered xfs_buf_submit_ex); cleared at retire.  A completion with no token, no rejection and no soft mark is UNTOKENED and fails the departure closed. */
	 ...
	 * (design-consult ruling item 2).  submit copies committed at bio
	...
	/*
	 * D-FOREIGN-SHADOW-UNWIND-HOST-SHUTDOWN-513B: this buffer's
	 ...
	 * (design-consult ruling).  Set at every pass-2 delwri-queue/bwrite site
	...
	/*
	 * lock-holder tracking (design review design-consult instrumentation): the
...
/* the audited pre-submission failure completion (no I/O was ever
...
/* departure-gate fault injectors (module param dbg_depart_inject,
...
/* DIAG: one-line buffer state + event ring (see pal/linux/xfs_buf.c). */
