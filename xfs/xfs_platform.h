...
/* MXFS_HAVE_<API>: what the kernel this build is for provides, probed from its
 * own headers at build time (pal/linux/kcompat_probe.sh, generated by Kbuild).
 * A shim gated on one of these is defined only where the kernel lacks the API,
 * whatever its version number says. */
#include "../pal/linux/mxfs_kcompat.h"
...
/* A dir_context actor returns bool from v6.1 (true: keep going) and int
 * before it (0: keep going, anything else: the buffer is full); RHEL 9 keeps
 * the int form.  MXFS's own actor returns through these and reads a wrapped
 * actor's answer through mxfs_filldir_ok(), so the two meanings cannot be
 * crossed: under the int form, "!actor()" is true for every entry that fit. */
#ifdef MXFS_HAVE_FILLDIR_BOOL
typedef bool mxfs_filldir_ret_t;
#define MXFS_FILLDIR_CONTINUE	true
#define MXFS_FILLDIR_STOP	false
#define mxfs_filldir_ok(ret)	(ret)
#else
typedef int mxfs_filldir_ret_t;
#define MXFS_FILLDIR_CONTINUE	0
#define MXFS_FILLDIR_STOP	(-EINVAL)
#define mxfs_filldir_ok(ret)	((ret) == 0)
#endif

/* super_set_uuid added in v6.9; RHEL 9.8 carries it on 5.14 */
#ifndef MXFS_HAVE_SUPER_SET_UUID
...
/* bdev file API changed in v6.9: bdev_handle → struct file (RHEL 9.8 has the
 * file API on 5.14) */
#ifndef MXFS_HAVE_BDEV_FILE_OPEN
 ...
 * (D-BIO-VMALLOC-CHUNK-COMPAT-INVERTED-RUNAWAY-528).  Each is gated on its own
 * probe: a kernel may carry some of the three and not the others. */
#include <linux/bio.h>
#ifndef MXFS_HAVE_BIO_ADD_VMALLOC_CHUNK
...
#endif
#ifndef MXFS_HAVE_BIO_ADD_VMALLOC
...
#endif
#ifndef MXFS_HAVE_BIO_ADD_VIRT_NOFAIL
...
/* memtostr_pad — new in v6.10, a macro, so tested by name (RHEL 9.8 has it) */
#ifndef memtostr_pad
...
/* bdev atomic write helpers — upstream added them one at a time:
 * bdev_can_atomic_write v6.11, bdev_atomic_write_unit_{min,max}_bytes
 * v6.13, bdev_validate_blocksize v6.15, bio_add_max_vecs v6.16.  A
 * distribution kernel can carry any subset (RHEL 9.8 has all but
 * bdev_validate_blocksize), so each is gated on its own probe. */
#ifndef MXFS_HAVE_BDEV_CAN_ATOMIC_WRITE
static inline bool bdev_can_atomic_write(struct block_device *bdev) { return false; }
#endif
#ifndef MXFS_HAVE_BDEV_ATOMIC_WRITE_UNIT_MIN_BYTES
static inline unsigned int bdev_atomic_write_unit_min_bytes(struct block_device *bdev) { return 0; }
#endif
#ifndef MXFS_HAVE_BDEV_ATOMIC_WRITE_UNIT_MAX_BYTES
static inline unsigned int bdev_atomic_write_unit_max_bytes(struct block_device *bdev) { return 0; }
#endif
#ifndef MXFS_HAVE_BDEV_VALIDATE_BLOCKSIZE
static inline int bdev_validate_blocksize(struct block_device *bdev, int block_size) { return 0; }
#endif
#ifndef MXFS_HAVE_BIO_ADD_MAX_VECS
...
#ifndef MXFS_HAVE_SUPER_SET_SYSFS_NAME_ID
...
/* generic_fill_statx_atomic_writes — new in v6.11 with (stat, unit_min,
 * unit_max); v6.16 added unit_max_opt, which MXFS passes.  A kernel with only
 * the three-argument form (RHEL 9.8) has no field for the fourth, so it is
 * dropped there; a kernel with neither reports no atomic write limits. */
#if defined(MXFS_HAVE_GENERIC_FILL_STATX_ATOMIC_WRITES_4)
/* the kernel's own, as called */
#elif defined(MXFS_HAVE_GENERIC_FILL_STATX_ATOMIC_WRITES_3)
#define generic_fill_statx_atomic_writes(stat, unit_min, unit_max, unit_max_opt) \
...
/* iomap constants and types — new in 6.15+.  Tested after the kernel's own
 * header, never before it: RHEL 9.8 defines IOMAP_F_BOUNDARY there, and a 0
 * fallback defined first would stand in every file that reads the flag before
 * including linux/iomap.h. */
 ...
 * RHEL 9.8 has both: the first as a static inline in linux/iomap.h, the second
 * exported.  Each is gated on its own probe, and MXFS's own delalloc release
 * (below) stands in only where no kernel export exists. */
#ifndef MXFS_HAVE_IOMAP_LAST_WRITTEN_BLOCK
...
#endif
#if LINUX_VERSION_CODE < KERNEL_VERSION(6, 12, 0) && \
    !defined(MXFS_HAVE_IOMAP_WRITE_DELALLOC_RELEASE)
/*
...
/* iomap_dio_rw gained a `private` argument, handed to the iomap callbacks as
 * iomap_iter->private; RHEL 9 has neither.  MXFS passes a non-NULL private only
 * for a zoned write, and a zoned filesystem is refused at mount on a kernel
 * without them (xfs_super.c), so dropping it here drops only NULL. */
#ifdef MXFS_HAVE_IOMAP_DIO_RW_PRIVATE
#define mxfs_iomap_dio_rw(iocb, iter, ops, dops, flags, private, done_before) \
	iomap_dio_rw(iocb, iter, ops, dops, flags, private, done_before)
#else
#define mxfs_iomap_dio_rw(iocb, iter, ops, dops, flags, private, done_before) \
	iomap_dio_rw(iocb, iter, ops, dops, flags, done_before)
#endif
#ifdef MXFS_HAVE_IOMAP_ITER_PRIVATE
#define mxfs_iomap_iter_private(iter)	((iter)->private)
#else
#define mxfs_iomap_iter_private(iter)	((void)(iter), NULL)
#endif
...
/* A dio can allocate its bios from iomap_ioend_bioset only where
 * iomap_dio_ops has a bio_set member and the kernel exports the set, which
 * iomap initializes at boot.  There is no stand-in: a set nobody has
 * bioset_init()ed has no mempool allocator, and the first bio_alloc_bioset
 * from it crashes the kernel.  Where this is unset (6.8, RHEL 9) nothing that
 * needs the set is installed. */
#if defined(MXFS_HAVE_IOMAP_DIO_BIO_SET) && defined(MXFS_HAVE_IOMAP_IOEND_BIOSET)
