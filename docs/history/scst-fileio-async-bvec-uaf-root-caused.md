<!-- sess137 ROOT CAUSE of the stage-(ii) fileio wedge: SCST fileio_exec_async() kfree()s the in-flight bvec at -EIOCBQUEUED - a UAF that GPF'd the loop0… -->
...
GPT design-consult ruling on contamination (gpt-5.6-sol): **"Treat previous unexplained
...
- Test target `iqn.2026-08.mxfs.fence:inflightf` — 6 stale SCST sessions, 4
  ...
  all SCST management (see docs/history/docs/history/compiled-scst-iscsi-infra-wedge-recovery.md).
- the never-reboot-clyde rule honoured: clyde not rebooted, no reboot proposed.
...
3. Reloading SCST is NOT a host reboot (the never-reboot-clyde rule permits it) but drops the 32-node
