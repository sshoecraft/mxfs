<!-- sess30-33 dir_reuse 2/tcp: FACE-C data-over-inode-cluster double-ownership root, FACE-B fix (5E706CBF), SB-counter incoherence lead. -->
...
Per `docs/history/docs/history/compiled-sess30-33-dir-reuse-ag-inode-cluster-double-alloc.md` and
`docs/history/docs/history/compiled-sess30-33-dir-reuse-ag-inode-cluster-double-alloc.md`, all present in baseline AB435ACC:
...
- **FACE C (INODE-CLUSTER CORRUPTION → SHUTDOWN, severest, ~1/3 runs)**:
  ...
  the proven root (`docs/history/docs/history/compiled-sess30-33-dir-reuse-ag-inode-cluster-double-alloc.md`).
...
- **M1-cross-node allocator double-alloc: NOT fully ruled out.** The only
  ...
  (the derived-budget rule, 300s no-result) → REVERTED. A cheap variant is still owed.
- **M2-stale extent-map mis-WRITE: leading theory.** A node's in-core dir/file
  ...
  prevent FACE C. `docs/history/docs/history/compiled-sess30-33-dir-reuse-ag-inode-cluster-double-alloc.md` aligns this with
  the 16-node `docs/history/docs/history/compiled-sess30-33-dir-reuse-ag-inode-cluster-double-alloc.md` conclusion.

### sess33 decisive evidence favoring M2 (chk_mxfs on frozen corrupt image)
`docs/history/docs/history/compiled-sess30-33-dir-reuse-ag-inode-cluster-double-alloc.md`: after a corrupting run
...
`docs/history/docs/history/compiled-sess30-33-dir-reuse-ag-inode-cluster-double-alloc.md`: `P62-RELOAD-FORK-SHRINK
...
`docs/history/docs/history/compiled-sess30-33-dir-reuse-ag-inode-cluster-double-alloc.md`: FACE B (inode-revert /
...
`docs/history/docs/history/compiled-sess30-33-dir-reuse-ag-inode-cluster-double-alloc.md`: after 5E706CBF, every run
...
## GPT-5.5 consults (the design-consult rule) and what they refuted
`docs/history/docs/history/compiled-sess30-33-dir-reuse-ag-inode-cluster-double-alloc.md`:
...
`docs/history/docs/history/compiled-sess30-33-dir-reuse-ag-inode-cluster-double-alloc.md`: a later GPT consult
...
`docs/history/docs/history/compiled-sess30-33-dir-reuse-ag-inode-cluster-double-alloc.md`: GPT-5.5 2nd consult verdict — the
...
`docs/history/docs/history/compiled-sess30-33-dir-reuse-ag-inode-cluster-double-alloc.md`: GPT-5.5 #1 gap = dir-grow
...
- **Acquire-side pin-drain EXTENSION** (mxfs_dir_drain_evict_data_blocks loop
  ...
  BEFORE cold-reading. Reverted. (`docs/history/docs/history/compiled-sess30-33-dir-reuse-ag-inode-cluster-double-alloc.md`)
...
- **publish-at-create** (`mxfs_dlm_publish_inode(du.ip)` in xfs_create): SHUT DOWN
  ...
  (`docs/history/docs/history/compiled-sess30-33-dir-reuse-ag-inode-cluster-double-alloc.md`, `docs/history/docs/history/compiled-sess30-33-dir-reuse-ag-inode-cluster-double-alloc.md`)
...
- **Per-alloc on-disk di_magic check (P33-ALLOC-OVER-DISKINODE)**: correct but
  ~19k sync reads holding AGF → 300s timeout (the derived-budget rule). Reverted; need a cheap
  variant.

## sess32 PROVEN reframe — AG free-space double-alloc on cross-node dir grow
`docs/history/docs/history/compiled-sess30-33-dir-reuse-ag-inode-cluster-double-alloc.md` (supersedes earlier sess32
...
`docs/history/docs/history/docs/history/compiled-sess30-33-dir-reuse-ag-inode-cluster-double-alloc.md`:
...
- **dmesg ring buffer WRAPS during a 24-round run** (`docs/history/docs/history/compiled-sess30-33-dir-reuse-ag-inode-cluster-double-alloc.md`).
...
- **instr=1 is 100× slower and HIDES the race** — run dir_reuse WITHOUT
  instr/dirwr modargs to preserve it (`docs/history/docs/history/compiled-sess30-33-dir-reuse-ag-inode-cluster-double-alloc.md`).
- **the derived-budget rule timeout trap**: per-alloc disk reads time out the test → REVERTED.
...
- **tests/reset2.sh before EVERY run** (`docs/history/docs/history/compiled-sess30-33-dir-reuse-ag-inode-cluster-double-alloc.md`):
  ...
  virsh-destroy+start both VMs (host clyde reboot is BANNED per the never-reboot-clyde rule, VM reboot
...
- **01E60C6E** — sess30 relay = AB435ACC + un-gated capped P-DBLALLOC detector
  only (log-only, xfs_alloc.c ~4214). (`docs/history/docs/history/compiled-sess30-33-dir-reuse-ag-inode-cluster-double-alloc.md`)
