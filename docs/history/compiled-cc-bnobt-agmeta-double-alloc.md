<!-- Compiled sess21-111: bnobt/AG-meta double-free + free-space double-alloc family; ended as RED HERRING (real root = stale inode/BMAP). -->
...
`docs/history/docs/history/docs/history/compiled-cc-bnobt-agmeta-double-alloc.md`.

### sess41-42 — false-fresh gen-stamp → free-space double-alloc FIXED (v5 fork)
`docs/history/docs/history/compiled-cc-bnobt-agmeta-double-alloc.md`: btree-block AG read-coherence hook added in `xfs_btree_read_buf_block()`
...
`docs/history/docs/history/compiled-cc-bnobt-agmeta-double-alloc.md`: **FIXED** the false-fresh gen-stamp bug (build `C6970FF9`). Winning the instrument-first loop method:
...
`docs/history/docs/history/compiled-cc-bnobt-agmeta-double-alloc.md` (the largest source). **PROVEN FIX build `BB54A138`:**
...
`docs/history/docs/history/compiled-cc-bnobt-agmeta-double-alloc.md`: `A0A86F31` was clean only at idle; under real 4-node `cache_coherency` it produced
...
`docs/history/session-46-lessons.md` (build `9AF61DF2`, later `0FA2CB16`): the ACQUIRER (freeing node) writes a PRISTINE
...
`docs/history/docs/history/compiled-cc-bnobt-agmeta-double-alloc.md` (build `29977E5D`): the 7-session `ltbno+ltlen>bno` corruption FIXED. NOT any
...
`docs/history/docs/history/compiled-cc-bnobt-agmeta-double-alloc.md` (build `BDC9BB93`, `mxfs_dlm_caw_ex_count` detector KEEP). Direct on-disk proof of
...
`docs/history/docs/history/docs/history/compiled-cc-bnobt-agmeta-double-alloc.md`: earlier bnobt-FUA hypothesis in that same file is REFUTED.
...
`docs/history/docs/history/compiled-cc-bnobt-agmeta-double-alloc.md` (build `D8F43418`, P81-DEXT probe). Isolated `unlink_visibility` DETERMINISTICALLY
...
`docs/history/docs/history/compiled-cc-bnobt-agmeta-double-alloc.md` (build `A18B3029`; Probe-A `0BC933EE`). P47 `DISK-LIVE-same-gen=>A-lost-removal` on
...
never set true). Gemini the design-consult rule diagnosis: **post-release xfsaild clobber via CIL→AIL pipeline race** —
...
`docs/history/docs/history/compiled-cc-bnobt-agmeta-double-alloc.md` (baseline `ECDE1FC5`). PROVED the bnobt double-free is a cross-node alloc/free +
**IN-CORE revert of the bnobt buffer from split→pristine BEFORE the release drain** — NOT a
drain-enumeration gap, NOT a storage-persistence failure. Refutations (the instrument-first loop): (1) FIX1 FUA-skip guard
...
`docs/history/docs/history/compiled-cc-bnobt-agmeta-double-alloc.md` (build `C4E65691`). Decoded the full P88 chronological trace for one leaf (AG9
...
`docs/history/docs/history/compiled-cc-bnobt-agmeta-double-alloc.md` + `docs/history/docs/history/compiled-cc-bnobt-agmeta-double-alloc.md` (builds `89C04201`, `87726318`). On a CLEAN cluster
...
`docs/history/docs/history/compiled-cc-bnobt-agmeta-double-alloc.md` (build `87726318`). Overrides sess110's framing: sess110 and
...
`docs/history/docs/history/docs/history/compiled-cc-bnobt-agmeta-double-alloc.md` — the OLD mxfs.1 (non-XFS-fork)
