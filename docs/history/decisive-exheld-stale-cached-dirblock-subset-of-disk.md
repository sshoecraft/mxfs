<!-- sess25: P25-RELVERIFY probe shows dir EX release mismatches (incore<disk) are ALL done=0 (benign lazy-evict, refetch on access) — release does NOT ha… -->
...
### TREE STATE: build 5954F53F = keeper 8A437A71 + gated-OFF dir_ail_defer (REGRESSES if on, `docs/history/compiled-dir-reuse-datablock-reread-wedge-aild-destage.md`) + gated-OFF dir_relverify probe + harmless sticky i_dlm_dir_contended (set on dir BAST, only read by off-defer). Functionally == keeper. 1/2/4 tcp PASS, 8/tcp 16/17 (dir_reuse loses ~1-2/run). NO regression. `docs/history/compiled-dir-reuse-datablock-reread-wedge-aild-destage.md`
