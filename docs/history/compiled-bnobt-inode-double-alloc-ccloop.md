<!-- Compiled: ccloop cache_coherency bnobt double-free / inode double-alloc root chain, write-side stopgaps, and AG-lock fence design. -->
...
- **Write-side revert-clobber (AG free-space trees).** `xfsaild` flushes a PRIOR-TENURE
  ...
  (`disk_nr=2`). Root cause named in `docs/history/docs/history/compiled-bnobt-inode-double-alloc-ccloop.md`: **AIL item removal is
  ...
  `docs/history/docs/history/compiled-bnobt-inode-double-alloc-ccloop.md`.
- **Read-side incoherent alloc (inobt + inode-cluster).** The allocator carves an inode
  ...
  `docs/history/docs/history/compiled-bnobt-inode-double-alloc-ccloop.md` (detector P-IALLOC-DBLALLOC, gated
  ...
  in `docs/history/docs/history/docs/history/compiled-bnobt-inode-double-alloc-ccloop.md` as **W2**.
...
allocations carved the same inode/chunk (`docs/history/docs/history/docs/history/compiled-bnobt-inode-double-alloc-ccloop.md`).

## Build / fix progression (chronological)
- **3CBB2553** (`docs/history/docs/history/compiled-bnobt-inode-double-alloc-ccloop.md`, KEEP): first bnobt double-free FS-shutdown fix —
...
- **82BB2A09** (`docs/history/docs/history/compiled-bnobt-inode-double-alloc-ccloop.md`, KEEP): cache_coherency 1/4 → 3/4 (cross/rename/cwr
...
- **E9963FFA** (`docs/history/docs/history/compiled-bnobt-inode-double-alloc-ccloop.md`, KEEP): the **P122 write-side
  ...
  cross-node).
- **57B04634** (`docs/history/docs/history/compiled-bnobt-inode-double-alloc-ccloop.md`, KEEP but incomplete): traced a
...
- **51755ECD** (`docs/history/docs/history/compiled-bnobt-inode-double-alloc-ccloop.md`, KEEP): implemented the **shared on-disk
...
- **C91DECFC → C2AD9E9D** (`docs/history/docs/history/docs/history/compiled-bnobt-inode-double-alloc-ccloop.md`): PROVEN the
...
- **2C16B9C9** (`docs/history/ccloop-suppression-was-corruptor-3of4.md`, KEEP): **the P122/P93
...
- **1F7AF33C** (`docs/history/docs/history/compiled-bnobt-inode-double-alloc-ccloop.md`): ROOT of the inode
...
`docs/history/docs/history/docs/history/compiled-bnobt-inode-double-alloc-ccloop.md`: a fresh `cache_coherency.sh --nodes 4`
...
a passing isolated run — it's just not always fatal. Corollary lesson (`docs/history/docs/history/compiled-bnobt-inode-double-alloc-ccloop.md`):
...
`docs/ag-metadata-coherency.md`: the cache_coherency **re-architecture** is a
three-fence-invariant design (provenance: **Grok+Gemini+Claude 2026-06-06**, NOT GPT-5.5 as
`docs/history/docs/history/compiled-bnobt-inode-double-alloc-ccloop.md` mis-attributes; problem statement in
...
`docs/history/docs/history/compiled-bnobt-inode-double-alloc-ccloop.md` shared-epoch, delivered in build 51755ECD).
...
- CIL→AIL runs on a background kworker AFTER `xfs_log_force` returns, so a just-committed
  ...
  whole-AG ail_push (`docs/history/docs/history/docs/history/compiled-bnobt-inode-double-alloc-ccloop.md`).
- Write-side numrecs suppression is broken by legit coalescing (nr decreases legitimately);
  it corrupted the AG-meta set (`docs/history/ccloop-suppression-was-corruptor-3of4.md`).
- P124 generalized suppression regressed to 0/4 (refreshing one bnobt desyncs siblings)
  (`docs/history/docs/history/docs/history/compiled-bnobt-inode-double-alloc-ccloop.md`).
