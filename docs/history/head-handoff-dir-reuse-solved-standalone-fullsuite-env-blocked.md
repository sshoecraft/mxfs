<!-- sess29(ccloop) HEAD: dir_reuse 8/tcp SOLVED standalone ~85% (dir_release_invalidate+relinval_clean, build 23FE6715). All tests pass STANDALONE. Full… -->
...
- GPT-5.5 architecture (design consult): "no old-epoch buffer reaches disk after its DLM lock releases; invalidate ALL dir-fork buffers at release; PLAIN cold-read on reacquire; FUA is NOT a coherency primitive (tears vs target write-back cache)."
...
1. **cache_coherency IN-SUITE 0/8 = ENVIRONMENTAL CONTAMINATION** (NOT my code — passes standalone; build edit is provably inert at default). Appeared after ~15 full-cluster test cycles this session (incl. the harmful flush_lockwait run). full8.sh virsh-resets VMs but the degradation persists (clyde target/LUN/harness state). First full8 (D1DD1926) PASSED cache_coherency in-suite; all subsequent failed 0/8. **NEXT SESSION: start COLD; if cache_coherency in-suite passes again it was transient. If it persists, the dev-host state needs clearing (user may need to reset clyde's SCST target — the never-reboot-clyde rule forbids rebooting clyde).**
...
See `docs/history/corrected-state-dir-reuse-85pct-not-100-flush-lockwait-harmful.md` `docs/history/compiled-dir-reuse-writeside-3way-merge-release-invalidate.md` `docs/history/compiled-dir-reuse-writeside-3way-merge-release-invalidate.md` `docs/history/compiled-dir-reuse-writeside-3way-merge-release-invalidate.md`.
