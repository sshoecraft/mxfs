<!-- sess49 COMPLETE diagnosis of the residual (tcp_dlm_scaling/dlm_fairness drain-fail): TCP BAST-not-demoting → stale cached i_dlm_mode==EX double-grant… -->
...
cache_coherency FIXED+reliable (P43 EX-gate, `docs/history/root-cache-coherency-uv-is-p43-fmtrevert-guard-misfire.md`).
...
### COMPLETE ROOT CHAIN (the instrument-first loop, this session + sess10/sess17):
...
### THE FIX (sess10 GPT verdict `docs/rulings/serialize-tenure-not-epoch.md`, NOT converged):
...
HB-interval reduction (500ms WEDGES, 1000ms no better — `docs/history/docs/history/compiled-tcp-dlm-doublegrant-gentoken-fix.md`),
...
Related: `docs/history/confirmed-staleflush-clobber-p17.md` `docs/history/docs/history/compiled-tcp-dlm-scaling-sess10-doublegrant-tenure.md`
