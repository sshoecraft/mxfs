<!-- sess106 — ROOT PROVEN: rename_visibility loss = concurrent same-name mkdir → 2 nodes modify parent dir inode (131) with DIVERGENT bases → divergent c… -->
...
## ROOT CAUSE — PROVEN (the instrument-first loop): concurrent same-name mkdir → mutual-exclusion VIOLATION via stale-cached-EX
...
## GEMINI (the design-consult rule) DESIGN — lease + cheap-read epoch verify (full design in transcript)
...
STEP 1 (PROVE, the instrument-first loop): at the dir-EX fast-path cache-hit (xfs_mxfs_dlm.c ~3254/3490 and ~3874), when
...
07665752 LOOPED run (test1). Own the instrument-first loop loop later; contaminates looped repros.
