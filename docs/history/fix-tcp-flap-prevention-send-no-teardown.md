<!-- sess40 KEEPER build A985424B: dlm/peer.c flap-prevention (transient send timeout no longer tears down socket). Helps mass-fail mode; does NOT fix 799… -->
...
- The 799 = concurrent-add/extent-fork lost-update → add an inode/bmbt extent-fork write-chokepoint detector (`docs/history/correction-799-is-concurrent-add-not-flap-bmbt-extent-fork.md`).
- The mass-fail/membership-flap mode → GPT-5.5 reliable-midcomms docs/project-history.md (seq/ack/resend-on-reconnect/dedup + grant-gen cookies) in `docs/history/reframe-799-is-tcp-flap-not-buffer-barrier-noop.md`.
