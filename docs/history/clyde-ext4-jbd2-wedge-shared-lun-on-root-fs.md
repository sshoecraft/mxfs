...
- **the never-reboot-clyde rule: never reboot clyde.** Document and report; the reset is the user's call.
- **the unkillable-wedge rule:** `virsh destroy` against a deadlocked domain hangs — always bound it
...
`sudo tools/clyde_wedge_diag.sh` (added sess385). the unkillable-wedge rule-safe: reads only
