<!-- sess15(ccloop) UNIFYING INSIGHT: BOTH 8/tcp blockers = the SAME TCP-DLM acquire starvation. dir_reuse=slow retries; tcp_dlm_scaling=acquire FAILS -11… -->
...
1. **dir_reuse_coherency** — acquire RETRIES succeed eventually (correct) but cost ~60s stalls per slow round ⇒ ~630s total ⇒ the derived-budget rule timeout FAIL at default budget. (Passes 8/8 at TEST_TIMEOUT=900.)
...
See `docs/history/docs/history/docs/history/compiled-dirreuse-8tcp-tcpdlm-acquire-starvation-slowness.md` `docs/history/docs/history/docs/history/compiled-dirreuse-8tcp-tcpdlm-acquire-starvation-slowness.md` `docs/history/criterion-met-2tcp-17of17-8consecutive.md`.
