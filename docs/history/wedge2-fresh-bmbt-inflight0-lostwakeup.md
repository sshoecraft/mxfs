<!-- sess5 wedge#2 FRESH repro (default cfg, build E5F760E6): rank1 rm stuck 192s in mxfs_dir_bmbt_scan→xfs_bwrite→xfs_buf_iowait, inflight=0 (lost-wakeup… -->
...
## Also this session: reproducible HARD-HANG (separate bug, spinlock deadlock) — see memory docs/history/docs/history/docs/history/compiled-dir-reuse-32caw-wedge-campaign.md. And run.sh convergence gate parallelized (was serial-32-SSH, false PREP FAIL at N=32).
