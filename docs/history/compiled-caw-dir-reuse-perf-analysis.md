<!-- dir_reuse_coherency perf holdout @16/32: cost is inherent disk-CAW per-inode coherence (verify-bound); budget-correct after create-only fast path. -->
...
- 32 nodes projected ~4× worse (O(N²)); same analysis, higher stakes — fix once, applies to both.
  `docs/history/docs/history/docs/history/compiled-caw-dir-reuse-perf-analysis.md`
