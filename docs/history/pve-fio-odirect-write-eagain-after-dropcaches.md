<!-- PROVEN: on mxfs N>=2, an O_DIRECT write to an inode just evicted by drop_caches returns spurious -EAGAIN (offset 0), zeroing fio_perf seqW/randW. Nat… -->
...
## the instrument-first loop evidence (all proven, not guessed)
...
## Hypothesis (under GPT review — design consult sent, task ktw6q985u)
...
Two the instrument-first loop discriminators run at ZERO build cost confirmed GPT's #1:
...
WRONG — see `docs/history/docs/history/pve-timestamp-update-ex-iflush-breaks-create-visibility.md` for the
...
`docs/history/docs/history/compiled-tcp32-dlm-correctness-campaign.md` (mxfs write-path / grant-lifecycle
context), `docs/history/docs/history/pve-agi-buf-hold-leak-umount-wedge-not-sess76-readahead.md` (the OTHER
open Proxmox bug, from fence_during_write), `docs/history/docs/history/runsh-ssh-node-pipeline-swallows-remote-exit-status.md`.
