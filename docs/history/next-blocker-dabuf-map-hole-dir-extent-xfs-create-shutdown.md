<!-- sess45: after the crash_consistency inode-cluster fix (KEEP), full ./run.sh 2 tcp = 0/17 because an EARLY test wedges test1: xfs_create→xfs_dabuf_map… -->
...
(`docs/history/fix-partial-iwrite-skips-fresh-free-inodes-inode-alloc-buf.md`, build C0554679/EF006296,
...
`docs/history/docs/history/docs/history/compiled-tcp-dir-reuse-stale-inode-fork-flush-fence.md`, `docs/history/docs/history/compiled-sess30-33-dir-reuse-ag-inode-cluster-double-alloc.md`).
...
### NEXT (the instrument-first loop): reproduce standalone — `./run.sh 2 tcp cache_coherency` (the historically-hard
...
(mxfs_dir_data_durable / b_mxfs_dir_gen invalidation). `docs/history/fix-partial-iwrite-skips-fresh-free-inodes-inode-alloc-buf.md`</body>
