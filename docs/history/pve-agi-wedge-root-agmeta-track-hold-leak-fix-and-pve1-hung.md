<!-- AGI umount-wedge ROOT CAUSE PROVEN = mxfs_ag_meta_track hold leaked on shutdown-abort (no writeback -> iodone never fires); one-shot-token FIX built… -->
# AGI umount-wedge: ROOT CAUSE PROVEN + FIX built; verification blocked by pve1 hang

Continues `docs/history/docs/history/pve-agi-buf-hold-leak-umount-wedge-not-sess76-readahead.md`. 2026-07-21, PVE rig
(pve1=192.168.1.80 / pve2=192.168.1.81, kernel 6.17.2-1-pve).

## ROOT CAUSE — PROVEN (the instrument-first loop, via reason-tagged hold-ring)
...
- called from `xfs_buf_item_release`'s abort branch (`xfs_buf_item.c:832`, the PROVEN leak
  path). (Stale branches 820/528 NOT hooked yet — unproven, deferred per the instrument-first loop.)
...
## REPRO (new harnesses, the source-tree rule, in scripts/)
...
## ALSO SURFACED (open, the zero-defect bar — do not drop)
...
1. **pve2 flush_workqueue umount wedge** — DISTINCT from the AGI drain wedge. Stack:
   ...
   Likely a cross-node teardown-ordering wedge; needs its own the instrument-first loop pass.
...
2. Run `scripts/agi_wedge_verify_det.sh` several cycles (alternate TARGET pve1/pve2, reprep
   ...
   FIXED-AND-VERIFIED bar (the zero-defect bar).
