<!-- sess377: clyde HOST kernel corrupts page-cache folio->private with iSCSI IQN text (3 oopses); kills VMs permanently. VM-clone recovery works with no… -->
...
## Why this matters to the campaign (the zero-defect bar pollution risk)
...
## RECOVERY PROCEDURE — restores a lost node WITHOUT rebooting clyde (the never-reboot-clyde rule)
...
iscsi_scst. Per the never-reboot-clyde rule no session may do this. Until then, every corruption-
