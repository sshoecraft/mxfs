<!-- Compiled: reused-inode/NL-cached reload coherency — type-flip (ENOTDIR/EISDIR), gen-check d_revalidate, ABA dir-block reuse; sess40-104. -->
...
`docs/history/docs/history/compiled-cc-inode-reuse-reload-typeflip.md`, `docs/history/docs/history/compiled-cc-inode-reuse-reload-typeflip.md`, `docs/history/docs/history/compiled-dirreuse-4tcp-bast-race-datablock-doublealloc.md`.
...
- **EISDIR** (`cat X: Is a directory`): reader holds a live in-core inode of the OLD type (S_IFDIR),
  number reused as a REG file; VFS serves the dir vtable. (`docs/history/docs/history/compiled-cc-inode-reuse-reload-typeflip.md` Failure A, `docs/history/docs/history/compiled-cc-inode-reuse-reload-typeflip.md` Face A)
- **ENOTDIR** (`Not a directory` on child create/delete): parent-dir inode flips DIR→REG mid-test;
  path resolution of children of a now-REG inode fails. (`docs/history/docs/history/compiled-dirreuse-4tcp-bast-race-datablock-doublealloc.md` unlink_visibility, `docs/history/docs/history/compiled-cc-inode-reuse-reload-typeflip.md` rename_visibility)
- **File invisible to own creator / missing-dirent**: concurrent shortform→block dir-conversion
  lost-update, or stale cached dir DATA block missing a peer's committed dirents. (`docs/history/docs/history/compiled-cc-inode-reuse-reload-typeflip.md` Failure B, `docs/history/docs/history/compiled-cc-inode-reuse-reload-typeflip.md` Face B)
- **Empty-content** (`.md5` sidecar reads di_size=0): reg-file di_size not flushed to cluster on
  BAST-release. (`docs/history/docs/history/compiled-cc-inode-reuse-reload-typeflip.md`, recurs `docs/history/docs/history/compiled-cc-inode-reuse-reload-typeflip.md`/`docs/history/docs/history/compiled-cc-inode-reuse-reload-typeflip.md` as cross_write_read)
- **FS shutdown**: bnobt double-free (`ltbno+ltlen>bno` xfs_alloc.c:2244), dialloc EFSCORRUPTED
  i!=1, iunlink AGI-garbage — same lost-update family surfacing on alloc/free metadata. (`docs/history/docs/history/compiled-cc-inode-reuse-reload-typeflip.md`, `docs/history/docs/history/compiled-dirreuse-4tcp-bast-race-datablock-doublealloc.md`, `docs/history/docs/history/compiled-cc-inode-reuse-reload-typeflip.md`)

## Two generation signatures (the key diagnostic, `docs/history/docs/history/compiled-dirreuse-4tcp-bast-race-datablock-doublealloc.md`)
...
2. **same-gen** (`incore_gen==disk_gen`, type differs): inode number allocated TWICE without an
   ...
   defense slips through this.** (`docs/history/docs/history/compiled-cc-inode-reuse-reload-typeflip.md` proved double-alloc; `docs/history/docs/history/compiled-dirreuse-4tcp-bast-race-datablock-doublealloc.md` same-gen ino=2097285)

## Chronological fixes & build markers

### sess40 (build progression `210D0DD`→`45F00582`→`4545ABEA`→`598B434F`→`8816466`→`EF3E975F`→`214A3360`)
`docs/history/docs/history/compiled-cc-inode-reuse-reload-typeflip.md` — the founding session.
...
- **All per-op DLM logging gated behind `mxfs.instr`** — ungated logs were the ~100x slowdown. LESSON:
  ...
  (100x printk slowdown serializes it) — recurs as a hard rule in `docs/history/docs/history/compiled-cc-inode-reuse-reload-typeflip.md`.
...
### sess53 — REFRAME: shortform-dir reload miss (`docs/history/docs/history/compiled-cc-inode-reuse-reload-typeflip.md`, build `C2D30DB0`)
...
### sess54 — REFUTES sess53's reframe, re-confirms NL-cache root (`docs/history/docs/history/compiled-cc-inode-reuse-reload-typeflip.md`, build `C2D30DB0`)
...
### sess55 — Gemini 3-part design saved (`docs/history/docs/history/compiled-cc-inode-reuse-reload-typeflip.md`, to notes/sess55_gemini_design.md)
...
does synchronous inode-cluster re-read of di_format/i_size before any txn ILOCK. Also codified the design-consult rule:
**omit max_tokens on ask_* consults.**

### sess72 — Face A FIXED (stale vtable), Face B isolated (`docs/history/docs/history/compiled-cc-inode-reuse-reload-typeflip.md`, build `8B1A4AD6`, KEEP)
...
### sess86 — rename_visibility FIXED via d_revalidate gen-check (`docs/history/docs/history/compiled-cc-inode-reuse-reload-typeflip.md`, build `5D6A63D2`, passed 1→2)
...
### sess87 — reload-shutdown FIXED; deep root = INODE DOUBLE-ALLOCATION (`docs/history/docs/history/compiled-cc-inode-reuse-reload-typeflip.md`, build `77C90663`, KEEP)
...
### sess90 — refuted xfsaild theory; FUA-over-logged root PROVEN (`docs/history/docs/history/compiled-dirreuse-4tcp-bast-race-datablock-doublealloc.md`, build `D1E532E2`)
...
### sess102 — fua_disable reframe; in_ail-discard fix (`docs/history/docs/history/compiled-cc-inode-reuse-reload-typeflip.md`, build `B3C15760`, KEEP, 3/4)
...
### sess104 — ABA inode-reuse root FIXED (`docs/history/docs/history/compiled-cc-inode-reuse-reload-typeflip.md`, build `736ECD00`, KEEP, 3/4, wedge gone)
...
- **instr=1 HIDES the race** — 100x printk slowdown serializes concurrent creates → false PASS. Diagnose
  ONLY with always-on detectors (SESS50-STARVE/COHOLD, P90-*, P-IRESURRECT). (`docs/history/docs/history/compiled-cc-inode-reuse-reload-typeflip.md`, `docs/history/docs/history/compiled-cc-inode-reuse-reload-typeflip.md`)
- **NO ILOCK_EXCL across CAW poll** — xfsaild AIL-drain wedge / D-state; in-place reload under ILOCK_EXCL
  ...
  locks, not inside xfs_iget. (`docs/history/docs/history/compiled-cc-inode-reuse-reload-typeflip.md`, `docs/history/docs/history/compiled-cc-inode-reuse-reload-typeflip.md`)
- **NO per-op FUA in the hot path** — barrier markers are empty files hit constantly → 120s timeouts.
  Gate FUA gen-checks to peer-AG DIRS only. (`docs/history/docs/history/compiled-cc-inode-reuse-reload-typeflip.md`, `docs/history/docs/history/compiled-cc-inode-reuse-reload-typeflip.md`)
- **Perf-vs-correctness tension is structural:** every reuse fix that adds a DLM/FUA round-trip per iget
  ...
  content alone. (`docs/history/docs/history/compiled-cc-inode-reuse-reload-typeflip.md`)
- **gen-based defenses are all blind to the same-gen case** (double-alloc + inode-cluster stale-flush) —
  need a TYPE-aware trap or to stop the stale-flush at the source. (`docs/history/docs/history/compiled-dirreuse-4tcp-bast-race-datablock-doublealloc.md`, `docs/history/docs/history/compiled-cc-inode-reuse-reload-typeflip.md`)
- **`disk_differs=1` at writeback is an artifact** (in-core=new, disk=old), not evidence of corruption.
  (`docs/history/docs/history/compiled-cc-inode-reuse-reload-typeflip.md`)
