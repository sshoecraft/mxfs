<!-- sess58 CRITERION MET: build 60EFBE5E = ./run.sh 2 tcp 17/17 × 8 consecutive clean-reboot runs (100%). Two roots fixed: create↔remove AG↔dir ABBA dead… -->
...
### TWO ROOT CAUSES PROVEN (the instrument-first loop) + FIXED — both in xfs/xfs_inode.c:

**(1) create↔remove AG↔dir ABBA deadlock** (was the tcp_dlm_scaling within-window/shutdown failure). See `docs/history/fix-create-remove-ag-dir-abba-deadlock.md`.
...
**(2) durable dirent RESURRECTION** = xfs_ilock_nowait skips the DLM grant for ILOCK. See `docs/history/fix-resurrection-lock-two-inodes-nowait-skips-dlm.md`.
