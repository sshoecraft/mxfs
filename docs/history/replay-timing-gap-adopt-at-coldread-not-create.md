<!-- sess65: epoch-adopt+pending-replay built but REPLAY fires ~0x — timing gap: losing node (rank1) adopts winning block0 only at COLD-READ reload (no tx… -->
...
Candidate: dir-inode-fork FLUSH FENCE (docs/history/docs/history/compiled-tcp-dir-reuse-stale-inode-fork-flush-fence.md, xfs_iflush ~4298) — don't let a node's iflush publish a dir extent[0] that orphans a peer's already-durable block0; instead union the dir blocks. OR a deterministic per-incarnation conversion owner.
See `docs/history/docs/history/docs/history/compiled-dirreuse-node1f1-double-block0-orphan.md` `docs/history/docs/history/docs/history/compiled-dirreuse-node1f1-doublealloc-block0-root.md`.</body>
