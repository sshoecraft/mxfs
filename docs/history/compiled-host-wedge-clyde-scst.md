<!-- Compiled: recurring clyde SCST/iscsi_conn_cleanup D-state host wedge — reboot-only clear (user's call, the never-reboot-clyde rule), trigger, and cluster-independent work. -->
...
per the never-reboot-clyde rule no session may reboot/sysrq clyde — the reboot is the USER'S call. This is not a new
...
host reboot cleared it. the never-reboot-clyde rule was written the SAME day precisely because sess14's
...
- **Trigger (DO NOT REPEAT):** tight-loop `virsh destroy` of ~32 VMs right after the 32-node
  ...
  so 32-runs stop wedging nodes. `docs/history/docs/history/docs/history/compiled-host-wedge-clyde-scst.md`
...
user's call; do NOT reboot and do NOT install `@reboot` hooks (the never-reboot-clyde rule).**
...
1. `cut -d' ' -f1 /proc/loadavg` + `ps -eo stat,comm|awk '$1~/Z/&&$2~/qemu/'|wc -l` (or
   ...
   - **load ~1000+ AND qemu zombies → STILL WEDGED** (not rebooted): concisely re-surface "clyde
     needs a manual reboot (the never-reboot-clyde rule) — analysis complete, criteria host-gated" and do CODE-ONLY work.
...
rebooted the playbook executes end-to-end. `docs/history/docs/history/docs/history/compiled-host-wedge-clyde-scst.md`
