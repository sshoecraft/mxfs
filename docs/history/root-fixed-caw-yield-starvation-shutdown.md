<!-- ROOT+FIX D-CAW-YIELD-STARVATION-SHUTDOWN: compatible-yield fresh acquires never registered in waiters -> invisible to yield_to ticket -> 100-retry ex… -->
...
## Fix (GPT-reviewed per the design-consult rule; option C of A/B/C/D/E)
...
## Verification state (the zero-defect bar: still OPEN, verification accumulating)
...
- Post-fix (0.11.269): 5/5 dirent_durability PASS at 65-66s (incl. 3 laps with
  ...
  (cache 24-26s, dir_reuse 101-110s — the derived-budget rule clean).
