<!-- sess43-44: 2/tcp criterion is FULL run.sh suite; most fails = contamination; dir_force_block=1 is a net regression, =0 fixes cache_coherency+dlm_fair… -->
...
**Criterion retracted and corrected** `docs/history/docs/history/compiled-sess43-44-2tcp-suite-contamination-forceblock-regression.md`: a premature "criteria-met YES" was challenged when the user showed `showstat.sh 2 tcp` = 2 PASS / 11 FAIL / 4 PENDING. The criterion had been over-narrowed to `dir_reuse_coherency` (all prior sessions in this lineage worked on that). Marker set back to NO. The `dir_reuse_coherency` fix (force_block=1 default + P43/P43B) is real and KEPT regardless.

**Suite mechanism**: `run.sh` preps ONCE and runs all 17 tests back-to-back with NO reset/reboot between them. An early test that shuts down the FS (prime suspect: `cache_coherency`, the sess79-92 lineage blocker) contaminates every test after it. So "2/17" is largely a harness artifact, not 11 separate bugs `docs/history/docs/history/compiled-sess43-44-2tcp-suite-contamination-forceblock-regression.md`.

**Standalone (fresh prep, clean reboot) results, build 422E6DC6** `docs/history/docs/history/compiled-sess43-44-2tcp-suite-contamination-forceblock-regression.md`:
...
**Proven under the instrument-first loop with clean reboots between runs** `docs/history/docs/history/compiled-sess43-44-2tcp-suite-contamination-forceblock-regression.md`, build 422E6DC6:
...
**Full `./run.sh 2 tcp` in-suite sweep, build 9A10A077 (force_block=0 default), partial** `docs/history/docs/history/compiled-sess43-44-2tcp-suite-contamination-forceblock-regression.md`:
