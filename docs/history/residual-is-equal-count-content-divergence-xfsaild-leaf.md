<!-- sess37: residual = GENUINE-EX-holder (real_mode=5, 1890/1892) equal-count content-divergent background reflush. Stale-cached-EX rare (2). Points to D… -->
...
### FIRST cheap diagnostic next session: at the P-DATACLOBBER detect, when real_mode=5 and content diverges, ALSO query+log whether a PEER currently holds EX/PR on this dir (cross-node holder dump) — settles double-grant vs read-served-stale decisively. See `docs/history/session-37-handoff.md` `docs/history/residual-tcp-doublegrant-dir-resurrection-complete-diagnosis.md` `docs/history/compiled-dir-reuse-platter-lag-grantevict-inail-clean-reflush.md`.
