<!-- AGI umount-wedge FIXED AND VERIFIED (the zero-defect bar): AG-meta track-hold reclaim on shutdown-abort; deterministic fault-injection (mxfs.dbg_dialloc_shutdown)… -->
# AGI umount-wedge: FIXED AND VERIFIED (the zero-defect bar)

Resolves the OPEN item in `docs/history/pve-agi-wedge-root-agmeta-track-hold-leak-fix-and-pve1-hung.md`
and the original `docs/history/docs/history/pve-agi-buf-hold-leak-umount-wedge-not-sess76-readahead.md`. 2026-07-21,
...
- New harnesses (the source-tree rule): `scripts/agi_wedge_verify_inject.sh` (deterministic, preferred),
