<!-- sess40 REFRAME: dir_reuse readdir=799 single-dirent loss CORRELATES with a transient TCP DLM flap (peer disconnect+reconnect ~500ms). Buffer barrier… -->
...
### PROVEN reframe (the instrument-first loop, drc_cap8.sh + node dmesg snapshots)
...
### NEXT (transport reliability — pick one, the instrument-first loop)
