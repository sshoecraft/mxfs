<!-- sess18 ccloop: 8/tcp dir_reuse_coherency correctness SOLVED at mht>=275/300 (build 58360875), only speed floor + reload-reliability residual remain. -->
...
- **8/tcp `dir_reuse_coherency`**: at DEFAULT mht=300, `TEST_TIMEOUT=400 ./run.sh 8 tcp
  ...
  See `docs/history/sess18run-milestone-8tcp-dirreuse-passes-correct-mht300-speed-only-residual.md`.
- At mht=275: failrounds=0 on all 8 nodes across **3 consecutive clean-reboot runs
  (72 rounds, 0 dirent loss)**. See
  `docs/history/docs/history/docs/history/compiled-dirreuse-8tcp-correctness-solved-speed-floor.md`.
...
1. **NO_INODE BAST recv-thread offload** — the headline fix, own build `16A3B9C9`.
   Root (PROVEN, the instrument-first loop via mxfs-DRCph phase markers): 8/tcp failed by SLOWNESS, the
   ...
   **rc110=0**, failrounds=0 at both mht=300 and mht=250.
   See `docs/history/docs/history/docs/history/compiled-dirreuse-8tcp-correctness-solved-speed-floor.md`.
...
Plus a the derived-budget rule masking-delay removal: the per-round `sleep 1` in
...
the fix. See `docs/history/docs/history/docs/history/compiled-dirreuse-8tcp-correctness-solved-speed-floor.md`.

### Speed facts (the derived-budget rule)
...
- Per-round ~13-14s at mht=275/300. Breakdown: create ~6s (8-node dir-EX serialization is
  ...
  the derived-budget rule 2× ceiling, but the test DOES complete correctly.

### NEXT-session leads (from `docs/history/docs/history/docs/history/compiled-dirreuse-8tcp-correctness-solved-speed-floor.md`)
...
- Do NOT widen the 300s blanket to pass (the derived-budget rule). A principled per-test budget for 8-node
