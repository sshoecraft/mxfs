<!-- sess59 PROVEN: dir_reuse 4-node readdir-miss root = grant-less NL readdir depends on ASYNC heartbeat eviction-ring DIR_MODIFY to trigger reload; at c… -->
## sess59 — PROVEN root cause (the instrument-first loop) of dir_reuse_coherency 4-node readdir-miss
...
- Tighten heartbeat/DIR_MODIFY latency: `docs/history/docs/history/compiled-tcp-dlm-doublegrant-gentoken-fix.md`
...
- **C:** readers acquire a real PR grant for readdir on shared dirs (correct
  clustered-FS coherency) — heaviest perf, the derived-budget rule risk.
...
Prior obvious fixes refuted + sharp perf constraint → may meet design consult
bar AFTER trying ≥1-2 distinct fixes. Repro: tests/tcp/drc4_repro.sh 4 50 24
(concurrent reads, fails ~round 9). See
`docs/history/docs/history/compiled-sess59-60-4node-tcp-dir-reuse-readdir-miss-writer-clobber.md`,
`docs/history/docs/history/compiled-sess59-60-4node-tcp-dir-reuse-readdir-miss-writer-clobber.md`.
