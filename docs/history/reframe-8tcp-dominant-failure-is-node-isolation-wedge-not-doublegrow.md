<!-- sess46(ccloop): 8/tcp dir_reuse MULTI-MODAL — ~equal MASS/node-isolation(>40s unreachable→dead→work lost) + SINGLE intra-block. Host NOT oversubscrib… -->
...
### KEY CORRECTIONS to sess45's model (all the instrument-first loop grounded):
...
3. **LIO target WRITE-THROUGH + REJECTS FUA passthrough** (kern.c:502-514):
   ...
   fixes it" = TIMING-MASKING (the derived-budget rule debt).
...
### NEXT (the instrument-first loop) — attack the DOMINANT isolation/wedge mode:
...
tests/drc_batch8.sh. See `docs/history/winning-config-and-remaining-work.md`.
</body>
