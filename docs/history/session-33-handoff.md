<!-- sess33 HEAD: DECISIVE — dir_reuse 799 loss-write IS the EX-release drain's xfs_bwrite (P-WMERGE-STACK: mxfs_dir_flush_one_daddr→xfs_bwrite, comm=dd).… -->
...
### WHY all retire/skip fixes failed (8 refuted, see `docs/history/compiled-dir-reuse-duplicate-buffers-ownedex-inail-reflush.md` `docs/history/compiled-dir-reuse-duplicate-buffers-ownedex-inail-reflush.md`):
...
### REPRO: `bash tests/tcp/drc_repro_loop.sh 6 "dir_writeprobe=1 dirwr=1" 24` (readdir=799, ~1/2 iters; probe amplifies). Capture P-WMERGE (loss-write state) + P-WMERGE-STACK (writer path). `docs/history/compiled-dir-reuse-duplicate-buffers-ownedex-inail-reflush.md` `docs/history/compiled-dir-reuse-duplicate-buffers-ownedex-inail-reflush.md`
