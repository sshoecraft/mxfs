<!-- sess107 — ROOT PROVEN: stale-cached-EX = deferred-publish never publishes shared inodes (unpublished=no slot→peer acquires w/o BAST). Backstop fix (b… -->
# sess107 (2026-06-06, ccloop run 29df431e)

Continues sess106. Builds: `B64499C5` (probe) → `0DB3BA32` (backstop FIX, current deployed).

## ROOT CAUSE — FULLY PROVEN (the instrument-first loop + code trace)
...
## STEP 2 FIX (build 0DB3BA32, the design-consult rule Gemini design D = A+B). KEEP (partial win).
...
- **P106-STALE-EX still fires 29× on test1** (0 on test2/3/4) → RESIDUAL stale-cached-EX NOT
  covered by the backstop. Hypotheses for next session (the instrument-first loop — instrument to pick):
