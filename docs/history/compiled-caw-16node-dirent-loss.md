<!-- Compiled: 16-node durable dir-block dirent-removal lost-update proven CASE B (post-release async clobber); Case A refuted; fix target + refuted tally. -->
## 16-node durable dir-block dirent-removal lost-update (dangling dirent) — compiled

ccloop 26c41354, sess1, 2026-07-06. Kernel build **591A76FB** (no kernel edits landed this session — only run.sh + scripts/caw_preflight.sh). All work on the CAW transport at 16 nodes. Sources: `docs/history/docs/history/docs/history/compiled-caw-16node-dirent-loss.md`, `docs/history/docs/history/docs/history/compiled-caw-16node-dirent-loss.md`, `docs/history/docs/history/docs/history/compiled-caw-16node-dirent-loss.md`, `docs/history/docs/history/docs/history/compiled-caw-16node-dirent-loss.md`.
...
The decisive result — no rebuild, from `docs/history/docs/history/docs/history/compiled-caw-16node-dirent-loss.md`: ran cache_coherency at 16 with `MXFS_EXTRA_MODARGS="dirwr=1 dirland=1 dir_relverify=1"`. `dir_relverify` verifies disk==incore at EVERY dir-EX release, logging `P25-RELVERIFY-MISMATCH ino= daddr= incore_cnt= disk_cnt=` on divergence.
...
- Caveat: relverify=1 also TIMES OUT the test at 16 (per-release FUA-read latency, the derived-budget rule) → DIAGNOSTIC ONLY, not shippable. But P25=0 is robust re: drain correctness regardless of timeout.

### Case-B narrowing: it is a PEER's stale buffer, not the demoting node's own
From `docs/history/docs/history/docs/history/compiled-caw-16node-dirent-loss.md`: tested `dir_release_stale=1` (GFS2 invalidate-on-demote: demoting node clears XBF_DONE|_XBF_FUA_FRESH on its own dir buffers at EX release).
...
### Refuted-fix tally (the design-consult rule: ≥3 distinct approaches refuted with evidence)
...
2. **Release guards ON** (`dir_relverify=1 dir_release_stale=1 dir_wr_barrier=1`): cache_coherency TIMEOUT 0/16 (~300s), the derived-budget rule. Heavyweight per-release FUA verify + write-barrier wait is TOO SLOW at 16-node handoff frequency. THE FIX MUST BE LIGHTWEIGHT (no per-handoff latency).
...
### GPT-5.5 root-cause ranking (design consult, from `docs/history/docs/history/docs/history/compiled-caw-16node-dirent-loss.md`)
...
### the design-consult rule escalation status
