<!-- sess28(ccloop) HANDOFF HEAD: keeper build C1B4BFC0 (all levers off == prior 164A6D5D). Loss=WRITE-SIDE confirmed. Next: NFS-stream P-WMERGE+mode to p… -->
...
### THE decisive sess28 finding (the instrument-first loop, clean cluster): the loss is WRITE-SIDE; the read is fully coherent.
- diff1=0 over 300 samples (in-core dir block == platter at addname), p28c=0, rdmiss=1, corrupt=0. Read-side fix (dir_addname_coherent) is a DEAD END. See `docs/history/decisive-loss-is-writeside-read-coherent-diff0-p28c0.md`.
- P-WMERGE classifier (dir_writeprobe=1): clobbering dir-data writes are MOSTLY **pure-stale** (incore_extra=0, disk_extra>0 — test1 daddr=120 disk_extra=153 = reverting 153 peer entries) + a few **MERGE-NEEDED** (incore_extra>0 AND disk_extra>0). See `docs/history/compiled-dir-reuse-writeside-3way-merge-release-invalidate.md`.
- ALL drop-suppression FAILS: full subset_guard -> readdir=316 (drops legit merge-needed); pure-stale-only (incore_extra==0) -> SHUTDOWN (structurally unsafe; a legit REMOVE also looks pure-stale). See `docs/history/compiled-dir-reuse-writeside-3way-merge-release-invalidate.md`.

### NEXT STEP (the instrument-first loop, two phases):
