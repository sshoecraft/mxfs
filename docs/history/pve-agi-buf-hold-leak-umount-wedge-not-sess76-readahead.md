<!-- 2/tcp on Proxmox: post-shutdown umount wedges forever in xfs_buftarg_drain on ONE xfs_agi buf (hold=2). Flags prove it is NOT the sess75/76 readahead… -->
...
`docs/history/docs/history/compiled-tcp32-dlm-correctness-campaign.md`, where a starved AG EX produced the
...
## Hypothesis (H1, NOT yet instrumented — instrument step 1)
...
## GPT consult (the design-consult rule, task kkxkr30ds) — mechanism + fix direction
...
`docs/history/docs/history/compiled-tcp32-dlm-correctness-campaign.md` — the `xfs_trans.c:1069` dirty-cancel
shutdown signature and `mxfs_dlm_withdraw_release_all`.
`docs/history/docs/history/runsh-ssh-node-pipeline-swallows-remote-exit-status.md` — other harness fixes
