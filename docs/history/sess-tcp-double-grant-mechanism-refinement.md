<!-- REFINEMENT of the tcp_dlm_scaling double-grant: holder never released (no EXREL) → it's a NON-release removal (Bug-51 stale-removal promoting a waite… -->
## Refines `docs/history/docs/history/docs/history/compiled-tcp-dlm-doublegrant-gentoken-fix.md` — narrows the double-grant mechanism.
...
- NOT a stale/duplicate LOCK_RELEASE removing a live grant (the holder sent no release). A
  grant-sequence/fencing-token-on-RELEASE fix would patch a DISPROVEN hypothesis (the instrument-first loop violation).
...
### NEXT SESSION (the instrument-first loop): build a LOCK-FREE per-CPU event ring in dlm.c (record
