<!-- sess58 PROVEN FIX (build 1686FC03): resurrection root = xfs_ilock_nowait skips DLM for ILOCK; lock_two_inodes AIL-nowait branch modified the dir with… -->
## sess58 — PROVEN ROOT + FIX of the tcp_dlm_scaling durable dirent RESURRECTION

### ROOT (captured, the instrument-first loop): xfs_lock_two_inodes nowait branch modifies the dir WITHOUT a DLM EX grant
...
### RESULT: clean `./run.sh 2 tcp` = **17/17 ONCE** (build 1686FC03, with the `docs/history/fix-create-remove-ag-dir-abba-deadlock.md` remove-fix). ALL tests pass incl tcp_dlm_scaling 2/2, dir_reuse 2/2, soak, fault_netpartition. CRITERION (reliable 100%) NOT yet confirmed — running reliability loop. Marker NOT written.
