<!-- NEW, previously-uncharacterized bug: simultaneous clean-unmount of all 32 nodes wedged 21/32 D-state in blk_execute_rq (block layer, not xfs_buf_iowa… -->
## What happened
...
3. What is `mxfs-worker` actually doing at the point it wedges — kernel
   ...
   something else (the instrument-first loop: instrument before hypothesizing further).
