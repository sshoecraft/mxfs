<!-- Compiled: c7ee71c6 sess20-29 (0.11.150-258) — silent-mkdir-loss hunt, typeflip corpse publish, ABBA, demoter-claim family, cluster-write authority, t… -->
...
success with the entry existing nowhere (`docs/history/docs/history/docs/history/compiled-c7ee71c6-sess20-29-identity-and-evidence-era.md`).
...
a **deterministic** gate (`docs/history/docs/history/docs/history/compiled-c7ee71c6-sess20-29-identity-and-evidence-era.md`).
...
- **One sync-wedged node makes SEVEN criteria read 0/32.** A node deadlocked by the ABBA bug
  ...
  (`docs/history/docs/history/docs/history/compiled-c7ee71c6-sess20-29-identity-and-evidence-era.md`).
- **Three evidence-integrity bugs** made cells report on the *previous* run: window scoping
  ...
  and scanned 0 lines (`docs/history/docs/history/docs/history/compiled-c7ee71c6-sess20-29-identity-and-evidence-era.md`).
...
the round-29 loss (`docs/history/docs/history/docs/history/compiled-c7ee71c6-sess20-29-identity-and-evidence-era.md`) —
...
(`docs/history/docs/history/docs/history/compiled-c7ee71c6-sess20-29-identity-and-evidence-era.md`,
`docs/history/docs/history/docs/history/compiled-c7ee71c6-sess20-29-identity-and-evidence-era.md`). P32E was then the only
marker that was 0 on every passing run and present in every captured loss
(`docs/history/docs/history/compiled-c7ee71c6-sess20-29-identity-and-evidence-era.md`).
...
median 37**, nothing else comparable (`docs/history/docs/history/docs/history/compiled-c7ee71c6-sess20-29-identity-and-evidence-era.md`).
...
belonged at a different layer (`docs/history/docs/history/docs/history/compiled-c7ee71c6-sess20-29-identity-and-evidence-era.md`).
...
neighbours (`docs/rulings/publication-authority-and-lru-tenure.md`).
...
resources* (`docs/history/docs/history/docs/history/compiled-c7ee71c6-sess20-29-identity-and-evidence-era.md`).
...
ABBA site** (`docs/history/docs/history/compiled-c7ee71c6-sess20-29-identity-and-evidence-era.md`).
...
- **The leaking site**, named by adding `demoter_comm/line/age_ms` to the probe: line 34955
  ...
  224 bails, 180 **seconds**, comm=mkdir (`docs/history/docs/history/docs/history/compiled-c7ee71c6-sess20-29-identity-and-evidence-era.md`).
...
112-126 hits per node on all 16 (`docs/history/docs/history/docs/history/compiled-c7ee71c6-sess20-29-identity-and-evidence-era.md`).
...
- **ILOCK/extent-map TOCTOU**: `xfs_ilock_data_map_shared` tests `xfs_need_iread_extents`
  *before* `xfs_ilock`, but MXFS's DLM acquire hook reloads the inode *inside* `xfs_ilock` —
  needs an aged cluster to reproduce (`docs/history/docs/history/compiled-c7ee71c6-sess20-29-identity-and-evidence-era.md`).
- **dir_epoch belongs to the inode NUMBER, not an incarnation** — tombstone/claim deliberately
  ...
  bug (`docs/history/docs/history/docs/history/compiled-c7ee71c6-sess20-29-identity-and-evidence-era.md`).
- **readdir's 200× `msleep(1)` retry can never land** — the caller already holds ILOCK_SHARED,
  ...
  (1201-1215ms every sample) was the tell (`docs/history/docs/history/docs/history/compiled-c7ee71c6-sess20-29-identity-and-evidence-era.md`).
...
- GPT's final-tenure narrowing (an inode joins the LRU only at `i_count==0`, so the survivor
  ...
  end** (`docs/history/docs/history/docs/history/compiled-c7ee71c6-sess20-29-identity-and-evidence-era.md`).
- The leak was also **decoupled** from the demoter clobber — reproduced on a build where both
  fixes were active (`docs/history/docs/history/docs/history/compiled-c7ee71c6-sess20-29-identity-and-evidence-era.md`).
...
- **An infinitely self-recursive inline** shipped in a pending build:
  ...
  proven by objdump, not theory (`docs/history/docs/history/docs/history/compiled-c7ee71c6-sess20-29-identity-and-evidence-era.md`).
...
- **A `grep -c` of 0 on a ring buffer is not absence.** A refutation that took minutes was
  ...
  damage (`docs/history/docs/history/docs/history/compiled-c7ee71c6-sess20-29-identity-and-evidence-era.md`).
