<!-- 2-node TCP harness bring-up + multi-node suite port; build B77AD901 five fixes; cache_coherency flaky = barrier desync from slow reloads, not corrupt… -->
...
Built and validated in `docs/history/docs/history/docs/history/compiled-tcp-2node-harness-cache-coherency.md`:
...
timeout (the derived-budget rule). Harness TODO (deferred): fail-fast coord abort when a node returns with no
...
### Crash caught first (build 305641B7) — `docs/history/docs/history/docs/history/compiled-tcp-2node-harness-cache-coherency.md`
...
### Build A70942DB — 3 proven root fixes (KEEP) — `docs/history/docs/history/compiled-tcp-2node-harness-cache-coherency.md`
...
3. **Reader verify-fail bypassed sess127 coordination.** `xfs/xfs_icache.c`: a new child's
   ...
   (clean 2-node reset + cross-node smoke, ~40s wall, NOT 320 — the derived-budget rule).

### Build B77AD901 — fixes 4 + 5 (KEEP) — `docs/history/docs/history/compiled-tcp-2node-harness-cache-coherency.md`, `docs/history/docs/history/compiled-tcp-2node-harness-cache-coherency.md`, `docs/history/docs/history/compiled-tcp-2node-harness-cache-coherency.md`
...
`docs/history/docs/history/compiled-tcp-2node-harness-cache-coherency.md` reliability data (warm mount, manual 2-node,
...
Ops are FAST — `docs/history/docs/history/docs/history/compiled-tcp-2node-harness-cache-coherency.md` measured (build B77AD901):
...
Root of the drift (three intertwined problems, `docs/history/docs/history/compiled-tcp-2node-harness-cache-coherency.md`):
1. **Slowness** — the `P-TCP-VERIFY-COORD` coordinated reload (DLM round-trip +
   ...
   23s-when-fast test drifts to 87s, desyncing barriers. the derived-budget rule: slowness IS a fail.
...
`docs/history/docs/history/compiled-tcp-2node-harness-cache-coherency.md` — ran the 5 ported tests twice + cache_coherency
...
## The suite port (PORTING, not net-new) — `docs/history/docs/history/compiled-tcp-2node-harness-cache-coherency.md`
...
no split-brain); scaling_curve ← scaling_curve.sh (perf, the derived-budget rule budget); dlm_scaling ← NONE,
...
User direction (`docs/history/docs/history/docs/history/compiled-tcp-2node-harness-cache-coherency.md`): don't tunnel on
