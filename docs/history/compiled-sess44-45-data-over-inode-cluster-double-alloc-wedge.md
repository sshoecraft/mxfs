<!-- sess44-45: inode-cluster wedge — corrupt/all-zeros/data-over-inode slots crash iget; ROOT = partial-iwrite dropping fresh free inodes; FIXED C0554679. -->
...
`docs/history/docs/history/docs/history/compiled-sess44-45-data-over-inode-cluster-double-alloc-wedge.md`.
...
- **All-zeros**: `xfs_inode_buf_verify` on block 0x87e7e0 (AG8 = test1's OWN, slot0 owns agno%4==0,
  ...
  leaked entries. See `docs/history/docs/history/compiled-sess44-45-data-over-inode-cluster-double-alloc-wedge.md`.
...
`docs/history/proven-data-over-inode-doublealloc-clobber-payload-visible.md`.

### sess45 REFINED ROOT (the instrument-first loop traced, refutes the clobber theory) + KEEP FIX
...
a clobbering write. See `docs/history/fix-partial-iwrite-skips-fresh-free-inodes-inode-alloc-buf.md`.
...
dir-slot clobber from `docs/history/docs/history/docs/history/compiled-sess44-45-data-over-inode-cluster-double-alloc-wedge.md` — watch in full
suite.
...
map is stale right after mount so the first cross-node create hits the hole. NEXT (the instrument-first loop): repro standalone
...
invalidation). See `docs/history/next-blocker-dabuf-map-hole-dir-extent-xfs-create-shutdown.md`.
...
### Recurring lessons
- Trust probe measurements over code-reading (the instrument-first loop): the double-alloc/clobber theory looked proven from
