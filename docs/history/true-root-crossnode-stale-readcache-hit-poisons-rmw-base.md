<!-- sess69 TRUE ROOT (proven): 4/tcp dir_reuse loss = cross-node STALE READ-CACHE HIT. A peer advances a dir block; this node's non-FUA read hits its sta… -->
...
Build DE3A7E21 (baseline + dirwr-gated probes; inert at production). Marker NOT written — criterion NOT met. This SUPERSEDES the write-side-dead-end conclusion: the write is downstream; the root is the read-side stale cross-node cache hit. See `docs/history/docs/history/docs/history/compiled-dirreuse-4tcp-xfsaild-reflush-stale-buffer.md`, `docs/history/docs/history/compiled-sess61-4node-dir-reuse-stale-shortform-reconversion-block0-clobber.md`.</body>
