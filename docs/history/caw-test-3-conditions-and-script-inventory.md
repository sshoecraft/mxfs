<!-- MXFS 3 deploy conditions (TCP / FC-sim passthrough / direct iSCSI+CAW). Setup scripts do host infra AND per-VM config, N=1..32. CAW bring-up scripts… -->
...
- Scripts NOT executed. Running scst_setup.sh TEARS DOWN the live 8/tcp LIO rig
  ...
  scst_unwedge recovers w/o reboot per the never-reboot-clyde rule). Run when ready to move off TCP.
...
See [[doc-enduser-iscsi-caw-setup]], `docs/history/docs/history/compiled-scst-iscsi-infra-wedge-recovery.md`,
[[infra-lio-for-tcp-scst-for-caw-rationale]], `docs/history/docs/history/compiled-scst-iscsi-infra-wedge-recovery.md`.
