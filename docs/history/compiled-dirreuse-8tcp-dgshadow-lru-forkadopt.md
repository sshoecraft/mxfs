<!-- sess17 ccloop: dg_shadow LRU + fork-adopt eliminated 8/tcp dir_reuse data-loss; residual = EX-contention slowness + leaf holes. -->
...
it is EX-contention slowness (the derived-budget rule) plus unhealed leaf-hash holes.
...
`docs/history/docs/history/docs/history/compiled-dirreuse-8tcp-dgshadow-lru-forkadopt.md`
`docs/history/docs/history/docs/history/compiled-dirreuse-8tcp-dgshadow-lru-forkadopt.md`
...
- Any PER-MODIFY staleness signal (master `dir_epoch`, `i_dlm_dir_gen` — both bumped by
  ...
  invalidate→cold-read storm → ~1900 DLM acquire timeouts (the derived-budget rule fail).
...
~67s/round (reached round 6 in ~400s) = the derived-budget rule slowness fail.
Profiling (the instrument-first loop): `P34-ACQ-SLOW` ino=131 median ~1.85s / max 2.8s = QUEUE wait (not
...
- **mht=300 (default): RDMISS=0 (data loss eliminated), but 8-node dir-EX STARVATION** →
  ...
  ~67s/round (round 6 of 24) = the derived-budget rule slowness + shutdown fail.
...
## GPT-5.5 design (design consult) — for the round-5+ incarnation-mismatch family
