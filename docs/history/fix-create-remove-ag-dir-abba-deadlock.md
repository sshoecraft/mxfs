<!-- sess58 PROVEN FIX (build 456774D1): xfs_remove was the lone dir→AG op; create(dialloc)+rename(preacquire) are AG→dir. Made remove pre-acquire child A… -->
## sess58 (ccloop 8ddb16a2) — PROVEN ROOT + FIX of the tcp_dlm_scaling AG-deadlock

### ROOT (captured directly, the instrument-first loop): create↔remove AG↔dir ABBA
...
### NEXT: run full `./run.sh 2 tcp` reliability loop. If tcp_dlm_scaling still fails it's the RESURRECTION (drain leftover, separate from this deadlock) — see `docs/history/docs/history/compiled-sess54-58-coresident-cluster-flush-ag-dir-abba-deadlock.md` (ICLUSTER lock).
