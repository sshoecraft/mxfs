<!-- sess111 design-consult ruling: my ctx->held mode-guard for blocker 6 is REJECTED (two more same-node races). Requires an authoritative per-resource local sta… -->
# sess111 — blocker 6 audit, measurement, and the design-consult ruling that rejected my fix
...
by construction and unobserved in practice. the zero-defect bar forbids closing on that.
