<!-- sess153: P248 design-consult ruling received (A approved+tripwires, B scoped to release_all -ESHUTDOWN retry x1). Exact edit list; dlm_caw.h fields LANDED. -->
# sess153 — P248 (D-RELEASEALL-LREQ-RETIRE-MISSING) GPT ruling + implementation plan for 0.11.455

## design consult DONE (gpt-5.6-sol, sess153 transcript, task k798fvv3c). Ruling digest
...
- Deterministic set on test1(+test2 for case 5) via knobs, script **tests/p248_inject.sh** (to write; the source-tree rule). Cases 1-6 above; case 5 needs 2-node contention (dir workload) + K5 mid-run.
