<!-- sess52 GPT design-consult ruling on step-4a: ctx-&gt;held alone is NOT a safe adopted-vs-orphan discriminator; adopt must CAS to serialise against the settle… -->
# sess52 — step-4a hardening after GPT review

Supersedes the settle design in
`docs/history/docs/history/docs/history/compiled-step4a-mount-recovery-barrier-campaign.md` (the sess51
*manifest-is-EX-only* finding itself stands and is confirmed).
Campaign: `docs/history/docs/history/compiled-foreign-replay-authority-tokens.md`.
...
## GPT's ruling (the design-consult rule)
