<!-- sess295: D-503 mechanism QUANTIFIED (convoy: 309ms fixed tenure x 32 rotation, per-create 5→50ms under load); GPT ruling = instrument timeline, nudge… -->
## sess295 — D-503 mechanism nailed from sess294's timeline; design-consult ruling obtained

### Refuted (the instrument-first loop 2a)
...
### GPT design-consult ruling (fix order)
...
2) Then B, then D, re-measure between each (the instrument-first loop).
